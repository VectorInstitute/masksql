...
from utils.classifier_metric.evaluator import auc_metric
...
def parse_option():
    ...
    parser.add_argument(
        "--device", type=str, default="3", help="the id of used GPU device."
    )
    parser.add_argument(
    ...
    parser.add_argument("--epochs", type=int, default=128, help="training epochs.")
    parser.add_argument(
        "--patience",
        type=int,
        default=32,
    ...
    parser.add_argument(
        "--output_filepath",
        type=str,
        default="data/pre-processing/dataset_with_pred_probs.json",
...
def _train(opt):
    ...
    tokenizer_class = (
        ...
        if "xlm" in opt.model_name_or_path
        else RobertaTokenizerFast
    )
...
def _test(opt):
    ...
    # load tokenizer
    tokenizer = tokenizer_class.from_pretrained(opt.save_path, add_prefix_space=True)
    ...
    # initialize model
    model = MyClassifier(
        model_name_or_path=opt.save_path, vocab_size=len(tokenizer), mode=opt.mode
    )
    ...
    if opt.mode == "train":
        ...
        for data_id, data in enumerate(dataset):
            ...
            if table_num == len(total_table_pred_probs[data_id]):
                ...
                table_pred_probs = total_table_pred_probs[data_id] + [
                    -1 for _ in range(table_num - len(total_table_pred_probs[data_id]))
                ]
            ...
            for table_id in range(table_num):
                if table_id >= len(total_column_pred_probs[data_id]):
                    ...
                    column_pred_probs.append(
                        [-1 for _ in range(len(data["column_labels"][table_id]))]
                    )
                    continue
                if len(total_column_pred_probs[data_id][table_id]) == len(
                    data["column_labels"][table_id]
                ):
                    ...
                    column_pred_probs.append(
                        total_column_pred_probs[data_id][table_id]
                        + [-1 for _ in range(truncated_column_num)]
                    )
        ...
        while len(truncated_data_info) != 0:
            ...
            for truncated_data_id, truncated_table_ids in truncated_data_info:
                ...
                truncated_data["db_schema"] = [
                    truncated_data["db_schema"][table_id]
                    for table_id in truncated_table_ids
                ]
                truncated_data["table_labels"] = [
                    truncated_data["table_labels"][table_id]
                    for table_id in truncated_table_ids
                ]
                truncated_data["column_labels"] = [
                    truncated_data["column_labels"][table_id]
                    for table_id in truncated_table_ids
                ]
                truncated_data["table_pred_probs"] = [
                    truncated_data["table_pred_probs"][table_id]
                    for table_id in truncated_table_ids
                ]
                truncated_data["column_pred_probs"] = [
                    truncated_data["column_pred_probs"][table_id]
                    for table_id in truncated_table_ids
                ]
            ...
            for data_id, data in enumerate(truncated_dataset):
                ...
                if table_num == len(total_table_pred_probs[data_id]):
                    ...
                    table_pred_probs = total_table_pred_probs[data_id] + [
                        -1
                        for _ in range(table_num - len(total_table_pred_probs[data_id]))
                    ]
                ...
                for table_id in range(table_num):
                    ...
                    if len(total_column_pred_probs[data_id][table_id]) == len(
                        ...
                        truncated_column_num = len(
                            data["column_labels"][table_id]
                        ) - len(total_column_pred_probs[data_id][table_id])
                        column_pred_probs.append(
                            total_column_pred_probs[data_id][table_id]
                            + [-1 for _ in range(truncated_column_num)]
                        )
                ...
                for idx, truncated_table_id in enumerate(truncated_table_ids):
                    dataset[truncated_data_id]["table_pred_probs"][
                        truncated_table_id
                    ] = table_pred_probs[idx]
                    dataset[truncated_data_id]["column_pred_probs"][
                        truncated_table_id
                    ] = column_pred_probs[idx]
            ...
            for data_id, data in enumerate(dataset):
                ...
                for table_id in range(table_num):
                    ...
                    if (
                        data["table_pred_probs"][table_id] != -1
                        and -1 in data["column_pred_probs"][table_id]
                    ):
                        truncated_table_ids.append(table_id)
