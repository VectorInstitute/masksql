...
def parse_option():
    parser = argparse.ArgumentParser(
        "command line arguments for generating the ranked dataset."
    )

    parser.add_argument(
        "--input_dataset_path",
        type=str,
        default="./data/pre-processing/dev_with_probs.json",
        help="filepath of the input dataset.",
    )
    parser.add_argument(
        "--output_dataset_path",
        type=str,
        default="./data/pre-processing/resdsql_dev.json",
...
def prepare_input_and_output(opt, ranked_data):
    ...
    for table_id in range(len(ranked_data["db_schema"])):
        ...
        for column_id in range(
            len(ranked_data["db_schema"][table_id]["column_names_original"])
        ):
            # extract column name
            column_name_original = ranked_data["db_schema"][table_id][
    ...
    if opt.output_skeleton:
        if opt.target_type == "sql":
            output_sequence = (
                ranked_data["sql_skeleton"] + " | " + ranked_data["norm_sql"]
            )
        elif opt.target_type == "natsql":
            output_sequence = (
                ranked_data["natsql_skeleton"] + " | " + ranked_data["norm_natsql"]
            )
    elif opt.target_type == "sql":
...
def generate_train_ranked_dataset(opt):
    ...
    for data_id, data in enumerate(dataset):
        ...
        if len(topk_table_ids) < opt.topk_table_num:
            remaining_table_ids = [
                idx
                for idx in range(len(data["table_labels"]))
                if idx not in topk_table_ids
            ]
            ...
            # otherwise, we randomly select some unused tables
                ...
                randomly_sampled_table_ids = random.sample(
                    remaining_table_ids, opt.topk_table_num - len(topk_table_ids)
                )
        ...
        for table_id in topk_table_ids:
            ...
            used_column_ids = [
                idx
                for idx, column_label in enumerate(data["column_labels"][table_id])
                if column_label == 1
            ]
            ...
            if len(topk_column_ids) < opt.topk_column_num:
                remaining_column_ids = [
                    idx
                    for idx in range(len(data["column_labels"][table_id]))
                    if idx not in topk_column_ids
                ]
                ...
                if opt.topk_column_num >= len(data["column_labels"][table_id]):
                    ...
                    randomly_sampled_column_ids = random.sample(
                        remaining_column_ids, opt.topk_column_num - len(topk_column_ids)
                    )
            ...
            new_table_info["db_contents"] = [
                data["db_schema"][table_id]["db_contents"][column_id]
                for column_id in topk_column_ids
            ]
        ...
        table_names_original = [
            table["table_name_original"] for table in data["db_schema"]
        ]
        needed_fks = []
        for fk in data["fk"]:
            source_table_id = table_names_original.index(
                fk["source_table_name_original"]
            )
            target_table_id = table_names_original.index(
                fk["target_table_name_original"]
            )
...
def generate_eval_ranked_dataset(opt):
    ...
    for data_id, data in enumerate(dataset):
        ...
        if opt.mode == "eval":
            ...
            table_coverage_state_list.append(
                lista_contains_listb(topk_table_ids, used_table_ids)
            )

            for idx in range(len(data["db_schema"])):
                used_column_ids = [
                    idx
                    for idx, label in enumerate(data["column_labels"][idx])
                    if label == 1
                ]
                ...
                column_coverage_state_list.append(
                    lista_contains_listb(topk_column_ids, used_column_ids)
                )
