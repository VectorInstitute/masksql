...
def find_most_similar_sequence(source_sequence, target_sequences):
    ...
    for target_sequence in target_sequences:
        match_length = (
            SequenceMatcher(None, source_sequence, target_sequence)
            .find_longest_match(0, len(source_sequence), 0, len(target_sequence))
            .size
        )
...
def tokenize_natsql(natsql):
    ...
    for start_id, end_id in zip(
        string_value_start_end_ids[0::2], string_value_start_end_ids[1::2]
    ):
    ...
    for string_value in set(string_values):
        natsql = natsql.replace(string_value, "'string_value_placeholder'")
...
def fix_fatal_errors_in_natsql(natsql, tc_original):
    ...
    for idx, token in enumerate(natsql_tokens):
        ...
        if (
            ...
            if (
                current_table_name in table_names
                and current_column_name in column_names
                or current_table_name not in table_names
                and current_column_name in column_names
            ):
                ...
                new_table_name = find_most_similar_sequence(
                    current_table_name, candidate_table_names
                )
                new_tokens.append(new_table_name + "." + current_column_name)
            # case 3: table name is correct but column name is not existing
            elif (
                current_table_name in table_names
                and current_column_name not in column_names
            ):
                ...
                new_column_name = find_most_similar_sequence(
                    current_column_name, candidate_column_names
                )
                new_tokens.append(current_table_name + "." + new_column_name)
            # case 4: both table name and column name are not existing
            elif (
                current_table_name not in table_names
                and current_column_name not in column_names
            ):
                new_column_name = find_most_similar_sequence(
...
def decode_natsqls(
    ...
    for batch_id in range(batch_size):
        ...
        # print("\n".join(tokenizer.batch_decode(
        #     generator_outputs[batch_id, :, :], skip_special_tokens = True)))

        for seq_id in range(num_return_sequences):
            cursor = get_cursor_from_path(db_file_path)
            pred_sequence = tokenizer.decode(
                generator_outputs[batch_id, seq_id, :], skip_special_tokens=True
            )

            pred_natsql = pred_sequence.split("|")[-1].strip()
            pred_natsql = (
                pred_natsql.replace("='", "= '").replace("!=", " !=").replace(",", " ,")
            )
            old_pred_natsql = pred_natsql
            # if the predicted natsql has some fatal errors, try to correct it
            pred_natsql = fix_fatal_errors_in_natsql(
                pred_natsql, batch_tc_original[batch_id]
            )
            if old_pred_natsql != pred_natsql:
                print("Before fix:", old_pred_natsql)
                print("After fix:", pred_natsql)
                print("---------------")
            pred_sql = natsql_to_sql(
                pred_natsql, db_id, db_file_path, table_dict[db_id]
            ).strip()
...
def decode_sqls(
    db_path, generator_outputs, batch_db_ids, batch_inputs, tokenizer, batch_tc_original
):
    batch_size = generator_outputs.shape[0]
    num_return_sequences = generator_outputs.shape[1]
    ...
    for batch_id in range(batch_size):
        ...
        for seq_id in range(num_return_sequences):
            ...
            pred_sql = (
                pred_sql.replace("='", "= '").replace("!=", " !=").replace(",", " ,")
            )
