...
from utils.text2sql_decoding_utils import decode_natsqls, decode_sqls


def parse_option():
    parser = argparse.ArgumentParser(
        "command line arguments for fine-tuning pre-trained language model."
    )
    ...
    parser.add_argument(
        "--device", type=str, default="2", help="the id of used GPU device."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="tensorboard_log/text2sql",
    ...
    parser.add_argument(
        "--train_filepath",
        type=str,
        default="data/preprocessed_data/resdsql_train_spider.json",
        help="file path of test2sql training set.",
    )
    parser.add_argument(
        "--dev_filepath",
        type=str,
        default="data/preprocessed_data/resdsql_dev.json",
    ...
    parser.add_argument(
        "--db_path", type=str, default="database", help="file path of database."
    )
    parser.add_argument(
        "--tables_for_natsql",
        type=str,
        default="NatSQL/NatSQLv1_6/tables_for_natsql.json",
...
def _train(opt):
    ...
    train_dataset = Text2SQLDataset(dir_=opt.train_filepath, mode="train")
    ...
    model_class = (
        MT5ForConditionalGeneration
        if "mt5" in opt.model_name_or_path
...
def _test(opt):
    ...
    tokenizer = T5TokenizerFast.from_pretrained(opt.save_path, add_prefix_space=True)

    if isinstance(tokenizer, T5TokenizerFast):
        tokenizer.add_tokens([AddedToken(" <="), AddedToken(" <")])

    dev_dataset = Text2SQLDataset(dir_=opt.dev_filepath, mode=opt.mode)
    ...
    for batch in tqdm(dev_dataloder):
        ...
        with torch.no_grad():
            ...
            model_outputs = model_outputs.view(
                len(batch_inputs), opt.num_return_sequences, model_outputs.shape[1]
            )
