...
def construct_hyper_param():
    ...
    parser.add_argument(
        "--table_file", default="NatSQLv1_6/tables_for_natsql.json", type=str
    )
    parser.add_argument(
        "--natsql_file",
        default="NatSQLv1_6/dev.json",
        type=str,
    ...
    for i, sql in enumerate(sqls):
        ...
        try:
            query, _, __ = create_sql_from_natSQL(
                sql["NatSQL"],
                sql["db_id"],
                "data/spider/database/" + sql["db_id"] + "/" + sql["db_id"] + ".sqlite",
                table_dict[sql["db_id"]],
                sq,
                remove_values=args.test_executable_natsql,
