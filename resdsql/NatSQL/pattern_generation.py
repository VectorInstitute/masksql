...
from natsql2sql.preprocess.col_match import col_match_main
from natsql2sql.preprocess.match import (
...
from natsql2sql.preprocess.sq import QuestionSQL, SubQuestion
from natsql2sql.preprocess.stemmer import MyStemmer
from natsql2sql.preprocess.TokenString import SToken, TokenString, get_spacy_tokenizer
from natsql2sql.preprocess.utils import (
    get_all_col_from_sq,
    get_all_table_from_sq,
    is_there_sgrsm_and_gr_or_sm,
    look_for_table_idx,
    sjjs_table,
    str_is_date,
)
...
def construct_hyper_param():
    ...
    parser.add_argument(
        "--table_file",
        default="NatSQLv1_6/tables.json",
        type=str,
...
def jjs_not_order(real_tp_str, tp_str2):
    # two col
    if (
        "AGG and JJS" not in tp_str2
        and "AGG and the JJS" not in tp_str2
        and "JJS and AGG" not in tp_str2
        and "JJS and the JJS" not in tp_str2
        and "JJS and JJS" not in tp_str2
        and "JJS SC and JJS S" not in tp_str2
        and "JJS STC and JJS S" not in tp_str2
        and "AGG SC and JJS S" not in tp_str2
    ):
        pass
    else:
        return False
    ...
    if (
        "AGG SC JJS SC JJS S" in real_tp_str
        or "AGG SC AGG SC JJS S" in real_tp_str
        or "AGG SC JJS SC AGG S" in real_tp_str
        or "JJS SC AGG SC AGG S" in real_tp_str
        or "JJS SC AGG SC JJS S" in real_tp_str
        or "JJS SC JJS SC AGG S" in real_tp_str
        or "JJS SC JJS SC JJS S" in real_tp_str
    ):
        return False
    return True
...
def others_where_order_analyse(token_pattern, question, select_q, select):
    ...
    for i, tps in enumerate(token_pattern):
        ...
        if (
            ...
            or "from GR_JJS NUM" in real_tp_str
        ) and ("first" in question[i] or "one" in question[i]):
            return_result[i][0] = 1
            return_result[i][2] = 1
        elif (
            (
                "IN SM_JJS COL NUM" in real_tp_str
                or "from SM_JJS SM_JJS NUM" in real_tp_str
                or "from SM_JJS NUM" in real_tp_str
            )
            and ("first" in question[i] or "one" in question[i])
            or "from " in real_tp_str
            and (
                " SM_JJS GR_JJS" in real_tp_str
                or " SM_GRSM GR_GRSM" in real_tp_str
                or " SM_JJS GR_GRSM" in real_tp_str
                or " SM_GRSM GR_JJS" in real_tp_str
            )
        ):
            return_result[i][0] = 1
        elif "from " in real_tp_str and (
            " GR_JJS SM_JJS" in real_tp_str
            or " GR_GRSM SM_GRSM" in real_tp_str
            or " GR_JJS SM_GRSM" in real_tp_str
            or " GR_GRSM SM_JJS" in real_tp_str
        ):
            return_result[i][0] = 1
            return_result[i][2] = 1
        else:
            ...
            if (
                tps[0] == "order"
                or "order by" in question[i]
                or "ordered by" in question[i]
                or "in order" in question[i]
                or "in the order of " in question[i]
                or "order the result by " in question[i]
                or "ordered the result by " in question[i]
                or "order the results by " in question[i]
                or "ordered the results by " in question[i]
            ):
                return_result[i][0] = 1
            if "descend" in tps or "descending" in tps:
                ...
                "ascend" in tps
                or "alphabetical" in tps
                or "ascending" in tps
                or "lexicographic" in tps
            ):
                return_result[i][0] = 1
            elif (
                "NOT" not in tps
                ...
                and "GR_JJS" in tps
                and (
                    ("GRSM" not in tp_str and " at most " not in question[i])
                    or before_or_after(tps, idx_jjr, "NUM")
                )
                and not (
                    real_tp_str.count(" ") <= 2 and not not_in_all_q(select_q, " each ")
                )
                and (
                    "COL" in real_tp_str
                ...
                and "SM_JJS" in tps
                and (
                    ("GRSM" not in tp_str and " at least " not in question[i])
                    or before_or_after(tps, idx_jjr, "NUM")
                )
                and not (
                    real_tp_str.count(" ") <= 2 and not not_in_all_q(select_q, " each ")
                ...
                and "NUM" in tps
                or (
                    "GR_JJS GR_JJS" in real_tp_str
                    and (
                        " most frequent" in question[i]
                        or " most common" in question[i]
                        or " most popular" in question[i]
                    )
                )
                or ("GR_JJS frequent" in real_tp_str)
                or ("GR_JJS common" in real_tp_str)
                or ("GR_JJS popular" in real_tp_str)
            ):
                return_result[i][0] = 1
                return_result[i][1] = 1
                return_result[i][2] = 1
                ...
                (
                    "SM_JJS SM_JJS" in real_tp_str
                    and (
                        " least frequent" in question[i]
                        or " least common" in question[i]
                        or " least popular" in question[i]
                    )
                )
                or ("SM_JJS frequent" in real_tp_str)
                or ("SM_JJS common" in real_tp_str)
                or ("SM_JJS popular" in real_tp_str)
                or "SM_JJS AGG " in real_tp_str
                and " number of " in question[i]
            ):
                return_result[i][0] = 1
                return_result[i][1] = 1
                ...
                "NOT" not in tps
                and (" maximum " in question[i] or " minimum " in question[i])
                and "than" not in tps
                and "GRSM" not in tp_str
                and "AGG" in tps
            ):
                qtoks = question[i].split(" ")
                idx_m = (
                    qtoks.index("maximum")
                    if " maximum " in question[i]
                    else qtoks.index("minimum")
                )
                if tps[idx_m] == "AGG":
                    return_result[i][0] = 1
                    return_result[i][1] = 1
        ...
        if return_result[i][0] == 0 and return_result[i][3] == 0:
            if "order" in tps or "sort" in tps:
                ...
                " majority of " in question[i]
                or " most " in question[i]
                and "GRSM" not in tp_str
                and "at most " not in question[i]
            ):
                return_result[i][0] = 1
                return_result[i][1] = 1
                return_result[i][2] = 1
                ...
                " least " in question[i]
                and "GRSM" not in tp_str
                and "at least " not in question[i]
            ):
                return_result[i][0] = 1
                return_result[i][1] = 1
        ...
        if return_result[i][1] == 1 and "JJS" in tp_str:
            if (
                "starting with" not in question[i]
                and "starting from" not in question[i]
                and "start with" not in question[i]
                and "start from" not in question[i]
                and "starts with" not in question[i]
                and "starts from" not in question[i]
                and "started with" not in question[i]
                and "started from" not in question[i]
            ):
                pass
            else:
                return_result[i][1] = 0
        ...
        if return_result[i][0] and "from GR_GRSM" in tp_str:
            return_result[i][2] = 1
        if (
            "start from" not in tp_str
            and return_result[i][0]
            and "NUM" in tp_str
            and "IN GR_JJS COL NUM" not in real_tp_str
            and "IN SM_JJS COL NUM" not in real_tp_str
            and "from GR_JJS GR_JJS NUM" not in real_tp_str
            and "from GR_JJS NUM" not in real_tp_str
            and "from SM_JJS SM_JJS NUM" not in real_tp_str
            and "from SM_JJS NUM" not in real_tp_str
        ):
            return_result[i][1] = 1
...
def select_where_order_analyse(token_pattern, question, select):
    ...
    limit = 0
    desc = 0
    return_result = [[order, limit, desc, where]] * len(
        token_pattern
    )  # order limit desc(1)/asc
    for i, tps in enumerate(token_pattern):
        tp_str = " ".join(tps)
        tp_str2 = ""
        ...
        if (
            ...
            for j, p in enumerate(tps):
                if j > 0 and tps[j - 1] == p:
                    continue
                if p in ["AGG", "SM_SJJS", "SM_JJS", "GR_SJJS", "GR_JJS"]:
                    jjs_num += 1
                if (
                    jjs_num
                    and col_table_after_jjs
                    and p in ["ST", "STC", "SC", "COL", "TABLE", "TABLE-COL"]
                ):
                    col_table_after_jjs = 2
                elif (
                    not jjs_num
                    and not col_table_after_jjs
                    and p in ["ST", "STC", "SC", "COL", "TABLE", "TABLE-COL"]
                ):
            ...
            if "descend" in tps or "descending" in tps:
                ...
                and "GRSM" not in tp_str
                and " at most " not in question[i]
                and not_in_all_q(question, " each ")
                and not_in_all_q(question, "for every")
                and not_in_all_q(question, "For every")
                and jjs_not_order(real_tp_str, tp_str2)
                and (
                    len(select) != 1
                    or col_table_after_jjs == 2
                    or " most common" in question[i]
                )
                and jjs_num == 1
            ):
                ...
                ("top" in tps and "NUM" in tps)
                or " top SC by " in tp_str2
                or " top STC by " in tp_str2
                or " most common " in question[i]
                or "NUM GR_JJS" in tp_str
                or "NUM GR_SJJS" in tp_str
                or "GR_JJS NUM" in tp_str
                or "GR_SJJS NUM" in tp_str
            ):
                return_result[i][0] = 1
                return_result[i][1] = 1
                return_result[i][2] = 1
            elif (
                "NUM SM_JJS" in tp_str
                or "NUM SM_SJJS" in tp_str
                or "SM_JJS NUM" in tp_str
                or "SM_SJJS NUM" in tp_str
            ):
                return_result[i][0] = 1
                return_result[i][1] = 1
            elif (
                (
                    ("GR_JJS GR_JJS " in real_tp_str or "most GR_JJS" in real_tp_str)
                    and (
                        " most frequent" in question[i]
                        or " most common" in question[i]
                        or " most popular" in question[i]
                ...
                (
                    ("SM_JJS SM_JJS " in real_tp_str or "least SM_JJS " in real_tp_str)
                    and (
                        " least frequent" in question[i]
                        or " least common" in question[i]
                        or " least popular" in question[i]
                ...
                " most " in question[i]
                and jjs_num == 1
                and "GR_JJS JJ S" in real_tp_str
            ):
                return_result[i][0] = 1
                return_result[i][1] = 1
                return_result[i][2] = 1
                ...
                " least " in question[i]
                and jjs_num == 1
                and "SM_JJS JJ S" in real_tp_str
                or "SM_JJS AGG " in real_tp_str
                and " number of " in question[i]
            ):
                return_result[i][0] = 1
                ...
                "NUM S" in real_tp_str
                and "name" not in question[i]
                and "grade" not in question[i]
                and real_tp_str.count("NUM") == 1
            ):
                real_tp_str = []
...
def reset_frist_last_col_sq(
    ...
    yes = False
    if (
        idx_first + 1 < len(real_tp_list)
        and real_tp_list[idx_first + 1][0] in next_allow_p_tok
    ):
        target_idx = idx_first + 1
    elif (
        idx_first + 2 < len(real_tp_list)
        and real_tp_list[idx_first + 1][0] in ["NUM", "*"]
        and real_tp_list[idx_first + 2][0] in next_allow_p_tok
    ):
    ...
    if target_idx >= 0:
        ...
        if yes:
            for k in reversed(
                range(len(sq.col_match[list_idx[i]][real_tp_list[target_idx][1]][0]))
            ):
                col = sq.col_match[list_idx[i]][real_tp_list[target_idx][1]][0][k]
                if (
                    schema.column_types[col] in ["text", "number"]
                    and schema.column_types[col] != in_col_type
                ):
            ...
            sq.question_tokens[list_idx[i]][real_tp_list[idx_first][1]] = SToken(
                text=new_word, lemma=new_lemma
            )
...
def question_modify(token_pattern, question, list_idx, sq, schema):
    for i, tps in enumerate(token_pattern):
        ...
        if (
            ...
            or "from GR_JJS GR_JJS NUM" in real_tp_str
            or "from GR_JJS NUM" in real_tp_str
            or "IN GR_JJS GR_JJS NUM" in real_tp_str
            or "IN GR_JJS NUM" in real_tp_str
        ) and ("first" in question[i] or "one" in question[i]):
            if "IN GR_JJS COL NUM" in real_tp_str:
                idx = real_tp_str.index("IN GR_JJS COL NUM")
                left_str = real_tp_str[:idx]
                idx = left_str.count(" ")
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 1][1]] = SToken(
                    text="descending", lemma="descend"
                )
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 3][1]] = SToken(
                    text="order", lemma="order"
                )
                sq.col_match[list_idx[i]][real_tp_list[idx + 1][1]] = []
                sq.col_match[list_idx[i]][real_tp_list[idx + 3][1]] = []
            elif (
                "from GR_JJS GR_JJS NUM" in real_tp_str
                or "IN GR_JJS GR_JJS NUM" in real_tp_str
            ):
                idx = (
                    real_tp_str.index("from GR_JJS GR_JJS NUM")
                    if "from GR_JJS GR_JJS NUM" in real_tp_str
                    else real_tp_str.index("IN GR_JJS GR_JJS NUM")
                )
                left_str = real_tp_str[:idx]
                idx = left_str.count(" ")
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 1][1]] = SToken(
                ...
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 2][1]] = SToken(
                    text="to", lemma="to"
                )
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 3][1]] = SToken(
                    text="end", lemma="end"
                ...
                idx = (
                    real_tp_str.index("from GR_JJS NUM")
                    if "from GR_JJS NUM" in real_tp_str
                    else real_tp_str.index("IN GR_JJS NUM")
                )
                left_str = real_tp_str[:idx]
                idx = left_str.count(" ")
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 1][1]] = SToken(
        ...
        if (
            ...
            or "from SM_JJS SM_JJS NUM" in real_tp_str
            or "from SM_JJS NUM" in real_tp_str
            or "IN SM_JJS SM_JJS NUM" in real_tp_str
            or "IN SM_JJS NUM" in real_tp_str
        ) and ("first" in question[i] or "one" in question[i]):
            if "IN SM_JJS COL NUM" in real_tp_str:
                ...
                "from SM_JJS SM_JJS NUM" in real_tp_str
                or "IN SM_JJS SM_JJS NUM" in real_tp_str
            ):
                idx = (
                    real_tp_str.index("from SM_JJS SM_JJS NUM")
                    if "from SM_JJS SM_JJS NUM" in real_tp_str
                    else real_tp_str.index("IN SM_JJS SM_JJS NUM")
                )
                left_str = real_tp_str[:idx]
                idx = left_str.count(" ")
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 1][1]] = SToken(
                ...
                idx = (
                    real_tp_str.index("from SM_JJS NUM")
                    if "from SM_JJS NUM" in real_tp_str
                    else real_tp_str.index("IN SM_JJS NUM")
                )
                left_str = real_tp_str[:idx]
                idx = left_str.count(" ")
                sq.question_tokens[list_idx[i]][real_tp_list[idx + 1][1]] = SToken(
        ...
        if "from " in real_tp_str and (
            ...
            sq.question_tokens[list_idx[i]][real_tp_list[idx][1]] = SToken(
                text="ascending", lemma="ascend"
            )
            sq.question_tokens[list_idx[i]][real_tp_list[idx + 1][1]] = SToken(
                text="end", lemma="end"
            ...
            sq.col_match[list_idx[i]][real_tp_list[idx][1]] = []
            sq.col_match[list_idx[i]][real_tp_list[idx + 1][1]] = []
        if " first " in question[i] and not re.fullmatch(
            r".*?\sfirst\s(([a-z]*|,)\s){0,4}name(s){0,1}\s.*", question[i]
        ):
            idx_first = -1
            for k, t_idx in enumerate(real_tp_list):
                if (
                    t_idx[0] == "NUM"
                    and sq.question_tokens[list_idx[i]][t_idx[1]].text == "first"
                ):
                    idx_first = k
                    break
            if idx_first >= 0:
                sq, yes = reset_frist_last_col_sq(
                    sq,
                    idx_first,
                    list_idx,
                    i,
                ...
                if yes:
                    continue
        if " last " in question[i] and not re.fullmatch(
            r".*?\slast\s(([a-z]*|,)\s){0,3}name(s){0,1}\s.*", question[i]
        ):
            idx_first = -1
            for k, t_idx in enumerate(real_tp_list):
                if (
                    t_idx[0] in ["last", "DATE", "YEAR"]
                    and sq.question_tokens[list_idx[i]][t_idx[1]].text == "last"
                ):
                    idx_first = k
                    break
            if idx_first >= 0:
        ...
        for_break = False
        if (
            re.fullmatch(r".*(start|end)(.){0,5}with.*", question[i])
            and "SM_JJS" not in tps
            and "GR_JJS" not in tps
        ):
            if " tilt " in question[i]:
                continue
            target = ["start", "end"]
            for j, tok in zip(
                range(len(sq.question_tokens[list_idx[i]]) - 1, -1, -1),
                reversed(sq.question_tokens[list_idx[i]]),
            ):
                for t in target:
                    if t in tok.text:
            ...
            re.fullmatch(
                r"(.*?(contain|includ|with\s|without\s|have\s|has\s|having\s|had\s|like|\sas\s)|as\s).*(substring|string|texts\s|text\s|char\s|chars\s|character\s|letter|word|phrase|prefix|suffix).*",
                question[i],
            )
            or re.fullmatch(r".*(contain|includ|like)(.){0,4}(').*?(').*", question[i])
            or re.fullmatch(
            ...
            for j, tok in zip(
                ...
                for t in target:
                    if (
                        tok.text.startswith(t) and tok.text != "without"
                    ) or t == tok.text:
                        if t == "without" and j + 1 < len(
                            sq.question_tokens[list_idx[i]]
                        ):
            ...
            for j, tok in zip(
                ...
                for t in target:
                    if tok.text.startswith(t):  # if t in tok.text:
                        if (
                            j + 1 < len(tps)
                            and tps[j + 1] in ["SDB", "DB", "PDB", "UDB"]
                            and t in ["with", "have", "has", "having", "had"]
                        ):
                            break
                        if j + 1 < len(tps) and tps[j + 1] in [
                            "COL",
                            "STC",
                            "SC",
                            "TABLE-COL",
                        ...
                        if (
                            j + 1 < len(tps)
                            and not sq.question_tokens[list_idx[i]][
                                j + 1
                            ].text.islower()
                        ):
                            if tok.text.endswith("ing"):
                                sq.question_tokens[list_idx[i]][j] = SToken(
                                    text="tilting"
                                )
                            elif tok.text.endswith("ed"):
                                sq.question_tokens[list_idx[i]][j] = SToken(
                ...
                if for_break:
                    break
        elif "DB" in real_tp_str and (
            real_tp_str.endswith("TABLE PDB")
            or real_tp_str.endswith("TABLE PDB PDB")
            or real_tp_str.endswith("TABLE PDB PDB PDB")
            or real_tp_str.endswith("TABLE PDB PDB PDB PDB")
            or real_tp_str.endswith("TABLE PDB PDB PDB PDB PDB")
            or real_tp_str.endswith("TABLE SDB")
            or real_tp_str.endswith("TABLE SDB SDB")
            or real_tp_str.endswith("TABLE SDB SDB SDB")
            or real_tp_str.endswith("TABLE SDB SDB SDB SDB")
            or real_tp_str.endswith("TABLE SDB SDB SDB SDB SDB")
            or real_tp_str.endswith("TABLE UDB")
            or real_tp_str.endswith("TABLE UDB UDB")
            or real_tp_str.endswith("TABLE UDB UDB UDB")
            or real_tp_str.endswith("TABLE UDB UDB UDB UDB")
            or real_tp_str.endswith("TABLE UDB UDB UDB UDB UDB")
            or real_tp_str.endswith("TABLE PDB NN")
            or real_tp_str.endswith("TABLE PDB PDB NN")
            or real_tp_str.endswith("TABLE PDB PDB PDB NN")
            or real_tp_str.endswith("TABLE PDB PDB PDB PDB NN")
            or real_tp_str.endswith("TABLE PDB PDB PDB PDB PDB NN")
            or real_tp_str.endswith("TABLE SDB NN")
            or real_tp_str.endswith("TABLE SDB SDB NN")
            or real_tp_str.endswith("TABLE SDB SDB SDB NN")
            or real_tp_str.endswith("TABLE SDB SDB SDB SDB NN")
            or real_tp_str.endswith("TABLE SDB SDB SDB SDB SDB NN")
            or real_tp_str.endswith("TABLE UDB NN")
            or real_tp_str.endswith("TABLE UDB UDB NN")
            or real_tp_str.endswith("TABLE UDB UDB UDB NN")
            or real_tp_str.endswith("TABLE UDB UDB UDB UDB NN")
            or real_tp_str.endswith("TABLE UDB UDB UDB UDB UDB NN")
            or real_tp_str.endswith("TABLE * PDB NN")
            or real_tp_str.endswith("TABLE * PDB PDB NN")
            or real_tp_str.endswith("TABLE * PDB PDB PDB NN")
            or real_tp_str.endswith("TABLE * PDB PDB PDB PDB NN")
            ...
            or real_tp_str.endswith("TABLE * UDB UDB UDB UDB NN")
            or real_tp_str.endswith("TABLE * UDB UDB UDB UDB UDB NN")
            or real_tp_str.endswith("TABLE PDB *")
            or real_tp_str.endswith("TABLE PDB PDB *")
            or real_tp_str.endswith("TABLE PDB PDB PDB *")
            or real_tp_str.endswith("TABLE PDB PDB PDB PDB *")
            or real_tp_str.endswith("TABLE PDB PDB PDB PDB PDB *")
            ...
            or real_tp_str.endswith("TABLE UDB UDB UDB UDB *")
            or real_tp_str.endswith("TABLE UDB UDB UDB UDB UDB *")
            or real_tp_str.endswith("PDB TABLE")
            or real_tp_str.endswith("UDB TABLE")
            or real_tp_str.endswith("SDB TABLE")
        ):
            if (
                real_tp_str.endswith("PDB TABLE")
                or real_tp_str.endswith("UDB TABLE")
                or real_tp_str.endswith("SDB TABLE")
            ):
        ...
        elif "NUM" in real_tp_str and (
            real_tp_str.endswith("ST NUM") or real_tp_str.endswith("TABLE NUM")
        ):
                    ...
                    for tb in sq.table_match[jj][kk]:
                        if tb:
                            sq = add_name_col(
                                ...
                                table_idx,
                                [
                                    sq.question_tokens[list_idx[i]][table_idx].lemma_
                                    + " id"
                                ],
                                table_prefix=schema.table_tokens_lemma_str[tb].split(
                                    " | "
                                )[0],
                            )
        ...
        if (
            re.fullmatch(
                r"(.*?\s(not|no)|not|no)\s(has\s|have\s|had\s|having\s){0,1}[a-z]{1,10}\sthan\s.*",
                question[i],
            )
            and tp_str.count("GRSM") == 1
            and tp_str.count("GR_GRSM") == 1
            and tp_str.count("NOT") == 1
        ):
            sq.question_tokens[list_idx[i]][tps.index("NOT")] = SToken(text="the")
            sq.question_tokens[list_idx[i]][tps.index("GR_GRSM")] = SToken(
                text="uglier"
            )
    return sq
...
def not_col_sub_q(sq, schema):
    for i, cols in enumerate(sq.col_match):
        if (
            cols.count([]) == len(cols)
            and i >= 1
            and sq.table_match[i - 1][-1]
            and (
                "PDB" in sq.pattern_tok[i]
                or "SDB" in sq.pattern_tok[i]
                or "UDB" in sq.pattern_tok[i]
            )
        ):
            ...
            if db and str_is_date(qt.text, qts, i):
                ...
                for j in range(i - 1, -1, -1):
                    if qts[j].text in ["before", "after"]:
                        ...
                        break
                    if (
                        qts[j].text in ["and", "or", ",", "in", "on", "at", "equal"]
                        or qts[j].text in S_ADJ_WORD_DIRECTION
                        or qts[j].text in ABSOLUTELY_GRSM_DICT
                    ):
                        break
                if delete_db:
            ...
            if "SJJS" in tp or "SGRSM" in tp:
                ...
                if j + 1 < len(tps) and tps[j + 1] == "TABLE":
                    ...
                    for t in sq.table_match[list_idx[i]][j + 1]:
                        re_list_idx, re_list_dir = sjjs_table(
                            sq.question_tokens[list_idx[i]][j],
                            schema.tbl_col_tokens_lemma_str[t],
                            schema.tbl_col_idx_back[t],
                        )
                ...
                result = is_there_sgrsm_and_gr_or_sm(
                    sq.question_tokens[list_idx[i]],
                    sq.question_tokens[list_idx[i]][j],
                    j,
                )
                if not sq.col_match[list_idx[i]][j] and (
                    not result
                    or sq.col_match[list_idx[i]].count([])
                    == len(sq.col_match[list_idx[i]])
                ):
                    table_idxs = []
                    ...
                    agg_id, col_id, sgrsm_word = get_AWD_column(
                        sq.question_tokens[list_idx[i]][j].lemma_,
                        table_idxs,
                        schema,
                        restrict=True,
                        re_all_word=True,
...
def add_name_col(
    ...
    def check_words_match(col_w, words):
        if len(words) == 4:
            ...
            for cw in col_w.split(" | "):
                for w in words:
                    if w[0] == " " and cw.endswith(w) or cw == w:
                        return True
        return False

    for k, cc in enumerate(schema.tbl_col_tokens_lemma_str[tb_idx]):
        if (
            check_words_match(cc, words)
            or (table_prefix and check_words_match(table_prefix + " " + cc, words))
            or (pk and schema.tbl_col_idx_back[tb_idx][k] in schema.primaryKey)
        ):
            if sq.col_match[sq_idx][added_col_idx] == []:
                sq.col_match[sq_idx][added_col_idx] = [
                    [schema.tbl_col_idx_back[tb_idx][k]],
                    [1],
                    [1],
                    [schema.tbl_col_idx_back[tb_idx][k]],
                ]
                ...
                schema.tbl_col_idx_back[tb_idx][k]
                not in sq.col_match[sq_idx][added_col_idx][0]
            ):
                sq.col_match[sq_idx][added_col_idx][0].append(
                    schema.tbl_col_idx_back[tb_idx][k]
                )
                sq.col_match[sq_idx][added_col_idx][1].append(1)
                sq.col_match[sq_idx][added_col_idx][2].append(1)
                sq.col_match[sq_idx][added_col_idx][3].append(
                    schema.tbl_col_idx_back[tb_idx][k]
                )
    return sq
...
def select_col_and_col_of(token_pattern, question, list_idx, sq, schema, select_num):
    ...
    for i, tps in enumerate(token_pattern):
        ...
        for j, tp, q in zip(range(len(tps)), tps, question[i].lower().split(" ")):
            ...
            index_list.append(j)
        patterns = [
            "COL and COL COL of COL",
            "COL and COL of COL",
            "COL and COL COL of TABLE",
            "COL and COL of TABLE",
            "COL and COL COL of all COL",
            "COL and COL of all COL",
            "COL and COL COL of all TABLE",
            "COL and COL of all TABLE",
            "COL and COL COL IN COL",
            "COL and COL IN COL",
            "COL and COL IN TABLE",
            "COL and COL COL IN TABLE",
            "COL and COL COL IN all COL",
            "COL and COL IN all COL",
            "COL and COL IN all TABLE",
            "COL and COL COL IN all TABLE",
        ]
        for p in patterns:
            if p in tp_str2 and tp_str2.count(p) == 1:
                ...
                if lfidx > 0 and tps[lfidx - 1] in ["TABLE", "TABLE-COL"]:
                    ...
                    lfidx > 1
                    and sq.question_tokens[list_idx[i]][lfidx - 1].text == "with"
                    and tps[lfidx - 2] in ["TABLE", "TABLE-COL"]
                ):
                    ridx = lfidx - 2
                    tables = sq.table_match[list_idx[i]][ridx]
                ...
                if (
                    sq.col_match[list_idx[i]][lfidx + 2]
                    and col_not_in_tables(
                        sq.col_match[list_idx[i]][lfidx + 2], tables, schema
                    )
                ) or (
                    sq.col_match[list_idx[i]][lfidx + 3]
                    and col_not_in_tables(
                        sq.col_match[list_idx[i]][lfidx + 3], tables, schema
                    )
                ):
                    break
                ts = TokenString(
                    None,
                    [
                        sq.question_tokens[list_idx[i]][ridx],
                        sq.question_tokens[list_idx[i]][lfidx],
                    ],
                )
                full_match = col_match_main(
                    tables,
                    ts,
                    schema,
                    [
                        sq.table_match[list_idx[i]][ridx],
                        sq.table_match[list_idx[i]][lfidx],
                    ],
                    True,
                )
                if (
                ...
                if not sq.col_match[list_idx[i]][lfidx] or (
                    full_match[1]
                    and max(full_match[1][1])
                    >= max(sq.col_match[list_idx[i]][lfidx][1])
                ):
                ...
                if full_match[0]:
                    if sq.col_match[list_idx[i]][ridx] == []:
                        ...
                        for f in range(len(full_match[0][0])):
                            if (
                                full_match[0][0][f]
                                not in sq.col_match[list_idx[i]][ridx][0]
                            ):
                                sq.col_match[list_idx[i]][ridx][0].append(
                                    full_match[0][0][f]
                                )
                                sq.col_match[list_idx[i]][ridx][1].append(
                                    full_match[0][1][f]
                                )
                                sq.col_match[list_idx[i]][ridx][2].append(
                                    full_match[0][2][f]
                                )
                                sq.col_match[list_idx[i]][ridx][3].append(
                                    full_match[0][3][f]
                                )
                break
        ...
        already_add = False
        patterns = [
            "COL of TABLE and TABLE ",
            "COL of TABLE and JJ TABLE ",
            "COL of TABLE and of TABLE ",
            "COL of TABLE and TABLE ? ",
            "COL of TABLE and JJ TABLE ? ",
            "COL of TABLE and of TABLE ? ",
            "COL of TABLE and TABLE . ",
        ...
        patterns_ = [
            "COL of TABLE and TABLE IN TABLE-COL ",
            "COL of TABLE and TABLE IN TABLE ",
        ]
        for p in patterns + patterns_:
            if tp_str2.endswith(p) and tp_str2.count(p) == 1:
                ...
                (ridx, lfidx) = (
                    (lfidx, ridx) if p.startswith("TABLE ") else (ridx, lfidx)
                )
                ...
                if not sq.col_match[list_idx[i]][idx_start]:
                    if p in patterns2:
                        ...
                        sq = add_name_col(
                            ...
                            list_idx[i],
                            idx_start,
                            ["date from", " date to", " date", "date", "year", " year"],
                        )
                    else:
        ...
        for p in patterns3:
            if (
                p in patterns1
                and not (
                    tp_str2.count("TABLE") == p.count("TABLE")
                    and tp_str2.count("COL") == p.count("COL")
                )
                or p in patterns22
                and (
                    len(list_idx) == 2
                    or tp_str2.count(" ") - p.count(" ") > 1
                    or tp_str2.startswith("how TABLE ")
                    or tp_str2.endswith("how TABLE-COL ")
                    or "AGG" in tp_str2
                    or "JJS" in tp_str2
                )
            ):
                ...
                if add_name:
                    ...
                    for tb in sq.table_match[list_idx[i]][lfidx]:
                        sq = add_name_col(
                            ...
                            list_idx[i],
                            lfidx,
                            [sq.question_tokens[list_idx[i]][lfidx].lemma_ + " name"],
                            table_prefix=schema.table_tokens_lemma_str[tb].split(" | ")[
                                0
                            ],
                        )
                    if old_col_match == sq.col_match[list_idx[i]][lfidx]:
                        for tb in sq.table_match[list_idx[i]][lfidx]:
                            sq = add_name_col(
                                ...
                                lfidx,
                                [
                                    sq.question_tokens[list_idx[i]][lfidx].lemma_
                                    + " detail",
                                    sq.question_tokens[list_idx[i]][lfidx].lemma_
                                    + " description",
                                ],
                    ...
                    if old_col_match == sq.col_match[list_idx[i]][lfidx]:
                        for tb in sq.table_match[list_idx[i]][lfidx]:
                            sq = add_name_col(
                                ...
                                list_idx[i],
                                lfidx,
                                [sq.question_tokens[list_idx[i]][lfidx].lemma_ + " id"],
                                table_prefix=schema.table_tokens_lemma_str[tb].split(
                                    " | "
                                )[0],
                            )
        ...
        for p in patterns5:
            if tp_str2.startswith(p) and tp_str2.count(p) == 1:
                ...
                if (p in patterns2 or p in patterns3) and (
                    "how many different " in question[i].lower()
                    or "how many distinct " in question[i].lower()
                ):
                    if p in patterns2:
                ...
                elif (p in patterns4) and (
                    "number of distinct " in question[i].lower()
                    or "number of different " in question[i].lower()
                ):
                    if "TABLE " in p:
                        tb_idx = tp_str2.split(" ").index("TABLE")
                    else:
                    ...
                    if col in schema.foreignKeyDict.keys():
                        for c2 in schema.foreignKeyDict[col]:
                            if (
                                ...
                                in sq.table_match[sq_idx1][sq_idx2]
                            ):
                                if sq.col_match[sq_idx1][sq_idx2] == []:
                                    sq.col_match[sq_idx1][sq_idx2] = [
                                        [col],
                                        [1],
                                        [1],
                                        [col],
...
def for_each_col(select, token_pattern, question, list_idx, sq, schema):
    ...
    for i, tps in enumerate(token_pattern):
        if (
            sq.sub_sequence_type[list_idx[i]] == 0
            and len(select) == 2
            and (
    ...
    if (len(select) == 2 and (select[0][0] or select[1][0])) or len(select) == 1:
        ...
        if correct_select:
            for cs in correct_select:
                if (
                    "name" in schema.column_tokens_lemma_str[cs]
                    or "title" in schema.column_tokens_lemma_str[cs]
                ):
                        ...
                        if sq.sub_sequence_type[i] <= 1:
                            for j, table_match in enumerate(table_matchs):
                                for t in table_match:
                                    if t == table_id:
                                        len_match = (
                                            schema.column_tokens_lemma_str[cs]
                                            .split(" | ")[0]
                                            .count(" ")
                                        )
                    ...
                    for d1, d2, d3, d4 in zip(col[0], col[1], col[2], col[3]):
                        ...
                        for ci in schema.same_col_idxs[d1]:
                            if (
                                ...
                                and ci not in col[0]
                                and schema.column_tokens_table_idx[ci] not in t_m[z]
                            ):
                                col[0].append(ci)
                                col[1].append(d2)
                                col[2].append(d3)
                        ...
                        if d1 in schema.foreignKeyDict.keys():
                            for ci in schema.foreignKeyDict[d1]:
                                if (
                                    ...
                                    and schema.column_tokens_table_idx[ci] not in t_m[z]
                                    and (
                                        schema.column_tokens_lemma_str[ci]
                                        == schema.column_tokens_lemma_str[d1]
                                        or schema.table_tokens_lemma_str[
                                            schema.column_tokens_table_idx[ci]
                                        ].split(" | ")[0]
                                        + " "
                                        + schema.column_tokens_lemma_str[ci]
                                        == schema.column_tokens_lemma_str[d1]
                                        or schema.column_tokens_lemma_str[ci]
                                        == schema.table_tokens_lemma_str[
                                            schema.column_tokens_table_idx[d1]
                                        ].split(" | ")[0]
                                        + " "
                                        + schema.column_tokens_lemma_str[d1]
                                    )
...
def for_each_analyse(token_pattern, question, list_idx, sq, schema):
    ...
    for i, tps in enumerate(token_pattern):
        ...
        if (
            ...
            or "COL each TABLE" in real_tp_str
        ):
...
def remove_other_cols(sq, schema, sq_i, sq_j):
    if sq.col_match[sq_i][sq_j]:
        ...
        for i in range(len(sq.col_match[sq_i][sq_j][0]) - 1, -1, -1):
            if (
                schema.column_names_original[sq.col_match[sq_i][sq_j][0][i]][0]
                not in tbs
            ):
...
if __name__ == "__main__":
    ...
    for table in table_json:
        ...
        if not ONE_ID:
            all_schema[table["db_id"]] = Schema_Token(
                _tokenizer, lstm, table, _concept_word
            )
        ...
        for loop in range(3):
            ...
            if sql["db_id"] not in all_schema:
                all_schema[sql["db_id"]] = Schema_Token(
                    _tokenizer, lstm, table_dict[sql["db_id"]], _concept_word
                )
            ...
            select_list, select_table, select_list_idx, select_db_idx = (
                sq.sentence_combine()
            )
            others_list, others_table, others_list_idx, others_db_idx = (
                sq.sentence_combine(combine_type=1)
            )
            qsql = QuestionSQL(sq, _tokenizer)
            sq.tokenize(qsql)

            try:
                select, where, group, order, distinct = select_analyze(
                    lstm,
                    select_list,
                    select_table,
                    select_list_idx,
                    all_schema[sql["db_id"]],
                    sq,
                    qsql,
                )
            except:
            ...
            others, _, others_token_pattern = others_analyze(
                others_list,
                others_table,
                others_list_idx,
                all_schema[sql["db_id"]],
            ...
            sq, others_token_pattern = sgrsm_add_col_match(
            ...
            sq, select_token_pattern = sgrsm_add_col_match(
                select_token_pattern,
                select_list,
                select_list_idx,
                sq,
                all_schema[sql["db_id"]],
            ...
            return_result = others_where_order_analyse(
                others_token_pattern, others_list, select_list, select
            )
            return_result += select_where_order_analyse(
                select_token_pattern, select_list, select
            )
            ...
            if oq == sql["question"]:
                ...
                sql["table_match"] = sq.gennerate_table_match(
                    all_schema[sql["db_id"]], sql["table_match"]
                )
            ...
            if "full_db_match" in sql and len(sql["full_db_match"]) != len(
                sql["db_match"]
            ):
