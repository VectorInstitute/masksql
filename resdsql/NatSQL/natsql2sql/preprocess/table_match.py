...
def exact_match_idx(question_tokens, str_, type_=1, table=True):
    ...
    while True:
        ...
        if (
            ...
            and table
            or idx == 0
            and question_tokens.tokens[0].text[0].isupper()
            or idx > 0
            and question_tokens.tokens[idx - 1].text in [".", "?"]
            or idx > 0
            and question_tokens.tokens[idx].tag_ in ["JJR", "JJS", "RBR", "RBS"]
            and question_tokens.tokens[idx].text != str_
        ):
            continue
...
def update_exact_match_tok(
    ...
    for tok_index in tok_indexes:
        ...
        for i in range(tok_len):
            if exact_match[
                i + tok_index
            ]:  # if there already contain data, reset data to delete it.
        ...
        if row and 0 in row[2]:
            ...
            if max_ > 1 and not for_table_match:
                for i in reversed(range(len(row[1]))):
                    if row[1][i] == max_:
                        col_end = schema.column_tokens_lemma_str[
                            schema.tbl_col_idx_back[table_idx][row[3][i]]
                        ].split(" ")[-1]
                        if " " + col_end in question_tokens.lemma_ or col_end in [
                            "name",
                            "id",
                        ]:
            ...
            for i in reversed(range(len(row[1]))):
                ...
                if row[1][i] != max_ or (
                    ...
                    and max_ == 1
                    and (
                        question_tokens.tokens[j].lemma_ in ["list", "name"]
                        or question_tokens.tokens[j].lemma_ in STOP_WORDS
                    )
                ):
                    if for_table_match:
                        ...
                        pass
                        if (
                            not remove_others_cols
                            and "name"
                            in schema.column_tokens_lemma_str[
                                schema.tbl_col_idx_back[table_idx][row[3][i]]
                            ]
                        ):
                            is_not_end_with = False

                if is_not_end_with and (
                    (row[1][i] != max_ and row[1][i] == 1)
                    or (
                        for_table_match
                        and max_ == 1
                        and (
        ...
        for i in range(tok_len):
            if exact_match[i + tok_index]:
                exact_match[i + tok_index][0].append(
                    round(
                        table_idx
                        + previous_match_idx[len(previous_match_idx) + i - tok_len]
                        / 1000,
                        3,
                    )
                )
                ...
                exact_match[i + tok_index].append(
                    [
                        round(
                            table_idx
                            + previous_match_idx[len(previous_match_idx) + i - tok_len]
...
def exact_match_table_name_plus_col(
    schema,
    question,
    question_tokens,
    table_name_list,
    ...
    for i, t_toks in enumerate(table_name_list):
        ...
        for t_tok in new_list:
            if " " not in t_tok and " " + t_tok + " " in question:
                tok_indexes = exact_match_idx(
                    question_tokens, t_tok, match_type, table=table
                )
                for tok_index in tok_indexes:
                    if i not in full_match[tok_index][1] and tok_index + 1 < len(
                        question_tokens.tokens
                    ):
                        extra_add = False
                        for col1, col2 in zip(
                            schema.table_col_lemma[i], schema.table_col_text[i]
                        ):
                            for c in col1.split(" | "):
    ...
    for i, t_toks in enumerate(table_name_list):
        ...
        for t_tok in new_list:
            if " " + t_tok + " " in question:
                ...
                full_match = update_exact_match_tok(
                    tok_indexes, i, t_tok.count(" ") + 1, full_match, table_idx
                )
    return full_match


def exact_stem_match_table_name(
    question, table_name_list, full_match, match_type=1, table=True, table_idx=-1
):
    for i, t_toks in enumerate(table_name_list):
        new_list = t_toks.split(" | ")
        for t_tok in new_list:
            ...
            for t_tok in new_list:
                if " " in t_tok:
                    ...
                    for j, t in enumerate(too):
                        ...
                        if (
                            (t not in table_name_for_col_match or j == len(too) - 1)
                            and t not in STOP_WORDS
                            and " " + t + " " in question
                            and not (
                                t == "name" and t_tok in ["first name", "last name"]
                            )
                        ):
                            tok_indexes = exact_match_idx(
                                question_tokens, t, match_type, table=table
                            )
                            previous_match_idx.append(j + 1)
                            full_match = update_partly_match_tok(
                                tok_indexes,
                                i,
                                j + 1,
                                1,
...
def combined_exact_match_table_name(
    question,
    question_tokens,
    table_name_list,
    full_match,
    ...
    for i, t_toks in enumerate(table_name_list):
        ...
        for t_tok in new_list:
            ...
            if " " + t + " " in question:
                ...
                full_match = update_exact_match_tok(
                    tok_indexes, i, 1, full_match, table_idx
                )
            elif t_tok.count(" ") == 0 and (
                (len(t_tok) == 5 and t_tok.endswith("name"))
                or len(t_tok) == 3
                and t_tok.endswith("id")
            ):
                startword = t_tok[0]
                endword = t_tok[1:]
                if " " + endword + " " in question:
                    ...
                    if idx_end > 2 and q_tokens[idx_end - 1][0] == startword:
                        tok_indexes = exact_match_idx(
                            question_tokens,
                            q_tokens[idx_end - 1] + " " + endword,
                            match_type,
                            table=table,
                        )
                        full_match = update_exact_match_tok(
                            tok_indexes, i, 2, full_match, table_idx
                        )
                    elif (
                        idx_end + 3 < len(q_tokens)
                        and q_tokens[idx_end + 2][0] == startword
                        and q_tokens[idx_end + 1] == "of"
                    ):
                        tok_indexes = exact_match_idx(
                            question_tokens,
                            endword + " of " + q_tokens[idx_end + 2],
                            match_type,
                            table=table,
                        )
                        full_match = update_exact_match_tok(
...
def modified_exact_match_table_name_one_word(
    question_tokens, table_name_list, full_match, table=True
):
                ...
                for z, qtok in enumerate(question_tokens.tokens):
                    if t == lstem.stem(qtok.text):
                        ...
                        if (
                            ...
                            and question_tokens.tokens[0].text[0].isupper()
                            or z > 0
                            and question_tokens.tokens[z - 1].text in [".", "?"]
                        ):
                            continue
                ...
                if " " in t_tok:
                    ...
                    for j, t in enumerate(too):
                        if t not in STOP_WORDS and j < len(too) - 1:
                            ...
                            tok_indexes = []
                            for z, qtok in enumerate(question_tokens.tokens):
                                if t == lstem.stem(qtok.text) and qtok.tag_ not in [
                                    "JJR",
                                    "JJS",
                                    "RBR",
                                    "RBS",
                                    ...
                                    while True:
                                        ...
                                        b_match, q_idx_match = next_modified_word_match(
                                            tw, question_tokens.tokens, tok_indexes[-1]
                                        )
                                        if b_match:
                                    ...
                                    if len(tok_indexes) > 1:
                                        ...
                                        for qtok_index in tok_indexes:
                                            if full_match[qtok_index]:
                                                if full_match[qtok_index][1][0] >= len(
                                                    tok_indexes
                                                ):
                                                    continue
                                            ...
                                            if full_match[
                                                ...
                                                full_match[qtok_index][1].append(
                                                    len(tok_indexes)
                                                )
...
def return_table_name(question_tokens, table_name_tokens):
    ...
    # Exact Match:
    full_match = exact_match_table_name(
        question_lemma,
        question_tokens,
        table_name_tokens.table_tokens_lemma_str,
        full_match,
        match_type=2,
    )
    ...
    full_match = exact_match_table_name(
        question_text,
        question_tokens,
        table_name_tokens.table_tokens_text_str,
        full_match,
    )

    full_match = exact_match_table_name_plus_col(
        table_name_tokens,
        question_lemma,
        question_tokens,
        table_name_tokens.table_tokens_lemma_str,
    ...
    # Partly Match:
    full_match = partly_match_table_name(
        question_lemma,
        question_tokens,
        table_name_tokens.table_tokens_lemma_str,
        full_match,
    ...
    full_match = modified_exact_match_table_name_one_word(
        question_tokens, table_name_tokens.table_tokens_lemma_str, full_match
    )
    full_match = modified_exact_match_table_name(
        question_tokens, table_name_tokens.table_tokens_lemma_str, full_match
    )
        ...
        if row:
            for i in reversed(range(len(row[1]))):
                if (
                    ...
                    if question_tokens.tokens[j].lemma_ in MUST_MORE_THAN_HALF or (
                        " id" in schema.column_tokens_lemma_str[row[0][i]]
                        and question_tokens.tokens[j].lemma_ != "id"
                    ):
                        if (
                            (" each " in question_tokens.lemma_ and len(row[0]) == 1)
                            or (
                                question_tokens.tokens[j].lemma_ == "first"
                                and "last name" in question_tokens.lemma_
                            )
                            or (
                                question_tokens.tokens[j].lemma_ == "last"
                                and "first name" in question_tokens.lemma_
                            )
                        ):
                            continue
                        del row[0][i]
                        ...
                        j + 1 < len(full_match)
                        and question_tokens.tokens[j].lemma_ == "number"
                        and question_tokens.tokens[j + 1].lemma_ == "of"
                        or row[1][i] == 1
                        and question_tokens.tokens[j].text.isdigit()
                    ):
                        del row[0][i]
                        del row[1][i]
                        del row[2][i]
...
def remove_ment_ship(t, question):
    ...
    if "ment " in question:
        return t + "ment"
    if "ments " in question:
        return t + "ments"
    return t


def return_column_match(
    question_tokens,
    table_name_tokens,
    table_idx,
    full_match=None,
    ...
    question_text = " " + question_tokens.text + " "
    question_stem = (
        " " + " ".join([lstem.stem(tok.text) for tok in question_tokens.tokens]) + " "
    )
    # Exact Match:
    full_match = exact_match_table_name(
        question_lemma,
        question_tokens,
        table_name_tokens.tbl_col_tokens_lemma_str[table_idx],
        full_match,
        match_type=2,
        table=False,
        table_idx=table_idx,
    ...
    full_match = exact_match_table_name(
        question_text,
        question_tokens,
        table_name_tokens.tbl_col_tokens_text_str[table_idx],
        full_match,
        table=False,
        table_idx=table_idx,
    )

    full_match = exact_stem_match_table_name(
        question_stem,
        table_name_tokens.tbl_col_tokens_stem_str[table_idx],
        full_match,
        table=True,
        table_idx=table_idx,
    )

    full_match = combined_exact_match_table_name(
        question_lemma,
        question_tokens,
        table_name_tokens.tbl_col_tokens_lemma_str[table_idx],
        full_match,
        ...
        table=False,
        table_idx=table_idx,
        tname=table_name_tokens.table_tokens_lemma_str[table_idx],
    )

    if not only_exact_match:
        # Partly Match:
        table_name_for_col_match = [
            table_name_tokens.table_tokens_lemma_str[table_idx]
            .split(" | ")[0]
            .split(" ")[-1],
            table_name_tokens.table_tokens_text_str[table_idx]
            .split(" | ")[0]
            .split(" ")[-1],
        ]
        full_match = partly_match_table_name(
            ...
            match_type=2,
            table=False,
            table_name_for_col_match=table_name_for_col_match,
        )
    ...
    full_match = clean_table(
        full_match, question_tokens, table_name_tokens, False, table_idx
    )
