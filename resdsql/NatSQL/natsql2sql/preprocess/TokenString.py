...
class TokenString:
    ...
    def match_all(self, off, in_toks, total_tokens, type_=1):
        ...
        if type_ == 1:
            ...
            for i in range(len(in_toks)):
                lemma = (
                    total_tokens[i + off].lemma_
                    if total_tokens[i + off].lemma_ != "-PRON-"
                    else total_tokens[i + off].text
                )
                if in_toks[i] != lemma:
                    return False
        else:
    ...
    def update(self, idx, tok):
        ...
        self.text = " ".join([tok.text for tok in self.tokens]).lower()
        self.lemma_ = " ".join(
            [tok.lemma_ if tok.lemma_ != "-PRON-" else tok.text for tok in self.tokens]
        ).lower()
        self.tag_ = " ".join([tok.tag_ for tok in self.tokens])

    def split(self, split_word, allowed_split_to_many=False):
        if split_word in self.text and (
            allowed_split_to_many or self.text.count(split_word) == 1
        ):
    ...
    def replace(self, oldword, new_word):
        ...
        for i, tok in enumerate(newself.tokens):
            if tok.lemma_ == oldword or tok.text == oldword:
                newself.tokens[i] = SToken(
                    text=new_word, lemma=new_word, tag=tok.tag_, lower=new_word.lower()
                )
        if new_word == "":
    ...
    def lemma_without_jjs_jjr(self):
        return " ".join(
            [
                tok.lemma_
                if tok.lemma_ != "-PRON-"
                and tok.tag_ not in ["JJR", "JJS", "RBR", "RBS"]
                else tok.text
                for tok in self.tokens
            ]
        ).lower()
    ...
    def construct_from_token(self, tokens):
        ...
        ts.lemma_ = " ".join(
            [tok.lemma_ if tok.lemma_ != "-PRON-" else tok.text for tok in ts.tokens]
        ).lower()
