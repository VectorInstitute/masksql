        ...
        def special_case_for_pass(col_id, schema, ts, ts_id, select_type=False):
            for col in schema.column_tokens_lemma_str[col_id].split(" | "):
                if (
                    col.endswith(" name")
                    or col.endswith(" id")
                    or col.endswith(" type")
                    or col.endswith(" code")
                    or col.endswith(" value")
                    or col.endswith(" count")
                    or col.startswith("count ")
                    or col.startswith("number ")
                    or col.endswith(" number")
                    or col.startswith("amount ")
                    or col.endswith(" amount")
                    or col.endswith(ts.tokens[ts_id].lemma_)
                ):
                    if select_type:
                        return True
                elif (
                    col.endswith(" rate")
                    and (" 0." in ts.text or "%" in ts.text)
                    or schema.column_types[col_id] == "time"
                    and (
                        " oldest " in ts.text
                        or " latest " in ts.text
                        or " earliest " in ts.text
                        or " longest " in ts.text
                        and " shortest " in ts.text
                        or " after 1" in ts.text
                        or " after 2" in ts.text
                        or " before 1" in ts.text
                        or " before 2" in ts.text
                        or "when " in ts.text
                    )
                ):
                    return True
                elif (
                    schema.column_types[col_id] == "time"
                    and ts_id + 5 < len(ts.tokens)
                    and (
                        ts.tokens[ts_id + 1].text == "between"
                        or ts.tokens[ts_id + 2].text == "between"
                    )
                ):
                    date_count = 0
                    for i in range(2, 6):
            ...
            for i, c in enumerate(cm):
                if c:
                    if input_match and k != 0 and input_match[i] and is_not_all_tables:
                        if (
                            i + 2 < len(cm)
                            and ts.tokens[i + 1].text == "of"
                            and table_match[i + 2]
                            and schema.column_names_original[c[0][0]][0]
                            in table_match[i + 2]
                            and col_match_not_in_table(
                                input_match[i], table_match[i + 2], schema
                            )
                            or i + 3 < len(cm)
                            and ts.tokens[i + 1].text == "all"
                            and ts.tokens[i + 2].text == "of"
                            and table_match[i + 3]
                            and schema.column_names_original[c[0][0]][0]
                            in table_match[i + 3]
                            and col_match_not_in_table(
                                input_match[i], table_match[i + 3], schema
                            )
                            or i - 1 >= 0
                            and table_match[i - 1]
                            and schema.column_names_original[c[0][0]][0]
                            ...
                            and 1 in c[2]
                            and col_match_not_in_table(
                                input_match[i], table_match[i - 1], schema
                            )
                        ):
                            input_match[i] = []
                            match[i] = []
                    ...
                    if (
                        ...
                        and table_match[i]
                        and input_match[i + 1]
                        and schema.column_tokens_table_idx[input_match[i + 1][0][0]]
                        in table_match[i]
                        and is_not_all_tables
                    ):
                        continue
                    if (
                        input_match
                        ...
                        and ts.tokens[i - 1].text == "of"
                        and input_match[i - 2]
                        and schema.column_tokens_table_idx[input_match[i - 2][0][0]]
                        in table_match[i]
                        and is_not_all_tables
                    ):
                        continue
                    if (
                        ...
                        and ts.tokens[i - 2].text == "of"
                        and input_match[i - 3]
                        and schema.column_tokens_table_idx[input_match[i - 3][0][0]]
                        in table_match[i]
                        and is_not_all_tables
                    ):
                        continue
                    if (
                        ...
                        if (
                            not hudun[i]
                            and col_match_not_in_table(
                                match[i], table_match[i + 2], schema
                            )
                            and (
                                i == 0
                                or col_match_not_in_table(
                                    match[i], table_match[i - 1], schema
                                )
                            )
                        ):
                            match[i] = []
                        ...
                        if (
                            not hudun[i]
                            and col_match_not_in_table(
                                match[i], table_match[i + 3], schema
                            )
                            and (
                                i == 0
                                or col_match_not_in_table(
                        ...
                        if (
                            ...
                            and max(c[1]) >= max(match[i][1])
                            and (i + 2 >= len(cm) or ts.tokens[i + 1].text != "of")
                        ):
                            match[i] = []
                        hudun[i] = True
                    elif (
                        i - 2 >= 0
                        and ts.tokens[i - 1].text in ["with", "'s"]
                        and table_match[i - 2]
                        and schema.column_names_original[c[0][0]][0]
                        in table_match[i - 2]
                        and 1 in c[2]
                        ...
                        ts.tokens[i].lemma_ == "name"
                        and match[i]
                        and max(c[1]) < max(match[i][1])
                        and max(match[i][2]) < 1
                        or ts.tokens[i].lemma_ == "id"
                        and match[i]
                        and max(c[1]) < max(match[i][1])
                        and max(match[i][2]) < 1
                    ):
                ...
                if (
                    ...
                    and (
                        ...
                        or (
                            ...
                            and ts.tokens[i - 2].text == "of"
                            and match[i - 3]
                            and schema.column_tokens_table_idx[match[i - 3][0][0]]
                            in table_match[i]
                        )
                    )
                ):
    ...
    if all_tables:
        ...
        if new_tables:
            match = col_match(
                ...
                table_match,
                match,
                only_exact_match=not select_type,
                is_not_all_tables=False,
                select_type=select_type,
            )
        new_tables = [
            i
            for i in range(len(schema.table_names_original))
            if i not in tables and i not in all_tables
        ]
    else:
        new_tables = [
            i for i in range(len(schema.table_names_original)) if i not in tables
        ]
