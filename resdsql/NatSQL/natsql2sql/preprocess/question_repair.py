...
from .match import ABSOLUTELY_GREATER, ABSOLUTELY_GRSM_DICT, NOT_STAR_WORD, word_is_grsm
from .TokenString import SToken as Token
from .utils import is_there_sgrsm_and_gr_or_sm, str_is_num
...
def question_repair(nlp, q, all_word, full_target, top_target, end_target):
    ...
    if (
        sentences_num == 2
        and tokens[0].lemma_ == "which"
        and tokens[2].lemma_ == "have"
    ):
        tokens[2] = Token(text="has", lemma="have")
    for i, tok in enumerate(tokens):
        if (
            tok.lemma_ not in all_word
        ...
        if (
            ...
            if (
                tokens[i - 1].lemma_ == "than"
                and word_is_grsm(tokens[i - 2])
                and tokens[i + 1].lemma_ == "equal"
            ):
                if tokens[i + 2].lemma_ == "to":
                    del tokens[i + 2]
                find_sgrsm = is_there_sgrsm_and_gr_or_sm(tokens, tokens[i - 2], i - 2)
                if find_sgrsm == "GR_" or (
                    find_sgrsm is None and tokens[i - 2].lemma_ in ABSOLUTELY_GREATER
                ):
                ...
                str_is_num(tokens[i - 1].text)
                and (
                    word_is_grsm(tokens[i + 1])
                    or tokens[i + 1].text
                    in [
                        "young",
                        "younger",
                ...
                and i >= 4
                and not word_is_grsm(tokens[i - 2])
                and not word_is_grsm(tokens[i - 3])
                and not word_is_grsm(tokens[i - 4])
            ):
                tokens[i] = tokens[i - 1]
                find_sgrsm = is_there_sgrsm_and_gr_or_sm(tokens, tokens[i + 1], i + 1)
                if find_sgrsm == "GR_" or (
                    find_sgrsm is None and tokens[i + 1].lemma_ in ABSOLUTELY_GREATER
                ):
