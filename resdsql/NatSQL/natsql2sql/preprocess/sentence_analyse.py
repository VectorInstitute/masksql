...
from .db_match import DBEngine, datebase_match_tables, get_database_string
from .match import (
    A_ADJ_WORD_DIRECTION,
    ABSOLUTELY_GRSM_DICT,
    COUNTRYS_DICT,
    NEGATIVE_WORDS,
...
from .others_pattern import (
    get_AWD_column,
    get_col_from_related_word,
    pattern_recomand,
    pattern_reconize,
)
from .pattern_question_type import *
from .pattern_question_type import PATTERN_FUN, PATTERNS_TOKS
from .sq import QuestionSQL, SubQuestion
from .stemmer import MyStemmer
from .table_match import return_column_match
from .TokenString import SToken, TokenString
from .utils import (
    get_all_table,
    get_punctuation_word,
    look_for_table_idx,
    str_is_date,
    str_is_num,
)
...
def one_group_in_ahead(all_tokens, token, father_idx, offset):
    ...
    while True:
        ...
        if tok_now.i == 0 or (
            tok_now.i > 2 and all_tokens[tok_now.i - 1 - offset].text == ","
        ):
            start = True
...
def check_behind_children_len(token):
    ...
    if (
        count == 1
        and final_child
        and final_child.text in STOP_WORDS
        and len(list(final_child.children)) == 0
    ):
        return 0
    return count
...
def prep_table_break(token, table_match):
    ...
    while True:
        ...
        for i, child in enumerate(children):
            ...
            if (
                child.dep_ == "prep"
                and child.text not in ["of", "for", "by", "between", "than", "per"]
                and check_behind_children_len(child)
                or child.dep_ == "relcl"
            ):
                there_is_break = True
                continue
            if child.text in STOP_WORDS or table_match[child.i]:
                pass
            else:
                return False
            next_token.append(child)
...
def check_break(
    ...
    if (
        token.dep_ == "prep"
        and token.text not in ["of", "for", "by", "between", "than", "per"]
        and check_behind_children_len(token)
        and not special_case_for_prep(all_tokens[father_idx - offset])
    ):
        if father_type >= 1:
            if check_break_before(
                token_list, father_type, table_match
            ) and check_valid_words(token, table_match, 1):
                return BREAK_PREP
            return NO_BREAK
        if prep_table_break(token, table_match) and token.i > father_idx:
            return NO_BREAK
        return BREAK_PREP
    ...
    if token.dep_ == "acl":
        ...
        return BREAK_RELCL
    if (
        token.dep_ in ["nsubj", "npadvmod", "csubj", "nsubjpass"]
        and token.text != "List"
        and one_group_in_ahead(all_tokens, token, father_idx, offset)
    ):
        if len(list(token.children)) == 0 and sent_idx == 0 and sent_num == 2:
            return BREAK_SELECT
        if len(list(token.children)) == 0 or (
            token.dep_ == "csubj" and not check_valid_words(token, table_match, 1)
        ):
            return NO_BREAK
        return BREAK_SELECT
    if (
        father_type >= 1
        and token.dep_ == "conj"
        and (check_valid_words(token, table_match, 2) or check_continue_sentence(token))
    ):
        return BREAK_RELCL
    if (
        token.text == "ordered"
        and token.i + 2 - offset < len(all_tokens)
        and all_tokens[token.i + 1 - offset].text == "by"
    ):
        ...
        if tok[1] == val:
            for t in tok[0].text.split(" "):
                ...
                if table_match[tok[0].i]:
                    count += 0.5
                num_tok += 1
    return bool(count >= 1 and num_tok > 1)
...
def sentence_cut(sentence, table_match, offset, sent_idx, sent_num):
    ...
    while True:
        ...
        for child in next_.children:
            ...
            break_type = check_break(
                sentence,
                child,
                token_list[father_idx][1],
                token_list[father_idx][0].i,
                token_list,
                table_match,
                offset,
                sent_idx,
            ...
            if break_type in [BREAK_RELCL, BREAK_PREP, BREAK_ORDERBY]:
                ...
                type_, token_list = token_list_to_one(
                    token_list, token_list[father_idx][1]
                )
...
def re_set_select_value(token_list, start):
    ...
    if (
        start > 5
        and start + 2 < len(token_list)
        and token_list[start - 2][0].text == "in"
        and token_list[start][0].text == "for"
        and token_list[start - 3][1] == 0
        and token_list[start - 2][1] != token_list[start + 2][1]
        and token_list[start + 2][1] != token_list[start][1]
        and token_list[start - 2][1] != 0
    ):
        return reset(
            token_list, start - 2, offset=0, reset_original_v=token_list[start - 2][1]
        )

    if (
        token_list[start][0].tag_ == "IN" or token_list[start][0].dep_ == "preconj"
    ) and token_list[start][0].text not in ["and", "or"]:
    ...
    for i in range(start, len(token_list), 1):
        if token_list[i][1] != 0:
            break
        if token_list[i][0].tag_ == "IN" and token_list[i][0].text not in [
            "of",
            "for",
            "by",
            "in",
    ...
    for i in range(start, len(token_list), 1):
        if token_list[i][1] != 0:
            break
        if token_list[i][0].tag_.startswith("VB") or token_list[i][0].text in [
            "sort",
            "order",
        ]:
            return reset(token_list, start)
    ...
    for new_v in [2, 1]:
        for i, l in enumerate(len_count):
            ...
            if (
                ...
                and l[0] == 0
                and i >= 2
                and token_list[l[2] + 1][0].text in [".", "?"]
            ):
...
def correct_special_pattern(token_list, question, offset_list):
    def only_how_many(token_list):
        if (
            token_list[0][0].text == "How"
            and token_list[1][0].text == "many"
            and token_list[0][1] == 0
            and token_list[1][1] == 0
            and token_list[2][1] == 1
        ):
            for i, tok in enumerate(token_list):
                if tok[1] > 1:
                    break
    ...
    def punctuation_for(token_list, question):
        if " , for " in question:
            for i in range(1, len(token_list) - 1, 1):
                if (
                    token_list[i][0].text == ","
                    and token_list[i + 1][0].text == "for"
                    and token_list[i][1] == 0
                    and token_list[i + 1][1] == 0
                ):
                    next_ = 0
    ...
    def reset_sort_type(token_list, offset_list):
        if len(offset_list) == 2 and token_list[0][0].lower_ == "sort":
            ...
            len(offset_list) == 1
            and token_list[0][0].lower_ == "sort"
            and token_list[0][1] == 0
            and token_list[0][-1] == 0
        ):
            for i, tok in enumerate(token_list):
...
def final_correct_special_pattern(token_list):
        ...
        for i, tok in enumerate(token_list):
            if (
                (
                    ...
                    and not (
                        token_list[i - 2][0].text == "between"
                        or token_list[i - 3][0].text == "between"
                    )
                )
                and tok[1] > 0
                and token_list[i - 1][1] == tok[1]
            ):
                count = 0
                for j in range(i + 1, len(token_list) - 1, 1):
                    ...
                    if count >= 2:
                        break
                if (
                    token_list[i - 2][1] != token_list[i - 1][1]
                    and token_list[i - 1][1] == tok[1]
                    and tok[1] != token_list[i + 1][1]
                    or token_list[i - 2][1] == token_list[i - 1][1]
                    and token_list[i - 1][1] != tok[1]
                    and tok[1] != token_list[i + 1][1]
                ):
    ...
    def repair_disjoin_phase(token_list):
        for i, tok in enumerate(token_list):
            if (
                (
                    tok[0].lemma_ in ["start", "end", "participate", "serve", "same"]
                    and i + 2 < len(token_list)
                    and token_list[i + 1][0].text in ["from", "on", "in", "as"]
                    and tok[1] != token_list[i + 1][1]
                )
                or (
                    tok[0].lemma_ in ["prior", "equal", "assign"]
                    and i + 2 < len(token_list)
                    and token_list[i + 1][0].text == "to"
                    and tok[1] != token_list[i + 1][1]
                )
                or (
                    tok[0].lemma_ == "ascend"
                    and i + 2 < len(token_list)
                    and token_list[i + 1][0].text == "order"
                    and tok[1] != token_list[i + 1][1]
                )
                or (
                    tok[0].lemma_ == "descend"
                    and i + 2 < len(token_list)
                    and token_list[i + 1][0].text == "order"
                    and tok[1] != token_list[i + 1][1]
                )
                or (
                    tok[0].lemma_ == "be"
                    and i + 2 < len(token_list)
                    and token_list[i + 1][0].lemma_ in ABSOLUTELY_GRSM_DICT
                    and tok[1] != token_list[i + 1][1]
                )
                or (
                    tok[0].lemma_ == "belong"
                ...
                or (
                    tok[0].lemma_ == "locate"
                    and i + 2 < len(token_list)
                    and token_list[i + 1][0].text in ["in", "at"]
                    and tok[1] != token_list[i + 1][1]
                )
                or (
                    tok[0].lemma_ == "long"
                ...
                or (
                    (
                        tok[0].lemma_ == "than"
                        or str_is_num(tok[0].text)
                        or tok[0].lemma_ in ["equal", "in"]
                    )
                    and i >= 2
                    and i + 2 < len(token_list)
                    and token_list[i + 1][0].text == "or"
                    and tok[1] != token_list[i + 1][1]
                    and (
                        tok[0].lemma_ == "than"
                        or token_list[i + 2][0].lemma_ in ABSOLUTELY_GRSM_DICT
                        or token_list[i + 2][0].lemma_ in A_ADJ_WORD_DIRECTION
                    )
                )
                or (
                    tok[0].lemma_ == "or"
                    and i > 2
                    and i + 2 < len(token_list)
                    and token_list[i - 1][0].text == "than"
                    and token_list[i + 1][0].text in ["equal", "in"]
                    and tok[1] != token_list[i + 1][1]
                )
                or (
                    tok[0].lemma_ in ["and", "to"]
                    and i > 3
                    and i + 2 <= len(token_list)
                    and token_list[i - 1][0].text not in ["and", "or"]
                    and token_list[i - 2][0].text not in ["and", "or"]
                    and token_list[i - 3][0].text not in ["and", "or"]
                    and tok[1] != token_list[i + 1][1]
                    and (
                        token_list[i - 1][0].text == "between"
                        or token_list[i - 2][0].text == "between"
                        or token_list[i - 3][0].text == "between"
                    )
                )
                or (
                    i + 2 <= len(token_list)
                    and token_list[i + 1][0].text in ["and", "to"]
                    and i > 3
                    and token_list[i - 1][0].text not in ["and", "or"]
                    and token_list[i - 2][0].text not in ["and", "or"]
                    and token_list[i - 3][0].text not in ["and", "or"]
                ...
                or (
                    ...
                    and tok[1] != token_list[i + 1][1]
                    and (
                        token_list[i - 1][0].text in ["range", "from"]
                        or token_list[i - 2][0].text in ["range", "from"]
                        or token_list[i - 3][0].text in ["range", "from"]
                    )
                )
                or (
                    i + 2 <= len(token_list)
                ...
                for j in range(i + 2, len(token_list)):
                    if token_list[j][1] == token_list[i + 1][1]:
                        token_list[j][1] = tok[1]
                    else:
                        break
                token_list[i + 1][1] = tok[1]
            if (
                str_is_num(tok[0].text)
                and i > 0
                and i + 2 < len(token_list)
                and token_list[i + 1][0].text in ["and", "or", "but"]
                and tok[1] != token_list[i - 1][1]
            ):
                token_list[i][1] = token_list[i - 1][1]

            if tok[0].lemma_ in NEGATIVE_WORDS and tok[1] == 0:
                if tok[1] != token_list[i + 1][1]:
                    token_list[i][1] = token_list[i + 1][1]
                elif i + 4 < len(token_list) and tok[1] != token_list[i + 2][1]:
                    for iii in range(2):
                        token_list[i + iii][1] = token_list[i + 2][1]
                elif i + 5 < len(token_list) and tok[1] != token_list[i + 3][1]:
                    for iii in range(3):
                        token_list[i + iii][1] = token_list[i + 3][1]
                elif i + 6 < len(token_list) and tok[1] != token_list[i + 4][1]:
                    for iii in range(4):
...
def sentence_cut_analyze(
    ...
    for sentence, table_match, list_idx, type_ in zip(
        ...
        if skip_once:
            skip_once = False
            pr, db_match = pattern_reconize(
                ts,
                table_match,
                full_match,
                sub_q.sequence_entt[list_idx],
                schema,
                tables,
                [[["START", "SEARCH", "DATABASE"]]],
                [None],
                in_db_match=sub_q.db_match[list_idx],
                db_table_idxs=db_tables,
            )
            previous_db_match[list_idx - 1].extend(db_match)
            pattern_for_return.append(
                [
                    pattern_recomand(
                        ts,
                        table_match,
        ...
        tks = (
            PATTERNS_TOKS
            if sub_q.sentence_num == 2
            and (
                sub_q.sub_sequence_type.count(0) >= 2
                or (
                    list_idx == 0
                    and sub_q.question_tokens[list_idx][0].lemma_ == "which"
                )
            )
            else [[["SKIP"]] if ip == 1 else pt for ip, pt in enumerate(PATTERNS_TOKS)]
        )
        pr, db_match = pattern_reconize(
            ts,
            table_match,
        ...
        if pr[0] is not None:
            ...
            if pr[0] == 0 and type_ != 0 and run_time != 2:
                ...
                if (
                    list_idx < len(sub_q.original_idx) - 1
                    and sub_q.sub_sequence_type[list_idx] > 0
                    and list_idx > 0
                    and (sub_q.sub_sequence_type[list_idx + 1] > 0)
                    and sub_q.question_tokens[list_idx][-1].text
                    not in ["or", "and", ",", "but"]
                    and sub_q.question_tokens[list_idx + 1][0].lemma_
                    not in ["or", "and", ",", "order", "sort", "but"]
                ):
                    tables = look_for_table_idx(sub_q, list_idx + 1, 0, schema)
                    next_col_match = col_match_main(
                        tables,
                        TokenString(None, sub_q.question_tokens[list_idx + 1]),
                        schema,
                    )
                    if not table_matchs[list_idx + 1][0] and (
                        not next_col_match[0]
                        or pr[0] == 9
                        or (
                            full_match[-1]
                            and (
                                full_match[-1][0][0] in next_col_match[0][0]
                                or next_col_match[0][0][0] in full_match[-1][0]
                            )
                        )
                    ):  # change next
                        if (
                            list_idx + 2 >= len(table_matchs)
                            or sub_q.sub_sequence_type[list_idx + 2] != type_
                        ):
                    ...
                    (
                        list_idx == len(sub_q.original_idx) - 1
                        or sub_q.question_tokens[list_idx][-1].text
                        in [".", "?", ",", "and", "or"]
                        or (
                            (
                                sub_q.question_tokens[list_idx + 1][0].lemma_
                                in ["or", "and", ",", "order", "sort"]
                            )
                            and sub_q.question_tokens[list_idx][0].tag_ == "IN"
                        )
                    )
                    and sub_q.sub_sequence_type[list_idx] > 0
                    and list_idx > 0
                    and (
                        (
                            list_idx == len(sub_q.original_idx) - 1
                            and sub_q.question_tokens[list_idx][0].lemma_
                            not in ["or", "and", ","]
                            and sub_q.question_tokens[list_idx - 1][-1].lemma_
                            not in ["or", "and", ","]
                        )
                        or (
                            sub_q.question_tokens[list_idx][0].lemma_
                            not in ["or", "and", ",", "order", "sort"]
                            and sub_q.question_tokens[list_idx - 1][-1].lemma_
                            not in ["or", "and", ",", "order", "sort"]
                        )
                    )
                        ...
                        for i in range(0, list_idx):
                            ...
                            for cm in previous_col_match[i]:
                                if cm:
                                    for c in cm[0]:
                                        all_tables.append(
                                            schema.column_names_original[c][0]
                                        )
                        for tbm in table_match:
                            if tbm:
                                check_follow_last = [t not in all_tables for t in tbm]
                                if len(check_follow_last) == check_follow_last.count(
                                    True
                                ):
                                    follow_last = False
                                    break
                    ...
                    list_idx == len(sub_q.original_idx) - 1
                    or (list_idx > 0 and sub_q.sub_sequence_type[list_idx - 1] == 0)
                ) and pr[3] in [
                    ["+", "TABLE", "PCOL"],
                    ["+", "PCOL"],
                    ["+", "PCOL", "*"],
                ...
                if (
                    pr[0] == 2
                    and pr[3][-1] in ["COL"]
                    and list_idx + 1 < len(sub_q.original_idx)
                    and sub_q.question_tokens[list_idx + 1][0].text
                    not in ["and", "or", ",", ".", "?"]
                    and sub_q.question_tokens[list_idx][-1].text
                    not in ["and", "or", ",", ".", "?"]
                    and (not patterns or patterns[list_idx + 1][1] != 5)
                    and (
                        not previous_col_match[list_idx - 1][-1]
                        or not (
                            previous_col_match[list_idx - 1][-1]
                            == previous_col_match[list_idx][0]
                            or (
                                len(previous_col_match[list_idx]) > 1
                                and previous_col_match[list_idx - 1][-1]
                                == previous_col_match[list_idx][1]
                            )
                        )
                    )
                ):
                ...
                if sub_q.question_tokens[list_idx - 1][-1].text not in [".", "?"] and (
                    (
                        ...
                        not in ["or", "and", ",", "but"]
                        and sub_q.question_tokens[list_idx][0].text
                        not in ["or", "and", ",", "but"]
                    )
                    or (
                        (
                            sub_q.question_tokens[list_idx - 1][-1].text in ["and", ","]
                            or sub_q.question_tokens[list_idx][0].text in ["and", ","]
                        )
                        and sub_q.sub_sequence_type[list_idx - 1] == 0
                    )
                ):
                ...
                sub_q.sub_sequence_type[list_idx] = (
                    sub_q.sub_sequence_type[list_idx - 1] + 1
                )
            elif pr[0] == 4 and type_ > 0:
                there_is_table = True
                if (
                    list_idx > 0
                    and list_idx + 1 < len(sub_q.sequence_entt)
                    and sub_q.sub_sequence_type[list_idx - 1] == 0
                ):
                    there_is_table = False
                    for t in table_matchs[list_idx - 1]:
                        if t:
                            ...
                            break
                if list_idx > 0 and (
                    list_idx + 1 == len(sub_q.sequence_entt)
                    or not there_is_table
                    or sub_q.question_tokens[list_idx][-1].text
                    in ["?", ".", ",", "and", "or", "but"]
                    or sub_q.question_tokens[list_idx + 1][0].text
                    in [
                        "?",
                        ".",
                        ",",
                    ...
                    sub_q.sub_sequence_type[list_idx + 1] > 0
                ):  # set this sub string to following next type
                    for i in sub_q.original_idx[list_idx]:
                        token_list[i][1] = sub_q.sub_sequence_type[list_idx + 1]
                        sub_q.sub_sequence_type[list_idx] = sub_q.sub_sequence_type[
                            list_idx + 1
                ...
                if (
                    "DATE" not in pr[3]
                    and list_idx > 1
                    and sub_q.sub_sequence_type[list_idx]
                ...
                if list_idx + 1 < len(sub_q.sub_sequence_type) and (
                    (
                        sub_q.sub_sequence_type[list_idx + 1] > 0
                        and " ' " in sub_q.sub_sequence_list[list_idx + 1]
                    )
                    or (
                        list_idx == 0
                        and len(sub_q.sub_sequence_type) > 1
                        and sub_q.sub_sequence_type[list_idx + 1] == 0
                    )
                ):
                    for i in sub_q.original_idx[list_idx]:
                ...
                pr[0] == 8
                and list_idx + 1 < len(sub_q.sub_sequence_list)
                and sub_q.sentence_num == 2
                and " also " not in " ".join(sub_q.sub_sequence_list).lower()
                and " And " not in " ".join(sub_q.sub_sequence_list)
            ):
                tables = look_for_table_idx(sub_q, list_idx + 1, 0, schema)
                next_col_match = col_match_main(
                    tables,
                ...
                if len(sub_q.original_idx[list_idx]) <= 3 or (
                    not table_matchs[list_idx + 1][0] and not next_col_match[0]
                ):  # change next
                    if type_ != 0:
                        for i in sub_q.original_idx[list_idx + 1]:
                            token_list[i][1] = type_
...
def pattern_sentence_analyse(
    token_list, sql, schema, sentence_num, run_time, patterns=None
):
    sq, qsql = generate_sq_qsql(token_list, sql, schema, sentence_num)
    token_list, previous_col_match, previous_db_match, pattern_for_return = (
        sentence_cut_analyze(
            sq.sub_sequence_list,
            sq.table_match,
            sq.original_idx,
            schema,
            sq,
    ...
    for i, t in enumerate(sq.sub_sequence_type):
        if i > 0 and t == sq.sub_sequence_type[i - 1]:
            pattern_for_return[i - 1][0] = copy.deepcopy(
                pattern_for_return[i - 1][0]
            ) + copy.deepcopy(pattern_for_return[i][0])
...
def add_col_analyze(
    ...
    def add_col(
        ...
        if insert_idx == -1:
            for p_tok, q_tok in zip(p_tokens, sub_q.question_tokens[list_idx]):
                if (
                    (break_grsm and ("GRSM" in p_tok or "JJS" in p_tok))
                    or "DATE" in p_tok
                    or "YEAR" in p_tok
                    or (
                        "NUM" in p_tok
                        and "DATE" not in p_tokens
                        and "YEAR" not in p_tok
                    )
                    or q_tok.text == "between"
                    or q_tok.text == "'"
                ):
                    break
                start_idx += 1
        ...
        if add_word_directly:
            token_list.insert(
                offset + sub_q.original_idx[list_idx][start_idx],
                [
                    SToken(
                        text=add_word_directly,
                        lemma=add_word_directly,
                        ent_type="",
                        tag="NN",
                    ),
                    sub_q.sub_sequence_type[list_idx],
            ...
            for tok in schema.column_tokens[col_id]:
                ...
                token_list.insert(
                    offset + sub_q.original_idx[list_idx][start_idx],
                    [tok, sub_q.sub_sequence_type[list_idx]],
                )
                new_insert.append(offset + sub_q.original_idx[list_idx][start_idx])
                offset += 1
        return token_list, offset, new_insert

    def add_sentence_or_col(
        qtype,
        pattern,
        sub_q,
        list_idx,
        ...
        cut = False
        for i in range(1, 5):
            if insert_idx > i and sub_q.question_tokens[list_idx][
                insert_idx - i
            ].text in ["of", "for"]:
                cut = True
                break
        ...
        if not cut and (
            (
                insert_idx + 1 < len(sub_q.question_tokens[list_idx])
                and sub_q.question_tokens[list_idx][insert_idx + 1].text == "'s"
            )
            or (
                insert_idx > 3
                and (
                    str_is_num(sub_q.question_tokens[list_idx][insert_idx - 2].text)
                    or str_is_num(sub_q.question_tokens[list_idx][insert_idx - 3].text)
                )
            )
            or sub_q.question_tokens[list_idx][0].lower_ in ["who", "where"]
            or (
                insert_idx + 1 < len(sub_q.question_tokens[list_idx])
                and insert_idx + 3 >= len(sub_q.question_tokens[list_idx])
                and len(sub_q.question_tokens) == 1
                and p_tokens[insert_idx + 1] in ["COL", "TABLE-COL"]
            )
        ):
            cut = True
            extract_list = []
            ...
            for i in range(1, 5):
                if insert_idx > i and sub_q.question_tokens[list_idx][
                    insert_idx - i
                ].text in ["of", "for", "the", "is", "are", "be", "was", "were"]:
                    cut = True
                    break
                extract_list.append(sub_q.question_tokens[list_idx][insert_idx - i])
                extract_list_idx.append(insert_idx - i)
        if cut:
            ...
            offset += 1
            token_list.insert(
                offset + sub_q.original_idx[list_idx][last_idx],
                [SToken(text="that", ent_type="", tag="WP"), token_list_type],
            )
            new_insert.append(offset + sub_q.original_idx[list_idx][last_idx])
            offset += 1
            token_list.insert(
                offset + sub_q.original_idx[list_idx][last_idx],
                [SToken(text="has", ent_type="", tag="VP"), token_list_type],
            )
            new_insert.append(offset + sub_q.original_idx[list_idx][last_idx])
            offset += 1
            ...
            for i in reversed(extract_list_idx):
                token_list[i] = [
                    SToken(text="*" + token_list[i][0].text + "*", ent_type="", tag=""),
                    token_list[i][1],
                ]
            ...
            if cols:
                for c in cols[0]:
                    if schema.column_types[
                        c
                    ] == time_type or schema.column_tokens_lemma_str[c] in [
                        "date",
                        "year",
                    ]:
                        count.add(c)
    ...
    def add_full_time_type_col(
        full_match, schema, sub_q, list_idx, offset, token_list, pattern_guess
    ):
    ...
    for sentence, table_match, list_idx, type_ in zip(
        ...
        full_match = col_match_main(tables, ts, schema)
        previous_col_match.append(full_match)
        from .pattern_question_type import ADD_COL_PATTERN_FUN as PATTERN_FUN
        from .pattern_question_type import ADD_COL_PATTERNS as PATTERNS_TOKS
        ...
        if pr[0] is not None:
            if pr[0] != 6 and not use_pattern_generate_col:
                continue
            if pr[0] == 1:
                pattern_guess = pattern_recomand(
                    ts,
                    table_match,
                    full_match,
                    sub_q.sequence_entt[list_idx],
                ...
                if (
                    (" old " not in ts.lemma_ and " young " not in ts.lemma_)
                    and (
                        (
                            "GRSM than COL" in pattern_guess[0]
                            and not (
                                "than,(than) | AGG,COL" in pattern_guess[2]
                                and "SGRSM than COL" in pattern_guess[0]
                            )
                        )
                        or "JJS COL" in pattern_guess[0]
                        or "DB" in pattern_guess[0]
                    )
                    or "SJJS COL" in pattern_guess[0]
                    and (" old id " in ts.lemma_ or " young id " in ts.lemma_)
                    or "(high)" in pattern_guess[2]
                    and "COL" in pattern_guess[0]
                    or "SGRSM" not in pattern_guess[0]
                    and "SJJS" not in pattern_guess[0]
                    and " most " not in ts.text
                    and " more " not in ts.text
                ):
                    ...
                    for i, pt in enumerate(p_toks):
                        if "SGRSM" in pt or "SJJS" in pt:
                            ...
                            if last_word[1:-1] in S_ADJ_WORD_DIRECTION.keys():
                                ...
                                for w_tok, k in zip(
                                    reversed(ts.tokens),
                                    range(len(ts.tokens) - 1, -1, -1),
                                ):
                                    if w_tok.lemma_ in n_words:
                                        do_not_need_add = True
                                        break
                                    if w_tok.tag_ == "IN" and k < i:
                                        break
                                if do_not_need_add:
                                    break
                    ...
                    for i, pt in enumerate(p_toks):
                        if "SGRSM" in pt or "SJJS" in pt:
                            ...
                            if last_word[1:-1] in S_ADJ_WORD_DIRECTION.keys():
                                ...
                                if (
                                    (
                                        (len(col_id) == 1 and col_id[0] < 0)
                                        or len(col_id) != 1
                                    )
                                    and not sgrsm_word
                                    and i + 1 < len(table_match)
                                    and table_match[i + 1]
                                ):
                                    agg_id, col_id, sgrsm_word = get_AWD_column(
                                        last_word[1:-1],
                                ...
                                if len(col_id) == 1 and col_id[0] > 0:
                                    token_list, offset, new_insert = (
                                        add_sentence_or_col(
                                            ...
                                            pattern_guess[1],
                                            insert_idx=-1
                                            if type_ != 0 and "1&SJJS" not in pr[3]
                                            else i + 1,
                                        )
                                    )
                                    break
                ...
                type_ != 0 or " between 1" in ts.text or " between 2" in ts.text
            ):  # There is DATE without COL
                pattern_guess = pattern_recomand(
                    ts,
                    table_match,
                    full_match,
                ...
                should_add = True
                if "between" in pattern_guess[0] and not (
                    "between DATE DATE" in pattern_guess[0]
                    or "between YEAR YEAR" in pattern_guess[0]
                    or "between DATE and DATE" in pattern_guess[0]
                    or "between YEAR and YEAR" in pattern_guess[0]
                ):
                    should_add = False
                ...
                if should_add:
                    table_idxs = tables if tables else [-1]
                    date_types = set()
                    for pgs, tok_tmp, tok_idx in zip(
                        pattern_guess[1], ts.tokens, range(len(ts.tokens))
                    ):
                        if pgs in ["DATE", "YEAR"]:
                ...
                if (
                    "COL " not in pattern_guess[0]
                    and " COL" not in pattern_guess[0]
                    and "TABLE " not in pattern_guess[0]
                    and " TABLE" not in pattern_guess[0]
                ):
                ...
                if 0 in sub_q.sub_sequence_type or old != 0:
                    ...
                    for ei in range(
                        sub_q.original_idx[list_idx][0],
                        sub_q.original_idx[list_idx][-1] + 2,
                    ):
                ...
                need_add = True
                if sub_q.col_match[list_idx].count([]) == len(
                    sub_q.col_match[list_idx]
                ):
                    ...
                    if need_add:
                        need_add = False
                        for ti in range(
                            len(sub_q.table_match[list_idx - 1]) - 1, -1, -1
                        ):
    ...
    for pattern in easy_cut_pattern:
        ...
        for i, tok in enumerate(token_list):
            ...
            for j, p in enumerate(pattern):
                ...
                if (
                    (
                        token_list[i + j][0].text != p
                        and token_list[i + j][0].lemma_ != p
                    )
                    or table_match[i + j]
                    or col_match[i + j]
                ):
            ...
            if match:
                ...
                for z in range(i + offset, stop_idx, 1):
                    if (
                        token_list[z][0].text in [",", "and", ".", "?"]
                        and z - i < len(pattern)
                        and pattern[z - i] not in [",", "and", ".", "?"]
                    ) and z + 3 < len(token_list):
                        break
                    token_list[z][1] = 1
                return token_list
    return token_list
...
def select_split(token_list, col_match_list, db_match):
    def how_many_or_how_much():
        if (
            i >= 2
            and token_list[i - 2][0].lemma_ == "how"
            and token_list[i - 1][0].lemma_ in ["much", "many"]
            and i + 1 < len(token_list)
            and col_match_list[i]
            and col_match_list[i + 1]
        ):
        ...
        if (
            ...
            and token_list[i - 2][0].lemma_ in ["much", "many"]
            and i + 1 < len(token_list)
            and col_match_list[i]
            and col_match_list[i + 1]
        ):
    ...
    if len(col_match_list) != len(db_match_list) or len(db_match_list) != len(
        token_list
    ):
        return None
    for i, tl in enumerate(token_list):
        if (
            how_many_or_how_much()
            and there_are_condition_values()
            and tl[1] == token_list[i + 1][1]
        ):
        ...
        if (
            tl[0].lemma_ == "neither"
            and there_are_condition_values()
            and token_list[i][1] == token_list[i + 1][1]
        ):
            token_list = split(i)
            return token_list
        ...
        if tl_type != last_tl:
            ...
            for j in range(i + 1, len(token_list), 1):
                if token_list[j][1] == tl_type:
                    ...
                    break
            if (
                tl_type not in tl_type_set
                or (
                    tl_type > 2
                    and (
                        tl[0].text in [",", "but", "and", "or", ".", "?"]
                        or token_list[i - 1][0].text
                        in [",", "but", "and", "or", ".", "?"]
                    )
                )
                or last_tl == 0
            ) and count != 0:  # means it will have at least two token for this type
                last_tl = tl_type
                tl_type_set.add(last_tl)
            elif count <= 2:
                if (
                    i + count + 1 < len(token_list)
                    and tl[0].lemma_ in SELECT_FIRST_WORD
                    and tl_type == 0
                    and not tl[0].text.islower()
                ):
                    for j in range(i + count + 2, len(token_list), 1):
                        if token_list[j][1] == token_list[j - 1][1]:
                            token_list[j][1] = tl[1]
                    token_list[i + count + 1][1] = tl[1]
                    last_tl = token_list[j][1]
                elif (
                    count >= 1
                    and tl[0].text in [",", "but", "and", "or", ".", "?"]
                    or token_list[i - 1][0].text in [",", "but", "and", "or", ".", "?"]
                    and i + count + 1 < len(token_list)
                ):
...
def special_word_modify(question, schema):
    ...
    if (
        len(question_toks[-1].text) == 2
        and question_toks[-1].text[0].isalpha()
        and not question_toks[-1].text[1].isalpha()
    ):
    ...
    for start_idx, tok in enumerate(question_toks):
        ...
        if "_" in tok.lower_:
            ...
            start_idx > 2
            and tok.text == "what"
            and question_toks[start_idx - 1].text == "and"
        ):
            if (
                question_toks[start_idx - 3].lower_ == "which"
                or start_idx > 3
                and question_toks[start_idx - 4].lower_ == "which"
            ):
                question_toks[start_idx] = SToken(text="")
        elif (
            tok.lemma_ == "least"
            and question_toks[start_idx - 1].text in ["of", "at"]
            and start_idx + 1 < len(question_toks)
            and question_toks[start_idx + 1].text != "one"
            and (
                str_is_num(question_toks[start_idx + 1].text)
                or (
                    start_idx + 2 < len(question_toks)
                    and str_is_num(question_toks[start_idx + 2].text)
                )
                or (
                    start_idx + 3 < len(question_toks)
                    and str_is_num(question_toks[start_idx + 3].text)
                )
            )
        ):
            ...
            tok.lemma_ == "most"
            and question_toks[start_idx - 1].lemma_ in ["of", "at", "have"]
            and (
                str_is_num(question_toks[start_idx + 1].text)
                or (
                    start_idx + 2 < len(question_toks)
            ...
            tok.lemma_ == "more"
            and question_toks[start_idx - 1].lemma_ == "no"
            and start_idx + 2 < len(question_toks)
            and str_is_num(question_toks[start_idx + 2].text)
        ):
            question_toks[start_idx - 1] = SToken(text="uglier")
            question_toks[start_idx] = SToken(text="than")
        elif tok.lemma_ in ["reach", "reached", "reaching"] and str_is_num(
            question_toks[start_idx + 1].text
        ):
            question_toks[start_idx] = SToken(text="shinier")
        elif (
            tok.text in ["title", "titles", "titled"]
            and "title" not in schema.table_col_lemma[-1]
            and "titled" not in schema.table_col_lemma[-1]
            and "titles" not in schema.table_col_lemma[-1]
        ):
            if tok.text == "title":
            ...
            (tok.lemma_ in SYNONYM or tok.text in SYNONYM)
            and tok.lemma_ not in schema.table_tokens_lemma_str
            and tok.text not in schema.table_tokens_text_str
            and tok.text not in schema.table_col_lemma[-1]
            and tok.lemma_ not in schema.table_col_lemma[-1]
            and tok.lemma_ not in schema.table_col_text[-1]
        ):
            key_s = tok.lemma_ if tok.lemma_ in SYNONYM else tok.text
            for syn in SYNONYM[key_s]:
                if (
                    syn in schema.table_tokens_lemma_str
                    or syn in schema.table_tokens_text_str
                    or syn in schema.table_col_lemma[-1]
                    or syn in schema.table_col_text[-1]
                ):
                    question_toks[start_idx] = SToken(text=syn)
                    break
        elif start_idx + 4 < len(question_toks) and start_idx >= 1:
            if (
                tok.lemma_ in ["range", "from", "between"]
                and start_idx + 4 < len(question_toks)
                and question_toks[start_idx + 2].text != "or"
                and question_toks[start_idx + 3].text != "or"
            ):
                if str_is_num(question_toks[start_idx + 1].text) and str_is_num(
                    question_toks[start_idx + 3].text
                ):
                    question_toks[start_idx] = SToken(text="between")
                    question_toks[start_idx + 2] = SToken(text="and")
                elif str_is_num(question_toks[start_idx + 2].text) and str_is_num(
                    question_toks[start_idx + 4].text
                ):
                    question_toks[start_idx] = SToken(text="between")
                    question_toks[start_idx + 3] = SToken(text="and")
                    question_toks[start_idx + 1] = SToken(text="")
            elif tok.lemma_ == "full" and question_toks[start_idx + 1].lemma_ in [
                "name",
                "names",
            ]:
...
def anaylse_punctuate(token_list, question, schema, all_word, keep_original_question):
    def reset_next_type(token_list, i):
        if i + 1 < len(token_list):
            ...
            if (
                i >= 3
                and j + 4 < len(token_list)
                and token_list[i - 1][1] == token_list[j + 1][1]
                and token_list[i - 1][0].text not in ["(", "'", ")"]
                and token_list[i - 2][0].text not in ["(", "'", ")"]
                and token_list[i - 3][0].text not in ["(", "'", ")"]
                and token_list[i][0].text not in ["(", "'", ")"]
            ):
                return comma(
                    token_list,
                    i - 1,
    ...
    def jjr_check(token_list, i, tokens):
        ...
        if than_idx and than_type != token_list[i][1]:
            ...
            for j in range(i, -1, -1):
                if (
                    token_list[j][0].tag_.startswith("V")
                    or token_list[j][0].tag_ == "IN"
                    or token_list[j][0].text in ["that", "with", "which"]
                ):
                    token_list[j][1] = 9
                    break
                token_list[j][1] = 9
    ...
    for i, tok in enumerate(token_list):
        if i > 0 and tok[1] != token_list[i - 1][1]:
            if (
                " ' " in question
                and get_punctuation_word(tokens, i)
                or " ( " in question
                and get_punctuation_word(tokens, i, punct=["(", ")"])
            ):
        ...
        if tokens[i].tag_ in ["JJR", "RBR"]:
            ...
            tokens[i].text == "between"
            and i + 4 < len(tokens)
            and tokens[i + 1].text.isdigit()
            and tokens[i + 2].text == "and"
            and tokens[i + 3].text.isdigit()
            and token_list[i][1] != token_list[i + 3][1]
        ):
            token_list = between_set(
                token_list, i, token_list[i][1], token_list[i + 3][1]
            )
        elif (
            tokens[i].text == ","
            and i > 0
            and token_list[i - 1][1] > 0
            and i + 2 < len(tokens)
            and token_list[i - 1][1] == token_list[i + 1][1]
        ):
            ...
            tokens[i].text == "'"
            and i > 0
            and token_list[i - 1][1] != token_list[i][1]
            and get_punctuation_word(tokens, i + 1)
        ):
            token_list = reset_next_type(token_list, i)
        elif (
            tokens[i].text == "or"
            and i + 5 < len(token_list)
            and i > 0
            and token_list[i + 1][1] == token_list[i - 1][1]
            and token_list[i][1] == token_list[i - 1][1]
            and tokens[i + 2].text not in [",", ".", "?"]
            and tokens[i + 3].text not in [",", ".", "?"]
            and token_list[i + 1][1] != token_list[i + 3][1]
            and tokens[i + 1].text.islower()
            and len(tokens[i + 1].text) > 2
            and tokens[i + 1].lemma_ in all_word
        ):
            if " ' " not in question or not get_punctuation_word(tokens, i):
                token_list = comma(token_list, i, token_list[i + 1][1])
        elif (
            tokens[i].text in ["with", "in"]
            and i + 2 < len(token_list)
            and i > 0
            and token_list[i - 1][1]
            and token_list[i][1]
            and token_list[i][1] != token_list[i - 1][1]
            and tokens[i - 1].lemma_ in ["end", "start"]
        ):
            if " ' " not in question or not get_punctuation_word(tokens, i):
                token_list = reset_next_type(token_list, i)
        elif (
            tokens[i].text == "and"
            and "between" not in question
            and i + 4 < len(token_list)
            and token_list[i][1] == token_list[i + 1][1]
            and (
                token_list[i + 1][1] != token_list[i + 2][1]
                or token_list[i + 3][1] != token_list[i + 1][1]
            )
            and token_list[i][1] > 0
        ):
            if " ' " not in question or not get_punctuation_word(tokens, i):
            ...
            if " ' " not in question or not get_punctuation_word(tokens, i):
                token_list = comma(
                    ...
                    i,
                    token_list[i][1],
                    nt=token_list[i][1] + 10,
                    nj=len(token_list),
                )
        elif (
            not keep_original_question
            and tokens[i].text == "least"
            and tokens[i - 1].tag_.startswith("V")
            and str_is_num(tokens[i + 1].text)
            and tokens[i + 1].text != "one"
        ):
            ...
            tokens[i].text == "("
            and token_list[i][1] != token_list[i - 1][1]
            and get_punctuation_word(tokens, i + 1, punct=["(", ")"])
        ):
            if " ' " not in question or not get_punctuation_word(tokens, i):
            ...
            (tokens[i].lemma_ in ["order", "relate"])
            and i + 3 < len(token_list)
            and token_list[i][1] != token_list[i + 1][1]
            and (tokens[i + 1].text in ["by", "to"])
        ):
            if " ' " not in question or not get_punctuation_word(tokens, i):
                token_list = comma(token_list, i, token_list[i][1])
        elif (
            i > 2
            and tokens[i].text in ["most", "least"]
            and token_list[i - 2][1] == token_list[i][1]
            and token_list[i - 2][1] == 0
            and tokens[i - 2].text not in ["that", "who", "which", "when"]
            and tokens[i - 1].lemma_ == "have"
        ):
            token_list = comma(token_list, i - 1, 0)
        elif (
            i > 2
            and tokens[i].text in ["most", "least"]
            and token_list[i - 3][1] == token_list[i][1]
            and token_list[i - 3][1] == 0
            and tokens[i - 3].text not in ["that", "who", "which", "when"]
            and tokens[i - 2].lemma_ == "have"
            and tokens[i - 1].lemma_ == "the"
        ):
            ...
            and i > 0
            and token_list[i][1] != token_list[i - 1][1]
            and tokens[i - 1].tag_ in ["JJS", "RBS"]
            and i + 4 >= len(tokens)
        ):
            if " ' " not in question or not get_punctuation_word(tokens, i):
                token_list = reset_next_type(token_list, i)
            ...
            i == 1
            and tokens[i - 1].lower_ == "which"
            and token_list[i][1] != token_list[i - 1][1]
            and (
                token_list[i][1] != token_list[i + 1][1]
                or token_list[i][1] != token_list[i + 2][1]
            )
        ):
            if token_list[0][1] == 0:
    ...
    if (
        len(token_list) >= 3
        and token_list[-3][0].text == "in"
        and token_list[-3][1] == token_list[-4][1]
        and token_list[-2][0].text.isdigit()
    ):
...
def db_correction(token_list, col_match, sql, schema):
    ...
    def correct_multiple_tokens(schema, sql, start_idx, question_toks, token_list):
        db_str = get_database_string(
            schema,
            sql["db_match"][start_idx][0][0],
            question_toks[
                start_idx : start_idx
                + sql["db_match"][start_idx][0][1][1]
                - sql["db_match"][start_idx][0][1][0]
                + 1
            ],
        )
        if (
            db_str.count(" ")
            < sql["db_match"][start_idx][0][1][1] - sql["db_match"][start_idx][0][1][0]
        ):
            len_start = 0
            ...
            for z, tok_z in enumerate(db_str.split(" ")):
                token_list[start_idx + z] = [
                    SToken(text=tok_z),
                    token_list[start_idx + z][1],
                ]
                question_toks[start_idx + z] = SToken(text=tok_z)
                sql["db_match"][start_idx + z] = [
                    [dbm[0], [len_start, len_end]]
                    for dbm in sql["db_match"][start_idx + z]
                ]
        elif db_str != " ".join(
            [
                token_list[sii][0].text
                for sii in range(start_idx, start_idx + db_str.count(" ") + 1)
            ]
        ):
            for sii, db_tok in zip(
                range(start_idx, start_idx + db_str.count(" ") + 1), db_str.split(" ")
            ):
    ...
    for start_idx, tok in enumerate(question_toks):
        ...
        if (
            (not tok.text.islower() or get_punctuation_word(question_toks, start_idx))
            and not sql["db_match"][start_idx]
            and not str_is_num(tok.text)
            and not col_match[start_idx]
            and not sql["table_match"][start_idx]
            and len(tok.text) > 3
        ):
            if (
                re.fullmatch(r".*(start|end)(.){0,5}with.*", sql["question"])
                or re.fullmatch(
                    r"(.*?(contain|includ|with\s|without\s|have\s|has\s|having\s|had\s|like|\sas\s)|as\s).*(substring|string|char\s|chars\s|character\s|letter|word|phrase|prefix|suffix).*",
                    sql["question"],
                )
                or re.fullmatch(
                    r".*(contain|includ|like)(.){0,4}(').*?(').*", sql["question"]
                )
                or re.fullmatch(
                    r".*(hav|includ|has|with)(.){0,4}((an|a|the)\s){0,1}(').*?(')\sin(\s|\?|\.).*",
                    sql["question"],
                )
            ):
                continue
            ...
            if start_idx + 1 < len(question_toks):
                words.append(
                    (
                        tok.text + " " + question_toks[start_idx + 1].text,
                        len(tok.text + " " + question_toks[start_idx + 1].text),
                        tok.text[0].lower(),
                    )
                )
            if start_idx + 2 < len(question_toks):
                words.append(
                    (
                        ...
                        + question_toks[start_idx + 1].text
                        + " "
                        + question_toks[start_idx + 2].text,
                        len(
                            tok.text
                            + " "
                            + question_toks[start_idx + 1].text
            ...
            if start_idx > 0 and sql["db_match"][start_idx - 1]:
                words.append(
                    (
                        question_toks[start_idx - 1].lower_ + " " + tok.lower_,
                        len(question_toks[start_idx - 1].text + " " + tok.text),
                        question_toks[start_idx - 1].text[0].lower(),
                    )
                )
            ...
            for ass in all_str:
                for col_str in ass:
                    ...
                    if start_idx + word_num >= len(question_toks) or word_num > 2:
                        continue
                    tok_l = len(col_str[0])
                    if col_str[0][0].lower() == words[word_num][2] and (
                        tok_l == words[word_num][1]
                        or tok_l + 1 == words[word_num][1]
                        or tok_l - 1 == words[word_num][1]
                    ):
                        ...
                        if sw or edit_dis <= 1 or (tok_l >= 9 and edit_dis <= 2):
                            ...
                            for k in range(word_num + 1):
                                question_toks[start_idx + k] = SToken(
                                    text=col_str[0].split(" ")[k]
                                )
                                token_list[start_idx + k] = [
                                    SToken(text=col_str[0].split(" ")[k]),
                                    token_list[start_idx][1],
                                ]
                            res = datebase_match_tables(
                                schema,
                                question_toks[start_idx],
                                start_idx,
                                question_toks,
                                [-1],
                                True,
        ...
        if not sql["db_match"][start_idx] and (
            ...
            table_idxs = get_all_table(
                sql["table_match"], col_match, sql["db_match"], schema
            )
            if datebase_match_tables(
                schema,
                question_toks[start_idx],
                start_idx,
                question_toks,
            ...
            for c in COUNTRYS_DICT[key]:
                res = datebase_match_tables(
                    schema, SToken(text=c), 0, [SToken(text=c)], table_idxs, True
                )
                if res:
                    k_space = key.count(" ")
                    c_space = c.count(" ")
                    if k_space < c_space:
                        for z in range(c_space - k_space):
                            ...
                            sql["question_tag"].insert(
                                start_idx + 1, sql["question_tag"][start_idx]
                            )
                            sql["question_entt"].insert(
                                start_idx + 1, sql["question_entt"][start_idx]
                            )
                    ...
                    for z in range(c_space + 1):
                        tmp = c.split(" ")[z]
                        token_list[start_idx + z] = [
                            SToken(text=tmp[0].upper() + tmp[1:]),
                            token_list[start_idx][1],
                        ]
                        question_toks[start_idx + z] = SToken(
                            text=tmp[0].upper() + tmp[1:]
                        )
                        sql["db_match"][start_idx + z] = res
                    break

        if (
            tok.text in ["male", "female", "females", "males"]
            and not sql["db_match"][start_idx]
            and not col_match[start_idx]
            and not sql["table_match"][start_idx]
        ):
            ...
            for _i, qtok in enumerate(question_toks):
                if (
                    qtok.lower_ == "f"
                    and sql["db_match"][_i]
                    and tok.text in ["female", "females"]
                    or qtok.lower_ == "m"
                    and sql["db_match"][_i]
                    and tok.text in ["male", "males"]
                ):
                    add_db = False
            if add_db:
                res = []
                for i, col in enumerate(schema.column_tokens_lemma_str):
                    if (
                        schema.column_types[i] == "boolean"
                        or col in ["sex | gender", "gender | sex"]
                    ) and ("sex" in col or "male" in col or "female" in col):
                        res.append([i, [start_idx, start_idx]])
                sql["db_match"][start_idx] = res

        elif (
            start_idx > 0
            and lstem.stem(tok.text)
            in [
                "jan",
                "feb",
                "mar",
            ...
            and not sql["db_match"][start_idx]
            and (
                question_toks[start_idx + 1].text.isdigit()
                or question_toks[start_idx + 1].text[:-2].isdigit()
                or question_toks[start_idx - 1].text.isdigit()
                or question_toks[start_idx - 1].text[:-2].isdigit()
            )
            and start_idx + 3 < len(question_toks)
            and not sql["db_match"][start_idx]
        ):
            date = 0
            year_offset = 2
            ...
            if question_toks[start_idx + year_offset].text.isdigit():
                ...
                question_toks[start_idx + year_offset].text == ","
                and start_idx + year_offset + 1 < len(question_toks)
                and question_toks[start_idx + year_offset + 1].text.isdigit()
            ):
            ...
            if year and date:
                ...
                if res:
                    ...
                    sql["db_match"][start_dt] = [
                        resss for ress in res for resss in ress
                    ]
                else:
                    question_toks[start_dt] = SToken(text=gold_dt)
                    token_list[start_dt] = [
                        SToken(text=gold_dt),
                        token_list[start_dt][1],
                    ]
        elif (
            start_idx > 0
            and not sql["db_match"][start_idx - 1]
            and sql["db_match"][start_idx]
            and sql["db_match"][start_idx][0][1][1]
            > sql["db_match"][start_idx][0][1][0]
        ):
            correct_multiple_tokens(schema, sql, start_idx, question_toks, token_list)
        elif (
            start_idx > 0
            and sql["db_match"][start_idx - 1] != sql["db_match"][start_idx]
            and sql["db_match"][start_idx]
            and sql["db_match"][start_idx][0][1][1]
            == sql["db_match"][start_idx][0][1][0]
        ):
            ...
            if not db_str and token_list[start_idx][0].text in {
                "male": "m",
                "female": "f",
                "females": "f",
                "males": "m",
            ...
            and start_idx + 2 < len(question_toks)
            and not sql["db_match"][start_idx]
            and token_list[start_idx][0].text.isalpha()
            and (
                not token_list[start_idx][0].text.islower()
                or (
                    token_list[start_idx - 1][0].text == "'"
                    and get_punctuation_word(question_toks, start_idx, True)
                )
            )
        ):
            for or_i, tok in enumerate(sql["or_question"]):
                if tok.text == token_list[start_idx][0].text and or_i >= start_idx:
                    res = datebase_match_tables(
                        schema, tok, or_i, sql["or_question"], [-1], True
                    )
                    if res:
                        add_word_offset = 0
                        for res_idx in range(res[0][0][1][0], res[0][0][1][1] + 1):
                            if (
                                sql["or_question"][res_idx].text
                                != token_list[start_idx + add_word_offset][0].text
                            ):
                                token_list.insert(
                                    start_idx + add_word_offset,
                                    [
                                        SToken(text=sql["or_question"][res_idx].text),
                                        token_list[start_idx + add_word_offset][1],
                                    ],
                                )
                                sql["db_match"].insert(
                                    start_idx + add_word_offset,
                                ...
                                sql["question_tag"].insert(
                                    start_idx + add_word_offset,
                                    sql["question_tag"][start_idx + add_word_offset],
                                )
                                sql["question_entt"].insert(
                                    start_idx + add_word_offset,
                                    sql["question_entt"][start_idx + add_word_offset],
                                )
                                sql["table_match"].insert(
                                    start_idx + add_word_offset, []
                                )
                                sql["question_dep"]["data"].insert(
                                    start_idx + add_word_offset, []
                                )
                                col_match.insert(start_idx + add_word_offset, [])
                                question_toks.insert(
                                    start_idx + add_word_offset,
                                    SToken(text=sql["or_question"][res_idx].text),
                                )
                            sql["db_match"][start_idx + add_word_offset] = [
                                resss for ress in res for resss in ress
                            ]
                            add_word_offset += 1
                        correct_multiple_tokens(
                            schema, sql, start_idx, question_toks, token_list
                        )
                    break
...
def db_continue_correction(token_list, sql, schema):
    ...
    for (start_idx, tok), db_m in zip(enumerate(token_list), sql["db_match"]):
        ...
        if (
            start_idx > 1
            and sql["db_match"][start_idx]
            and sql["db_match"][start_idx - 1]
            and tok[1] == token_list[start_idx - 1][1]
            and get_db_col(sql["db_match"][start_idx])
            != get_db_col(sql["db_match"][start_idx - 1])
            and tok[0].text.islower() == token_list[start_idx - 1][0].text.islower()
        ):
            offset = 1
        elif (
            start_idx > 2
            and sql["db_match"][start_idx]
            and sql["db_match"][start_idx - 2]
            and not sql["db_match"][start_idx - 1]
            and token_list[start_idx - 1][0].text in ["and", "or"]
            and tok[1] == token_list[start_idx - 2][1]
            and get_db_col(sql["db_match"][start_idx])
            != get_db_col(sql["db_match"][start_idx - 2])
            and tok[0].text.islower() == token_list[start_idx - 1][0].text.islower()
        ):
            offset = 2
        if offset:
            offset += (
                sql["db_match"][start_idx - offset][0][1][1]
                - sql["db_match"][start_idx - offset][0][1][0]
            )
            ...
            if tbs1 != tbs2:
                ...
                res2 = datebase_match_tables(
                    schema,
                    question_toks[start_idx - offset],
                    start_idx - offset,
                    question_toks,
                    tbs,
                    True,
                ...
                if update_db_m:
                    for i in range(res1[0][1][1] - res1[0][1][0] + 1):
                        ...
                        for rs in res1:
                            if (
                                schema.column_names_original[rs[0]][0] in res1_tb
                                and schema.column_names_original[rs[0]][0] in res2_tb
                            ):
...
def combine_none_subquestion(token_list, sql, schema, sentence_num, col_match):
    ...
    others, pattern_type, others_token_pattern = others_analyze(
        others_list,
        others_table,
        others_list_idx,
        schema,
    ...
    for (j, q_type), patterns in zip(enumerate(sq.sub_sequence_type), sq.pattern_tok):
        if j > 0 and len(sq.question_tokens[j]) == 1:
            ...
            q_type > 0
            and sq.question_tokens[j][-1].text not in [",", "?", "."]
            and j + 1 < len(sq.sub_sequence_type)
            and is_none_type(patterns, sq.question_tokens[j])
        ):
            if (
                (
                    sq.pattern_tok[j][0] == "WP"
                    or sq.question_tokens[j][0].lemma_
                    in ["can", "be", "have", "and", "or", ",", "but"]
                    or (
                        j > 1
                        and sq.question_tokens[j - 1][-1].lemma_ in ["and", "or", ","]
                    )
                    or sq.question_tokens[j][0].tag_.startswith("V")
                )
                and (
                    sq.pattern_tok[j + 1][0] == "IN"
                    or sq.question_tokens[j + 1][0].lower_ == "in"
                    or len(sq.pattern_tok[j]) <= 3
                    or (
                        j + 2 == len(sq.pattern_tok)
                        and is_none_type(
                            sq.pattern_tok[j + 1], sq.question_tokens[j + 1]
                        )
                    )
                )
                and sq.question_tokens[j][0].lower_ not in ["for", "along", "together"]
                and sq.question_tokens[j + 1][0].lower_
                not in ["but", "for", "along", "together", ","]
                and (
                    sq.question_tokens[j][-1].lower_ not in ["but", "and", ",", "."]
                    or (
                        sq.question_tokens[j][-1].lower_ == "and"
                        and len(sq.question_tokens[j]) <= 3
                        and (
                            j <= 0
                            or sq.sub_sequence_type[j - 1]
                            != sq.sub_sequence_type[j + 1]
                        )
                    )
                )
                and (
                    sq.sub_sequence_type[j + 1] >= 0 or len(sq.question_tokens[j]) <= 3
                )
                and not (len(sq.question_tokens[j]) <= 4 and "each" in patterns)
            ):
                sq.sub_sequence_type[j] = sq.sub_sequence_type[j + 1]
                for idx in sq.original_idx[j]:
                    token_list[idx][1] = sq.sub_sequence_type[j]
                continue
            if (
                "DB" in sq.pattern_tok[j + 1]
                or "UDB" in sq.pattern_tok[j + 1]
                or "PDB" in sq.pattern_tok[j + 1]
            ):
                match = False
                ...
                "DATE" in sq.pattern_tok[j + 1]
                or "YEAR" in sq.pattern_tok[j + 1]
                or "NUM" in sq.pattern_tok[j + 1]
            ):
                match = False
            ...
            if (
                j > 0
                and sq.table_match[j - 1].count([]) == len(sq.table_match[j - 1])
                and sq.table_match[j].count([]) != len(sq.table_match[j])
            ):
                match = (
                    True
                    if sq.col_match[j - 1].count([]) == len(sq.col_match[j - 1])
                    else False
                )
            ...
            q_type > 0
            and j > 0
            and "order" in patterns
            and sq.question_tokens[j][0].tag_ == "IN"
            and ("sort" in sq.pattern_tok[j - 1] or "order" in sq.pattern_tok[j - 1])
            and (sq.pattern_tok[j - 1][0] not in ["order", "sort"] or j > 1)
        ):
            sq.sub_sequence_type[j] = sq.sub_sequence_type[j - 1]
            for idx in sq.original_idx[j]:
                token_list[idx][1] = sq.sub_sequence_type[j]
