...
class QuestionSQL:
    def __init__(self, sql, _tokenizer, tokens=None):
        ...
        self.sequence_entt = []
        tmp_sequence_dep = []
        for ssl, tm, ent, st, sd, i in zip(
            sql.sub_sequence_list,
            sql.table_match,
            sql.sequence_entt,
            sql.sequence_tag,
            sql.sequence_dep["data"],
            range(len(sql.sequence_tag)),
        ):
            self.question += ssl + " "
        ...
        if tokens:
            ...
            if len(self.question_tokens) != len(self.table_match):
                assert len(self.table_match) == self.question.count(" ") + 1, (
...
class SubQuestion:
    def __init__(
        ...
        sub_sequence = ""
        last_tok = None
        for tok, type_, tm, qt, qd, idx, qe, db_m, col_m, pt, f_db_m in zip(
            toks,
            question_type,
            table_match,
            question_tag,
            ...
            if not tm:
                ...
                for tm_ in tm:
                    if tm_ and tm_[0] not in sub_table_match[-1]:
                        ...
                        sub_table_match_weight[-1][sub_table_match[-1].index(tm_[0])]
                        < tm_[1]
                    ):
                        sub_table_match_weight[-1][
                            sub_table_match[-1].index(tm_[0])
                        ] = tm_[1]
    ...
    def add_sub_element(self, sq, sq_idx, insert_idx=None):
        ...
        self.sub_sequence_lemma_toks.insert(
            insert_idx, sq.sub_sequence_lemma_toks[sq_idx]
        )
    ...
    def original_match_type(self, tm):
        if len(tm) == 4:
            if len(tm[0]) == 2:
                if (
                    type(tm[0][1]) == float
                    or type(tm[1][1]) == float
                    or type(tm[2][1]) == float
                    or type(tm[3][1]) == float
                ):
                    return False
                return bool(tm[0] == tm[3] or tm[1] == tm[2] or tm[0] == tm[2])
            return True
        return False
    ...
    def sentence_combine(self, combine_type=0, with_token_pattern=False, type_offset=0):
        ...
        pattern_tok = []
        if combine_type == 0:
            for sub_seq, sub_type, sub_table, idx, db_m, pt in zip(
                self.sub_sequence_list,
                self.sub_sequence_type,
                self.table_match,
                range(len(self.sub_sequence_list)),
                self.db_match,
                self.pattern_tok,
            ):
                if (
                    sub_type + type_offset in [0, 1]
                    and len(self.original_idx[idx]) >= 1
                ):
    ...
    def table_match_index(self, list_idx, schema, recurrent=True):
        ...
        if list_idx > 0 and recurrent:
            for i in range(list_idx, 0, -1):
                if len(self.question_tokens[i]) > 1 and (
                    self.question_tokens[i][0].lower_
                    in [
                        "that",
                        "who",
                        "whose",
                    ...
                    or self.question_tokens[i][1].lower_
                    in [
                        "that",
                        "who",
                        "whose",
                    ...
                    if self.question_tokens[i][0].lower_ in [
                        "that",
                        "who",
                        "whose",
                        "what",
                        ...
                        "whom",
                        "which",
                    ] or self.question_tokens[i][1].lower_ in [
                        "that",
                        "who",
                        "whose",
                        "what",
                        ...
                        if len(self.table_match[i - 1]) >= 2 and list_idx == i:
                            ...
                            if (
                                self.question_tokens[i - 1][-1].lower_ in ["and", "or"]
                                and i > 1
                                and len(self.table_match[i - 2]) > 2
                            ):
                                re_match_idx.extend(self.table_match[i - 2][-1])
                                re_match_idx.extend(self.table_match[i - 2][-2])
                        if (
                            self.db_match
                            and self.db_match[i - 1]
                            and self.db_match[i - 1][-1]
                        ):
                            for db in self.db_match[i - 1][-1]:
                                re_match_idx.append(
                                    schema.column_names_original[db[0]][0]
                                )
                    break
                question_str = self.sub_sequence_list[i].lower()
                if len(self.question_tokens[i]) > 1 and (
                    "that " in question_str
                    or "who " in question_str
                    or "whose" in question_str
                    or "what " in question_str
                    or "where " in question_str
                    or "while " in question_str
                    or "whom " in question_str
                    or "which " in question_str
                    or self.question_tokens[i][0].lower_ in ["and", "or"]
                    or self.question_tokens[i][1].lower_ in ["and", "or"]
                ):
                    pass
                else:
            ...
            if select_num == 2:
                for i, type_ in enumerate(self.sub_sequence_type):
                    if type_ <= select_type and i != list_idx:
                        if self.question_tokens[i][
                            0
                        ].lower_ == "which" and self.table_match[i].count([]) < len(
                            self.table_match[i]
                        ):
                    ...
                    for c, l, t in zip(col[0], col[1], col[2]):
                        if t == 1:
                            if c in cc_separate_1:
                                if cc_separate_2[cc_separate_1.index(c)] < 1:
                        ...
                        elif " | " not in schema.column_tokens_lemma_str[c]:
                ...
                for c, l, t in zip(col[0], col[1], col[2]):
                    if t == 1:
                        if c in cc_separate_1:
                            ...
                            cc_separate_1.append(c)
                            cc_separate_2.append(1)
                    elif " | " not in schema.table_tokens_lemma_str[c]:
                        ...
                        if l == 1:
                            for ccol in schema.table_tokens_lemma_str[c].split(" | "):
                                if (
                                    self.question_tokens[self.idx2sub_id[q_idx][0]][
                                        self.idx2sub_id[q_idx][1]
                                    ].lemma_
                                    + " "
                                    in ccol
                                    or " "
