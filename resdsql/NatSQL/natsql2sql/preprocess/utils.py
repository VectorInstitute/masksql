...
from .match import S_ADJ_WORD_DIRECTION, STOP_WORDS
...
def str_is_num(s, recurrent=False):
    if (
        ...
        and type(s) == str
        and s.endswith("%")
        and str_is_num(s[:-1], True)
    ):
        return True
    return (
        s.lower() in NUM
        or s.replace(",", "").isdigit()
        or is_float(s.replace(",", ""))
        or is_negative_digit(s.replace(",", ""))
    )
...
def str_is_special_num(s):
    if (
        len(s) < 4
        or not s.isdigit()
        or re.fullmatch(r"^([1][5-9]\d{2}|[2][0]\d{2})$", s, flags=0)
        or s.endswith("00")
    ):
        return False
    return True
...
def look_for_table_idx(sub_q, list_idx, select_type, schema):
    ...
    if (
        select_type == 1
        and sub_q.sub_sequence_type[list_idx] <= select_type
        and 0 in sub_q.sub_sequence_type
    ):
        table_list = sub_q.table_match_index(
            sub_q.sub_sequence_type.index(0), schema, False
        )
    ...
    if not table_list:
        ...
        while True:
            ...
            if table_list_l == [-1] and table_list_r == [-1]:
                return [-1]
            if table_list_l != [-1] and table_list_l:
                return table_list_l
            if table_list_r != [-1] and table_list_r:
                return table_list_r
            i += 1
    if not table_list:
        table_list = [-1]
...
def look_for_closest_table_idx(table_idxs_list, start_idx):
    ...
    while True:
        ...
        if right < len(table_idxs_list) and table_idxs_list[right]:
...
def get_all_table_from_sq(sq, schema, col_appear_min_num=2):
    ...
    for ts, cs, dbs in zip(sq.table_match, sq.col_match, sq.db_match):
        ...
        for c in cs:
            if c:
                for cc in c[0]:
                    if schema.column_tokens_table_idx[cc] in tbs_c:
        ...
        for dbc in dbs:
            if dbc:
                for cc in dbc:
                    if schema.column_tokens_table_idx[cc[0]] in tbs_c:
        ...
        for dbc in dbs:
            if dbc:
                for cc in dbc:
                    if (
                        not table_idx
                        or schema.column_tokens_table_idx[cc[0]] in table_idx
                    ):
                        cols.append(cc)
    return cols
...
def construct_select_data(sentence, table_match, sentence_idxs, schema):
    ...
    def reset_element_list(
        element_sent, element_table, element_idx, layer_sent, layer_table, layer_idx
    ):
        if element_sent:
    ...
    for tok, tb, i in zip(sentence_toks, table_match, sentence_idxs):
        if (
            i == sentence_idxs[0]
            or (i == sentence_idxs[1] and sentence_toks[0] in [",", "?", "."])
        ) and tok in [
            "give",
            "show",
            "list",
        ...
        if tok in [",", "and"] and i != sentence_idxs[-1]:
            ...
            continue
        if tok in ["of", "by", "per", "for", "does", "do", "did"]:
            (
                element_sent,
                element_table,
                element_idx,
            ...
            continue
        if tok.lower() not in STOP_WORDS or tok.lower() == "when":
    ...
    if availble_sent and len(availble_sent[-1]) == 3:
        ...
        for i in range(len(availble_sent) - 1):
            ...
            for w, t, idx in zip(
                original_sent[-1], original_table[-1], original_idx[-1]
            ):
    ...
    if key:
        ...
        while True:
            ...
            for s_adj in S_ADJ_WORD_DIRECTION[key]:
                if (
                    s_adj[0] == t_tok.lemma_
                    or s_adj[0] == lstem_tok
                    or s_adj[0] == t_tok.text
                ):
                    if s_adj[2]:
                        return "SM_"
                    return "GR_"
...
def get_punctuation_word(question_tokens, start_idx, only_bool=True, punct=['"', "'"]):
    ...
    if num_left and num_right:
        if (
            (num_left % 2 == 0)
            and question_tokens[start_idx].text in punct
            and (num_right % 2 == 1)
        ):
            pass
        elif (
            num_right % 2 == 1
        ):  # I use this simple dangerous one to instead of the previous one.
            ...
            for i in range(start_idx, 0, -1):
                if question_tokens[i].text in punct:
                    ...
                    break
            if (
                num_right - num_left <= 6
                or punct != ['"', "'"]
                or num_right + 3 == len(question_tokens)
            ):
                if only_bool:
                    return True
