...
from .utils import get_punctuation_word, str_is_num, str_is_special_num
...
class DBEngine:
    ...
    def db_col_type_check(self, table_idx):
        def str_is_date(s):
            ...
            if str_is_num(s):
                return "NUM"
            if re.fullmatch(
                r"((\d{4}((_|-|/){1}\d{1,2}){2})|(\d{1,2}(_|-|/)){2}(\d{4}|\d{2})){0,1}\s{0,1}(\d{2}(:\d{2}){1,2}){0,1}",
                s,
                flags=0,
            ) or re.fullmatch(
                r"(\d{1,2}(st|nd|rd|th){0,1}(,|\s|-)){0,1}((J|j)(an|AN)(uary){0,1}|(F|f)(eb|EB)(ruary){0,1}|(M|m)(ar|AR)(ch){0,1}|(A|a)(pr|PR)(il){0,1}|(M|m)(ay|AY)|(J|j)(un|UN)(e){0,1}|(J|j)(ul|UL)(y){0,1}|(A|a)(ug|UG)(ust){0,1}|(S|s)(ep|EP)(tember){0,1}|(O|o)(ct|CT)(ober){0,1}|(N|n)(ov|OV)(ember){0,1}|(D|d)(ec|EC)(ember){0,1})(\s|,|-)(\d{1,2}(st|nd|rd|th){0,1}(\s|,){1,3}){0,1}(\d{4}|\d{2})",
                s,
                flags=0,
            ):
                return "DATE"
            if re.fullmatch(
                r"^(\d{1,2}:\d{1,2}(.){0,1}\d{0,2}(.){0,1}\d{0,2})$", s, flags=0
            ):
                return "DATE"
            return None
        ...
        for col, n_col, c_type in zip(
            ...
            if len(values) == 0 or (len(values) == 1 and values[0][0] in ["", None]):
                if (
                    n_col.lower().endswith(" date")
                    or n_col.lower() == "data"
                    or n_col.lower().endswith(" time")
                    or n_col.lower() == "time"
                    or n_col.lower() == "duration"
                ):
            ...
            if len(values) == 2 or (len(values) == 3 and str(values[0][0]) == ""):
                ...
                if (
                    (left == "0" and right == "1")
                    or (left == "f" and right == "t")
                    or (left == "false" and right == "true")
                    or (left == "n" and right == "y")
                    or (left == "no" and right == "yes")
                    or (left == "f" and right == "m")
                    or (left == "female" and right == "male")
                    or (left == "f" and right == "s")
                    or (left == "fail" and right == "success")
                    or (left == "b" and right == "g")
                    or (left == "bad" and right == "good")
                    or (left == "cancelled" and right == "completed")
                    or (left == "l" and right == "r")
                    or (left == "left" and right == "right")
                ):
                    bool_col.append(col)
            ...
            if not is_str:
                ...
                for v in values:
                    if v[0] in [
                        ...
                        continue
                    if (v[0] or v[0] == 0) and (
                        type(v[0]) in [int, float]
                        or (
                            type(v[0]) == str
                            and (str_is_date(v[0]) or str_is_num(v[0]))
                        )
                    ):
                        check_max -= 1
                        if check_max <= 0:
                ...
                if is_str:
                    ...
                    if len(values) >= 2:
                        ...
                        elif type(v[0]) in [int, float] and (
                            (n_col == "year")
                            or ("year " in n_col)
                            or n_col.lower().endswith(" year")
                            or (n_col == "founded")
                        ):
                            all_cols.append((col, 4))
                        elif type(v[0]) in [int] and (
                            n_col.lower().endswith(" date")
                            or n_col.lower() == "data"
                            or n_col.lower().endswith(" time")
                            or n_col.lower() == "time"
    ...
    def db_content_are_same(self, col_1, col_2):
        query = (
            "select distinct "
            + self.column_list[col_1][1]
            + " from "
            + self.table_list[self.column_list[col_1][0]]
            + " limit 500"
        )
        query2 = (
            "select distinct "
            + self.column_list[col_2][1]
            + " from "
            + self.table_list[self.column_list[col_2][0]]
            + " limit 500"
        )
        ...
        if len(values) == 0 or len(values2) == 0:
            return True
        values, values2 = (
            (values, values2) if len(values) > len(values2) else (values2, values)
        )
        pass_one = False
        for v in values2:
    ...
    def contain_token(self, token, table, table_idx, all_utter_tokens, token_idx):
        ...
        try:
            ...
            self.db.text_factory = lambda x: str(x, "latin1")
            return None, None
        return self.contain_token_column_idx(
            values, token, col_idx, all_utter_tokens, token_idx
        )
    ...
    def query_return_match(self, str_return, one_token, all_utter_tokens, token_idx):
        ...
        if (
            str_return == one_token
            or str_return
            and str_return.isalpha()
            and " " not in str_return
            and lemmatization(str_return) == one_token
            and one_token not in ALL_SPECIAL_WORD
        ):
            return [token_idx, token_idx], db_string
        start_token_idx = token_idx
        token_idx += 1
        if all_utter_tokens:
            while len(str_return) > len(one_token) and token_idx < len(
                all_utter_tokens
            ):
                next_token = one_token + " " + all_utter_tokens[token_idx].lower_
                if (
                    str_return == next_token
                    or str_return + "s" == next_token
                    or str_return == next_token + "s"
                ):
    ...
    def contain_token_column_idx(
        self, values, token, col_idx, all_utter_tokens, token_idx
    ):
        if len(values) == 0:
            return [], None
        result = []
        ...
        for row in values:
            ...
            for i, v in enumerate(row):
                res, db_string = self.query_return_match(
                    v, token, all_utter_tokens, token_idx
                )
    ...
    def check_disjoint_column(self, column_id):
        query = (
            ...
            + str(self.column_list[column_id][1])
            + "),count("
            + str(self.column_list[column_id][1])
            + ") from "
            + str(self.table_list[self.column_list[column_id][0]])
            + " ;"
        )
        cursor = self.db.cursor()
        try:
...
def datebase_match(schema, tok, tok_idx, utter_tokens, table_idx, cross_table=True):
    ...
    Return list:
        < [ col_in_table_index ,
            [where_right_string_start_idx,where_right_string_stop_idx] ],
            ... >
    """

    def try_one_table(_db: DBEngine, tok, table, table_idx, utter_tokens, tok_idx):
        result, db_string = _db.contain_token(
            tok.text, table, table_idx, utter_tokens, tok_idx
        )
        if result:
            if len(result) == 1:
                return result
        ...
        if (
            ...
            result, db_string = _db.contain_token(
                tok.lemma_, table, table_idx, utter_tokens, tok_idx
            )
            if result:
                if len(result) == 1:
                    return result
    ...
    if (
        ...
        and (not tok.text.isalpha() or tok.text.islower())
    ):
        is_punt, punt_edge = get_punctuation_word(
            utter_tokens, tok_idx, only_bool=False
        )
    ...
    if table_idx >= 0:
        result = try_one_table(
            _db,
            tok,
            schema.table_names_original[table_idx],
            table_idx,
            utter_tokens,
            tok_idx,
        )
    ...
    if cross_table:
        ...
        for j, table in enumerate(schema.table_names_original):
            if j == table_idx:
                continue
            result = try_one_table(
                _db, tok, schema.table_names_original[j], j, utter_tokens, tok_idx
            )
            if result:
...
def datebase_match_tables(
    schema,
    tok,
    tok_idx,
    utter_tokens,
    table_idxs,
    return_all_match=False,
    search_extra_table=False,
):
...
def get_database_string(schema, col_idx, all_tokens):
    ...
    result, db_string = _db.contain_token(
        all_tokens[0].text,
        schema.table_names_original[schema.column_tokens_table_idx[col_idx]],
        schema.column_tokens_table_idx[col_idx],
        all_tokens,
        0,
    )
    if not result:
...
def get_database_col(schema, db_match, all_tokens):
    ...
    for i, t in enumerate(schema.table_names_original):
        ...
        if not result:
            result, db_string = _db.contain_token(
                all_tokens[0].lemma_, t, i, all_tokens, 0
            )
        if result:
            for r in result:
