from .col_match import col_match_main, of_for_structure_in_col
...
from .utils import construct_select_data, look_for_table_idx
...
def use_column_find_table(
    ...
    table_idxs = [-1]
    all_tables = set()
    agg_cols, _1, _2, _3, distinct = type_1(
        availble_sent,
        availble_table,
        availble_idx,
        original_sent,
...
def type_1(
    ...
    for col, a_col, a_col_idx, o_col_idx, col_table, c_idx in zip(
        ...
        original_idx,
        original_table,
        range(len(original_sent)),
    ):
        agg_id, col_id, table_idxs, distinct, where, group, order = one_col_match(
            col,
            a_col,
            a_col_idx,
            o_col_idx,
...
def type_2(
    ...
    column_all, _1, _2, _3, distinct = type_1(
        availble_sent,
        availble_table,
        availble_idx,
        original_sent,
...
def pre_analyse_table_match(
    ...
    for col, a_col, a_col_idx, o_col_idx, col_table, c_idx in zip(
        ...
        agg_id, col_id, new_table_idx, distinct, where, group, order = one_col_match(
            col,
            a_col,
            a_col_idx,
            o_col_idx,
        ...
        if (
            col_id
            and len(col_id) == 1
            and schema.column_tokens_table_idx[col_id[0]] != new_table_idx
        ):
            change_table_match(
                c_idx,
                new_table_idx,
...
def analyse_table_match(
    ...
    for at, ai, ast, i in zip(
        ...
        if (
            len(at) == 1
            and (len(at[0]) == 1 or (len(at[0]) == 2 and ast[0][0] in DISTINCT_WORDS))
            and at[0]
            and at[0][-1]
            and schema.column_contain_word(ast[0][-1])
            and not schema.column_contain_word("name", at[0][-1])
            and sub_q.sub_sequence_type.count(0) == 1
        ):
    ...
    if (
        len(sentences) == 1
        and len(availble_table) == 1
        and len(original_table) == 1
        and len(original_table[0]) == 1
        and original_table[0][0]
    ):
        cannot_delete = False
        ...
        if not cannot_delete:
            agg, col = schema.one_word_to_tables_column_match(
                [-1],
                original_sent[0][0],
...
def reconize_select_type(
    ...
    def only_one_table(
        tm_num, original_table, availble_table, availble_idx, q_toks, list_idx, sub_q
    ):
        ...
        if tm_num == 1:
            ...
            if original_table[0][0]:
                return True, original_table[0][0]
        elif tm_num == 2:
            table_idx = [-1]
            table_in_col_idx = -1
            for i, col, a_col in zip(
            ...
            for c in col:
                if c != []:
                    if len(c) == 1:
                        ...
                        return False
        return False not in t_in_c

    tm_num = table_match_num(sub_q.table_match[list_idx])
    oot, table_idxs = only_one_table(
        tm_num,
        original_table,
        availble_table,
        availble_idx,
    ...
    if is_no_table_match(
        q_sql.table_match
    ):  # Special type 1. No table in table_match but it is high possible the table is in the end of select which is the same as type 1
        if len(availble_sent[-1]) == 2:
            check = True
            for idx in availble_idx[-1][-1]:
                if (
                    sub_q.question_tokens[list_idx][idx - sub_q.offset[list_idx]].text
                    in schema.table_col_text[-1]
                    or sub_q.question_tokens[list_idx][
                        idx - sub_q.offset[list_idx]
                    ].lemma_
                    in schema.table_col_text[-1]
                    or sub_q.question_tokens[list_idx][
                        idx - sub_q.offset[list_idx]
                    ].lemma_
                    in schema.table_col_lemma[-1]
                ):
                    check = False
            if check:
                # Try to find the table through column name
                # table_idx is not a list.
                table_idx = use_column_find_table(
                    availble_sent,
                    availble_table,
                    availble_idx,
                    original_sent,
                ...
                if table_idx >= 0:
                    availble_table[-1][-1] = [
                        [table_idx] for i in range(len(availble_sent[-1][-1]))
                    ]
...
def others_analyze(
    ...
    for sentence, table_match, list_idx, db_m in zip(
        ...
        pr, db_match = pattern_reconize(
            ...
            schema,
            tables,
            pattern_toks=[[["START", "SEARCH", "DATABASE"]]],
            pattern_fun=[None],
            in_db_match=db_m,
        )
...
def select_analyze(
    stemmer, sentences, table_matchs, list_idxs, schema: Schema_Token, sub_q, q_sql
):
    def shall_we_skip_first_select(
        sentences, original_table, start_idx, list_idxs, sub_sentence_idx
    ):
        skip_table_set = set()
    ...
    real_sentence_num = (q_sql.question + " ").count(" . ") + (
        q_sql.question + " "
    ).count(" ? ")
    re_value = []
    key_word = []
    ...
    for i, sentence, table_match, list_idx in zip(
        range(sentence_num), sentences, table_matchs, list_idxs
    ):
        (
            availble_sent,
            availble_table,
        ...
        if (
            len(original_sent) == 1
            and not original_sent[0]
            or availble_sent
            and (
                "each" in availble_sent[0][0]
                or (len(availble_sent[0][0]) == 1 and " by each " in sentence)
            )
        ):
            continue
        if (
            sub_q.sub_sentence_idx[list_idx] == 0
            and real_sentence_num == 2
            and len(original_table) == 1
        ):
            skip_first_select, skip_table_set = shall_we_skip_first_select(
                sentences, original_table, i, list_idxs, sub_q.sub_sentence_idx
            )
            ...
            and sentence_num == 3
            and i < 2
            and " each " in sentence
        ):
            continue

        fun, table_idxs = reconize_select_type(
            stemmer,
            availble_sent,
            availble_table,
            availble_idx,
    ...
    if sentences and (
        ...
        or (
            (
                " for the " in sentences[0]
                or (len(sentences) > 1 and " for the " in sentences[1])
                or (len(sentences) > 2 and " for the " in sentences[2])
            )
            and len(select) == 1
            and select[0][0]
        )
        ...
        if (
            not select
            or len(select) == 1
            or (len(select) == 2 and (select[0][0] and select[1][0]))
        ):
            for i, sentence, table_match, list_idx in zip(
                range(sentence_num), sentences, table_matchs, list_idxs
            ):
                ...
                if (
                    len(original_sent) == 1 and not original_sent[0]
                ) or "each" not in original_sent[0]:
                    continue

                availble_table, original_table, sub_q, q_sql = analyse_table_match(
                    availble_sent,
                    availble_table,
                    availble_idx,
                    original_sent,
