...
ALL_SPECIAL_WORD = (
    STOP_WORDS.union(NEGATIVE_WORDS)
    .union(SELECT_FIRST_WORD)
    .union(AGG_WORDS)
    .union(ALL_IMPORTANT_PATTERN_TOKENS)
    .union(NOT_STAR_WORD)
)
...
for i, c in enumerate(COUNTRYS):
    for j, cn in enumerate(c):
        COUNTRYS[i][j] = cn.lower()
    for cnn in c:
        COUNTRYS_DICT[cnn] = [c for c in c if c != cnn]
...
def word_is_grsm(word_tok):
    if (
        word_tok.lemma_ in ABSOLUTELY_GRSM_DICT
        or word_tok.lemma_ == word_tok.text
        and word_tok.text.endswith("er")
        and word_tok.text[:-2] in ABSOLUTELY_GRSM_DICT
    ):
        return True
    return False
...
def column_match(col_str, schema, table_str=None, table_id=-1):
    ...
    if (
        ...
        agg_id, col_id = schema.column_match(
            table_id, [col_names[0] + " " + table_str[0]]
        )  # for suitable for number of column name, I modify the table name as behind
