...
from .db_match import datebase_match_tables, return_result
from .match import (
...
from .match import STOP_WORDS as ALL_STOP_WORDS
from .match import WHERE_STOP_WORDS as STOP_WORDS
...
def pattern_token_to_new_style(
    word, tok, all_tokens, schema, table_idxs=[-1], guess=True, col_match=[]
):
    if (
        not guess
...
def word_match(
    ...
    if word == "*":
        return not (
            ...
            or tok.tag_ in ["JJS", "JJR", "RBR", "RBS"]
            or (tok.lower_ in COUNTRYS_DICT and not tok.text.islower())
            or all_star
            and (
                tok.text in ALL_IMPORTANT_PATTERN_TOKENS
                or tok.lemma_ in ALL_IMPORTANT_PATTERN_TOKENS
            )
        )
    if word == "?":  # STOP WORD:
        ...
        if word in [
            ...
            "_TABLE_",
            "PTABLE",
        ]:  # _TABLE_ is for only one table token. TABLE is for at least one token.
            ...
            if col_match:
                if (
                    tok.tag_ in ["JJS", "RBS"]
                    and idx + 1 < len(all_tokens)
                    and tok.lemma_ == all_tokens[idx + 1].lemma_
                ):
                    return False
                if word == "COL":
                    return True
                if (word == "PCOL" and not table_match) or word in ["BCOL", "TCOL"]:
                    ...
                    for ccc in col_match[0]:
                        if schema.column_types[ccc] == "boolean":
                            bcol = True
                            break
                    if (bcol and word == "BCOL") or (
                        not bcol and word in ["PCOL", "TCOL"]
                    ):
                        return True
        elif word == "NOT":
            ...
            if (word == "SJJS" and tok.tag_ not in ["JJS", "RBS"]) or (
                word != "SJJS" and tok.tag_ in ["JJS", "RBS"]
            ):
                return False
            if (
            ...
            if (word == "JJS" and tok.tag_ not in ["JJS", "RBS"]) or (
                word != "JJS" and tok.tag_ in ["JJS", "RBS"]
            ):
                return False
            if word == "GR":
                if (
            ...
            return not (table_match or col_match)
        elif word == "WP" and tok.text in {
            "where",
            "because",
            "who",
            "while",
        ...
        word == tok.text
        or word == tok.lemma_
        or lstem.stem(tok.text) == word
        or (tok.lemma_.endswith("ly") and tok.lemma_[:-2] == word)
    ):
        return True
    return False
        ...
        if word_match(
            ...
            if p_word in [
                ...
                while True:
                    ...
                    if (
                        i >= len(table_match)
                        or (p_word in ["DB", "$DB"] and db_match[i] != db_match[i - 1])
                        or not word_match(
                            p_word,
                            tokens[i],
                            table_match[i],
...
def pattern_match(
    ...
    def can_delete(i, sentence_ts):
        ...
        return not (
            i > 0
            and i < len(sentence_ts.tokens) - 1
            and sentence_ts.tokens[i - 1].text == "'"
            and sentence_ts.tokens[i + 1].text == "'"
        )
    ...
    lss:
        useless now
    pattern_idx:
        return function index. also is the pattern index. when it is -1,
        return the first function.
    return_special_idx:
        return the token idx responding to the pattern token idx.
        (input pattern token idx)
    """
    i = 0
    negative = False
    pattern_for_ml = []
    if pattern[0] == "@":
        for j, pw in enumerate(pattern):
            if pw in ["@"]:
                continue
            match, pw_idx = global_match(
                pw, sentence_ts.tokens, table_match, col_match, entt, db_match, schema
            )
            if not match:
        ...
        for j, pw in enumerate(pattern):
            ...
            if pw[0] == "-":
                if (
                    pw[1:] == sentence_ts.tokens[i].lower_
                    or pw[1:] == sentence_ts.tokens[i].lemma_
                ):
                    return None, False, None
                continue

            while can_delete(i, sentence_ts) or (
                sentence_ts.tokens[i].tag_ == "DT"
                and sentence_ts.tokens[i].text not in ["each"]
                and sentence_ts.tokens[i].text not in NEGATIVE_WORDS
                and sentence_ts.tokens[i].text not in DELETE_WORDS
            ):  # delete STOP WORD.
                i += 1
                if i == len(table_match):
                    i -= 1
                ...
                if (
                    pw == sentence_ts.tokens[i].lower_
                    or pw == sentence_ts.tokens[i].lemma_
                ):
                    break
            ...
            while (
                ...
                and (
                    not col_match[i]
                    or (i + 1 < len(col_match) and col_match[i] != col_match[i + 1])
                )
            ):  # delete the table match before col
                i += 1
            ...
            if not word_match(
                ...
                while True:
                    ...
                    if (
                        i >= len(table_match)
                        or (pw in ["DB", "$DB"] and db_match[i] != db_match[i - 1])
                        or not word_match(
                            pw,
                            sentence_ts.tokens[i],
                            table_match[i],
                        ...
                        i -= 1
                        break  # break the while
                    if if_table_col_continue_not_same(pw, i, table_match, col_match):
                        pw in [
                            "TABLE",
                            "COL",
                            "PCOL",
            ...
            i += 1
        if (
            i != len(table_match) and pattern[0] != "@"
        ):  # if the rest words are not STOP WORDS, return None
            for j in range(i, len(table_match), 1):
                if (
                    sentence_ts.tokens[j].text not in STOP_WORDS
                    and sentence_ts.tokens[j].text not in DELETE_WORDS
                ):
                    return None, False, None
                pattern_for_ml.append("#")
    if pattern_idx < 0:
...
def pattern_reconize(
    ...
    for pidx, patterns in enumerate(pattern_toks):
        for p in patterns:
            if (
                p == ["START", "SEARCH", "DATABASE"] and not in_db_match
            ):  # create_db_match
                start_idx = 0
                not_see_punt = True
                while True:
                    ...
                    if (
                        (
                            not_see_punt
                            or not get_punctuation_word(sentence_ts.tokens, start_idx)
                        )
                        and not sentence_ts.tokens[start_idx].text.isupper()
                        and (
                            sentence_ts.tokens[start_idx].lower_ in ALL_STOP_WORDS
                        ...
                        and (
                            ...
                            or start_idx == 0
                            or (
                                not sentence_ts.tokens[start_idx].text.isalpha()
                                and len(sentence_ts.tokens[start_idx].text) == 1
                            )
                        )
                        and (
                            sentence_ts.tokens[start_idx].tag_
                            in [
                                "IN",
                                "DT",
                                "CC",
                            ...
                            or sentence_ts.tokens[start_idx].lemma_
                            in ["be", "have", "me", "do", "-PRON-", "top"]
                            or (
                                not sentence_ts.tokens[start_idx].text.isalpha()
                                and len(sentence_ts.tokens[start_idx].text) == 1
                            )
                        )
                    ):  # (start_idx>1 and sentence_ts.tokens[start_idx-1].lemma_ in ["of"]) or
                        start_idx += 1
                        continue
                    ...
                    if (
                        ...
                        and sentence_ts.tokens[start_idx].text.islower()
                        and (table_match[start_idx] or col_match[start_idx])
                    ):
                        new_res = []
                        new_res_str = []
                        for rss in res:
                ...
                p_fun, negative, p_all_toks = pattern_match(
                    lss,
                    p,
                    pidx,
                    sentence_ts,
...
def get_AWD_column(
    ...
    for sgrsm_word in S_ADJ_WORD_DIRECTION[word]:
        agg_id, col_id = one_word_to_column_match(
            table_idxs,
            sgrsm_word[0],
            cross_table_search=False,
            schema=schema,
            use_concept_match=False,
            allow_list=True,
        )
    ...
    if restrict and len(col_id_list) > 1 and len(r_word_list) > 1:
        ...
        for col in col_id_list:
            for sgrsm_word in S_ADJ_WORD_DIRECTION[word]:
                if sgrsm_word[0] == schema.column_tokens_lemma_str[
                    col
                ] or schema.column_tokens_lemma_str[col].endswith(sgrsm_word[0]):
                    r_word_set.add(sgrsm_word[0])
...
def one_word_to_column_match(
    table_idx,
    select_words,
    cross_table_search,
    schema: Schema_Token,
    use_concept_match=True,
    only_two_match_fuc=False,
    allow_list=True,
):
    agg_id, col_id = schema.one_word_to_tables_column_match(
        table_idx,
        select_words,
        table_idx[0] >= 0,
        cross_table_search=cross_table_search,
        use_concept_match=use_concept_match,
        only_two_match_fuc=only_two_match_fuc,
        allow_list=allow_list,
    )
    return agg_id, col_id


def words_to_column_match(
    table_idx,
    select_words,
    sel_word_idxs,
    available_idxs,
    ...
    select_word_all = reorder_col_availabe_word(
        select_words, sel_word_idxs, available_idxs, start
    )
...
def one_col_match(
    ...
    for word, t, o_col_idx, ii in zip(col, col_table, o_col_idxs, range(len(col))):
        if (
            ...
            and where_words
            and not t
            and col_inner_idx[where_words[-1][1]] == col_inner_idx[ii]
        ):
            where_words.append([word, ii])
        elif not meet_each and t:
            if len(t) >= 1 and t != table_idx:  # Update the table_idx
                if of_for_structure_in_col(
                    a_col_idx, q_sql.question_tokens
            ...
            not meet_each
            and schema.column_contain_word(word)
            or word in AGG_WORDS
            or word in INFORMATION_WORDS
        ):
            if (
                ii > 0
                and schema.column_contain_word(word)
                and word in AGG_WORDS
                and col_table[ii - 1]
            ):
    ...
    if table_idx and table_idx[0] >= 0 and meet_each and not col_availabe:
        ...
        for ti in table_idx:
            if "name" in schema.tbl_col_tokens_lemma_str[ti]:
                col_id.append(
                    schema.tbl_col_idx_back[ti][
                        schema.tbl_col_tokens_lemma_str[ti].index("name")
                    ]
                )
            else:
    ...
    if table_idx and table_idx[0] >= 0 and not col_availabe:
        if len(col) == 1:
            for t in table_idx:
                if (
                    col[0] in schema.tbl_col_tokens_lemma_str[t]
                    or col[0] in schema.tbl_col_tokens_text_str[t]
                    or schema.stem(col[0]) in schema.tbl_col_tokens_stem_str[t]
                ):
    ...
    while col_availabe:
        ...
        if col_availabe_idx:
            agg_id, col_id = words_to_column_match(
                table_idx,
                col_availabe,
                col_availabe_idx,
                a_col_idx,
            ...
            agg_id, col_id = one_word_to_column_match(
                table_idx, select_words, cross_table_search, schema
            )

        if agg_id == 0 and select_words in AGG_WORDS:
            if (
                ii < len(original_sent) - 1
                and len(original_sent[ii + 1]) >= 1
                and original_sent[ii + 1][0] in AGG_WORDS
            ):
            ...
            and col_id
            and col_id[0] >= 0
            and schema.column_types[col_id[0]] == "number"
            and col_id[0] not in schema.primaryKey
            and col_id[0] not in schema.primaryKey
            and col_id[0] not in schema.foreignKey
        ):
            agg_id = 4
...
def pattern_word_guess(
    ...
    table_idxs,
    idx,
    show_every_word=False,
):
    final = ""

    if tok.lemma_ in AGG_WORDS:
        if tok.lemma_ == "number" and (
            idx + 1 >= len(all_tokens) or all_tokens[idx + 1].text != "of"
        ):
            pass
    ...
    if (
        ...
        (
            (
                idx == len(all_tokens) - 2
                and all_tokens[-1].text in DELETE_WORDS
                and idx > 0
                and all_tokens[idx - 1].text != "to"
            )
            or idx == len(all_tokens) - 1
        )
        and tok.tag_ not in ["JJS", "JJR", "RBR", "RBS"]
    ):
        tmp = None
        if (tok.tag_ in ["JJS", "RBS"] and not tok.text.endswith("er")) or (
            (
                (idx > 0 and all_tokens[idx - 1].text in ["most", "least"])
                or (
                    idx > 1
                    and all_tokens[idx - 2].text in ["most", "least", "top"]
            ...
            if (
                tmp == "SJJS"
                and idx + 1 < len(all_table_match)
                and all_table_match[idx + 1]
            ):
                tmp = pattern_token_to_new_style(
                    "SJJS",
                    tok,
                    all_tokens,
                    schema,
                    all_table_match[idx + 1],
                    col_match=all_col_match,
                )
            if idx > 0 and all_tokens[idx - 1].text == "least" and tmp != "SJJS":
                tmp = (
                    tmp.replace("GR_", "SM_")
                    if tmp.startswith("GR_")
                    else tmp.replace("SM_", "GR_")
                )
        else:
            tmp = pattern_token_to_new_style(
                "SGRSM", tok, all_tokens, schema, table_idxs, col_match=all_col_match
            )
        if tmp:
            final += "," + tmp
    ...
    if (
        ...
        if tok.lemma_ in ALL_JJS:
            ...
            tok.text in ["more", "most", "less", "least"]
            ...
            if (
                ...
                if idx > 0 and all_tokens[idx - 1].text == "least":
                    ...
                    final += "GR_"
            elif idx > 0 and all_tokens[idx - 1].text == "least":
                final += "GR_"
            else:
                final += "SM_"
            if (
                (tok.tag_ in ["JJS", "RBS"] and not tok.text.endswith("er"))
                or tok.text in ["most", "least"]
                or (
                    idx > 0
                    and all_tokens[idx - 1].text in ["most", "least"]
                    and tok.text in ABSOLUTELY_GRSM_DICT
                )
            ):
                final += "JJS"
            else:
    ...
    if not final:
        final += (
            ",[TAG:" + tok.tag_ + ",WORD:" + tok.text + "(" + tok.lemma_ + ")" + "]"
        )
...
def choose_possible_pattern_token(
    pattern_str,
    tokens,
    idx,
    raw_pattern_tokens,
    ...
    def next_pattern_tok(raw_pattern_tokens, col_match, idx, q_tokens, skip_=False):
        for i in range(idx + 1, len(raw_pattern_tokens)):
            ...
            if skip_ and idx + 1 < len(raw_pattern_tokens):
                return next_pattern_tok(
                    raw_pattern_tokens, col_match, idx + 1, q_tokens, skip_
                )
            if q_tokens[i].text not in [
                "the",
                "a",
                "it",
                "an",
    ...
    for tok in words:
        if tok[0] == "$":
            tok = tok[1:]
        if (tok in STOP_WORDS or tok in {"WP", "WP$"}) and tok not in [
            "and",
            "but",
            "or",
        ]:
            if (
                ...
                return "WP"
            return "?"
        if tok in ALL_IMPORTANT_PATTERN_TOKENS:
            if tok == "COL" and len(words) > 2:
                tok2, col2 = next_pattern_tok(
                    raw_pattern_tokens, col_match, idx, q_tokens
                )
                tok3, col3 = next_pattern_tok(
                    raw_pattern_tokens, col_match, idx, q_tokens, True
                )
                ...
                if (
                    tok2
                    and (tok2.startswith("TABLE,(") or tok2.startswith("ST,("))
                    and ("GR_SJJS" in words or "SM_SJJS" in words)
                    or tok3
                    and "AGG" in tok3
                    and "AGG" in words
                    or tok2
                ...
                tok == "SJJS"
                and ("GR_JJS" in words or "SM_JJS" in words)
                or tok == "SGRSM"
                and ("GR_GRSM" in words or "SM_GRSM" in words)
            ):
                continue
                ...
                tok == "least"
                and idx + 1 < len(raw_pattern_tokens)
                and raw_pattern_tokens[idx + 1][0].startswith("SM_JJS")
            ):
                return "SM_JJS"
...
def pattern_recomand(
    ...
    for i, token in enumerate(sentence_ts.tokens):
        if (
            not get_punctuation_word(sentence_ts.tokens, i)
            and not table_match[i]
            and not col_match[i]
            and not db_match[i]
            and (
                token.text in DELETE_WORDS
                or token.text in [".", "?", ","]
                or (
                    sentence_ts.tokens[i].tag_ == "DT"
                    and sentence_ts.tokens[i].text not in ["each"]
            ...
            and (
                sentence_ts.tokens[i].text.islower()
                or i == 0
                or not sentence_ts.tokens[i].text.isalpha()
            )
            and sentence_ts.tokens[i].text not in ["and", "or"]
        ):  # delete STOP WORD.
        ...
        p_str, p_toks = pattern_word_guess(
            sentence_ts.tokens[i],
            table_match[i],
            col_match[i],
            table_match,
    ...
    if (
        "COL" not in pattern_str
        and "AGG,COL" in full_pattern_str
        and pattern_every_token.count("AGG") == 1
    ):
