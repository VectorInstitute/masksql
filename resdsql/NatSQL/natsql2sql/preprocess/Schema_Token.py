...
from .match import AGG_OPS, AGG_WORDS, COUNTRYS, INFORMATION_WORDS
...
class Schema_Token:
    def __init__(self, _tokenizer, stemmer, table_dict, _concept_word):
            ...
            for col in self.original_table["column_names_original"]:
                if col[0] >= 0:
                    self.original_table["tc_fast"].append(
                        (
                            self.original_table["table_names_original"][col[0]]
                            + "."
                            + col[1]
                        ).lower()
                    )
        ...
        for tctc in self.original_table["column_names"]:
            self.original_table["tc_fast_st"][
                self.original_table["table_names_original"][tctc[0]].lower()
            ].append(tctc[1].lower())
        for i in range(len(self.original_table["table_names_original"])):
            self.original_table["table_names_original"][i] = self.original_table[
                "table_names_original"
            ][i].lower()

        self.column_tokens = [
            _tokenizer.tokenize(col[1]) for col in table_dict["column_names"]
        ]
        self.table_tokens = [
            _tokenizer.tokenize(ta) for ta in table_dict["table_names"]
        ]
        ...
        self.column_tokens_lemma_str = [
            " ".join([tok.lemma_ for tok in col]) for col in self.column_tokens
        ]
        self.column_tokens_lemma_str_tokens = [
            col.split(" ") for col in self.column_tokens_lemma_str
        ]

        self.table_tokens_lemma_str = [
            " ".join([tok.lemma_ for tok in col]) for col in self.table_tokens
        ]
        self.table_tokens_text_str = table_dict["table_names"]
        self.all_table_tokens_lemma_str = set(
            [c for tok in self.table_tokens_lemma_str for c in tok.split(" ")]
        )
        self.all_table_tokens_text_str = set(
            [c for tok in self.table_tokens_text_str for c in tok.split(" ")]
        )
        ...
        self.column_tokens_stem_str = [
            " ".join([stemmer.stem(tok.text) for tok in col])
        ...
        for i in range(len(self.column_tokens_stem_str)):
            for j in range(i + 1, len(self.column_tokens_stem_str), 1):
                if (
                    self.column_tokens_stem_str[i] == self.column_tokens_stem_str[j]
                    and self.column_tokens_text_str[i] != self.column_tokens_text_str[j]
                    and self.column_tokens_lemma_str[i]
                    != self.column_tokens_lemma_str[j]
                ):
                    ...
                    for z in range(i, len(self.column_tokens_stem_str), 1):
                        if self.column_tokens_stem_str[z] == stem_word:
                            self.column_tokens_stem_str[z] = (
                                self.column_tokens_text_str[z]
                            )
        ...
        self.tbl_col_tokens_text_str_ori[-1] = [
            i[1].lower() for i in self.column_names_original
        ]
        ...
        for tid, text, lemma, stem, cid, cor in zip(
            self.column_tokens_table_idx,
            self.column_tokens_text_str,
            self.column_tokens_lemma_str,
            self.column_tokens_stem_str,
            self.tbl_col_idx_back[-1],
            self.tbl_col_tokens_text_str_ori[-1],
        ):
            if tid >= 0:
    ...
    def is_bridge_table(self, table_1, table_2, bridge_table):
        ...
        for net in self.original_table["network"]:
            if (
                len(net[1]) == 3
                and table_1 in net[1]
                and table_2 in net[1]
                and bridge_table in net[1]
            ):
    ...
    def add_lower_data(self, table_dict):
        self.table_names_original_low = [
            t.lower() for t in table_dict["table_names_original"]
        ]
        if "table_column_names_original" in table_dict:
            self.table_column_names_original_low = [
                t[1].lower() for t in table_dict["table_column_names_original"]
            ]
        else:
            tmp = [""] + self.table_names_original_low
            self.table_column_names_original_low = [
                tmp[col[0] + 1] + "." + col[1].lower() if col[0] >= 0 else "*"
                for col in self.column_names_original
            ]
    ...
    def column_contain_word(self, word, table_idx=-1):
        ...
        if isinstance(table_idx, list):
            for t in table_idx:
                if (
                    word in self.table_col_text[t]
                    or word in self.table_col_lemma[t]
                    or word_stem in self.table_col_nltk[t]
                    or word_token in self.table_col_text[t]
                    or word_token in self.table_col_lemma[t]
                ):
                    return True
        elif (
            word in self.table_col_text[table_idx]
            or word in self.table_col_lemma[table_idx]
            or word_stem in self.table_col_nltk[table_idx]
            or word_token in self.table_col_text[table_idx]
            or word_token in self.table_col_lemma[table_idx]
        ):
            return True
        return False
    ...
    def column_concept_contain_word(self, word, table_idx=-1):
        ...
        for table_idx in table_idxs:
            ...
            for w in words:
                if (
                    w in self.table_col_text[table_idx]
                    or w in self.table_col_lemma[table_idx]
                    or self._stemmer.stem(w) in self.table_col_nltk[table_idx]
                ):
                    return True
        return False
    ...
    def one_word_to_tables_column_match(
        self,
        table_idxs,
        word,
        table_in_this_col=False,
        ...
        if word.startswith("#"):
            ...
            match_function = [
                self.one_word_to_column_exact_match,
                self.one_word_to_column_exact_contain_match,
                self.one_word_to_column_agg_match,
                self.one_word_to_column_easy_contain_match,
                self.one_word_to_column_conceptnet_match,
            ]
        ...
        for table_idx in table_idxs:
            if (
                table_in_this_col
                and table_idx >= 0
                and (word == "number" or word in INFORMATION_WORDS)
            ):
        ...
        for table_idx in table_idxs:
            ...
            if self.in_outside_words(word, self.table_tokens_lemma_str[table_idx]):
                new_word = self.replace_to_delete_word(
                    word, self.table_tokens_lemma_str[table_idx], ""
                ).strip()
                new_word = self.replace_to_delete_word(
                    new_word, self.table_tokens_text_str[table_idx], ""
                ).strip()
            elif " | " in self.table_tokens_lemma_str[table_idx]:
                new_word = (
                    self.table_tokens_lemma_str[table_idx].split(" | ")[0] + " " + word
                )
            else:
        ...
        if input_lemma != word:
            for i in table_idxs:
                if input_lemma in self.table_col_lemma[i]:
                    return self.one_word_to_several_column_exact_contain_match(
                        i, input_lemma
                    )
        ...
        if len(table_idxs) < len(self.table_tokens_lemma_str) and cross_table_search:
            ...
            for f in match_function:
                if (
                    not use_concept_match
                    and f == self.one_word_to_column_conceptnet_match
                ):
                    continue
    ...
    def one_word_to_column_exact_contain_match(self, table_idx, word, allow_list=False):
        ...
        if len(re_) == 1:
            return 0, re_[0]
        if re_ and allow_list:
            return 0, re_

        return self.one_word_to_column_easy_contain_match(table_idx, word, easy=False)

    def one_word_to_column_easy_contain_match(
        self, table_idx, word, easy=True, allow_list=False
    ):
        word = self._tokenizer.tokenize(word)
        if len(word) > 1:
            ...
            if (
                easy
            ):  # only part of word in column string and this part is most in all col
                if re_count.count(max(re_count)) == 1:
                    return 0, re_count.index(max(re_count))
            elif max(re_count) == len(word):
                if re_count.count(max(re_count)) == 1:
                    return 0, re_count.index(max(re_count))
        ...
        def agg_in_words(words):
            for i, w in enumerate(words):
                if w in AGG_WORDS:
                    if i + 1 < len(words):
                        return (
                            " ".join(words[0:i]) + " " + " ".join(words[i + 1 :])
                        ).strip(), AGG_OPS[AGG_WORDS.index(w)]
                    return (" ".join(words[0:i])).strip(), AGG_OPS[AGG_WORDS.index(w)]
            return None, 0
