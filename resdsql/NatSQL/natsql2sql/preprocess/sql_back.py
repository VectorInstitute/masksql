...
def num_col_unit_back(col_unit, tables_json):
    ...
    if isinstance(col_unit[1], str):
        ...
        str_ += (
            tables_json["table_names_original"][
                tables_json["column_names_original"][col_unit[1]][0]
            ]
            + "."
            + tables_json["column_names_original"][col_unit[1]][1]
        )
...
def num_val_unit_back(val_unit, tables_json):
    if val_unit[0] and (type(val_unit[2]) == tuple or type(val_unit[2]) == list):
        return (
            num_col_unit_back(val_unit[1], tables_json)
            + " "
            + UNIT_OPS[val_unit[0]]
            + " "
            + num_col_unit_back(val_unit[2], tables_json)
        )
...
def from_string_back_based_idx(from_, tables_json):
    re = " from "
    if (
        len(from_["table_units"]) == 1
        and from_["table_units"][0][0] == "sql"
        and len(from_["table_units"][0]) == 2
    ):
        return " from ( " + sql_back(from_["table_units"][0][1], tables_json) + " )"
    if not from_["conds"] and len(from_["table_units"]) > 1:
            ...
            if not isinstance(conds, str):
                ...
                if t1 not in tables_list and t2 not in tables_list:
                    ...
                    if t1 not in tables_list:
                        re += (
                            " join " + tables_json["table_names_original"][t1] + " on "
                        )
                        tables_list.add(t1)
                    elif t2 not in tables_list:
                        re += (
                            " join " + tables_json["table_names_original"][t2] + " on "
                        )
                        tables_list.add(t2)
                    else:
                        re += " and "
...
def group_string_back_based_idx(group, tables_json):
    ...
    if group:
        ...
        for g in group:
            re += (
                tables_json["table_names_original"][
                    tables_json["column_names_original"][g[1]][0]
                ]
                + "."
                + tables_json["column_names_original"][g[1]][1]
                + " , "
            )
        re = re[:-2]
    return re
    ...
    if sql["intersect"]:
        if type(sql["intersect"]) == dict:
            ...
            sql_str = (
                sql_str
                + " intersect select"
                + select_col_num_val_back(sql["intersect"], table)
            )
    if sql["union"]:
        if type(sql["union"]) == dict:
            ...
            sql_str = (
                sql_str + " union select" + select_col_num_val_back(sql["union"], table)
            )
    if sql["except"]:
...
def return_all_where_col(where):
    ...
    for column in where:
        ...
        if column not in ALL_COND_OPS:
            ...
            if (
                type(where_right) == str
                or type(where_right) == int
                or type(where_right) == float
                or type(where_right) == dict
            ):
