...
#              'like', 'is', 'exists', 'not in', 'not like', 'not between',
#              'join')
...
# extend join on: table_1.*=table_2.* -> join these tables.
# If table_1 == table_2, only join one more table.
...
class Args:
    def __init__(self):
        ...
        self.group_for_exact_match = (
            True  # For get a higher score in exact match in spider benchmark
        )
...
def check_relation(
    ...
    try:
        (k_1, k_2) = -1, -1
        if (
            left_tables and not left_column
        ):  # means the left_column is @.@, so input set it to be None
            left_col = None
        else:
        ...
        if right_col == "*" and k_2 == -1 and k_1 >= 0:
            ...
            for fk in foreign_keys:
                if (
                    fk[0] == k_1
                    and column_names[fk[1]][1].split(".")[0].lower() == right_col
                ):
                    k_2 = fk[1]
                elif (
                    fk[1] == k_1
                    and column_names[fk[0]][1].split(".")[0].lower() == right_col
                ):
                    k_2 = fk[0]
            ...
            for fk in foreign_keys:
                if (
                    fk[0] == k_2
                    and column_names[fk[1]][1].split(".")[0].lower() in left_tables
                ):
                    k_1 = fk[1]
                elif (
                    fk[1] == k_2
                    and column_names[fk[0]][1].split(".")[0].lower() in left_tables
                ):
                    k_1 = fk[0]
            ...
            for fk in foreign_keys:
                if (
                    column_names[fk[0]][1].split(".")[0].lower() == right_col
                    and column_names[fk[1]][1].split(".")[0].lower() == left_col
                ):
                    return fk
                if (
                    column_names[fk[0]][1].split(".")[0].lower() == left_col
                    and column_names[fk[1]][1].split(".")[0].lower() == right_col
                ):
                    return fk
        if (
            [k_1, k_2] in foreign_keys
            or [k_2, k_1] in foreign_keys
            or k_1 >= 0
            and k_2 >= 0
            and (k_1 in primary_keys or k_2 in primary_keys)
            and column_names[k_1][0] != column_names[k_2][0]
            and left_column[0] == 0
            and left_column[1][0] == 0
            and right_column[0] == 0
        ):
            return [k_1, k_2]
    except:
        pass
        ...
        if (
            ...
            if column == "sub":
                ...
                break_idx, next_type = (
                    ...
                    or (sql_type == SQL_SUB and not next_type)
                    else (break_idx, next_type)
                )
                # next_type = SQL_SUBSUB if idx > start_index and not next_type
                # else next_type
                ...
                break_idx, next_type = (
                    (idx, SQL_TOP)
                    if (idx > start_index or break_idx == -1) and not next_type
                    else (break_idx, next_type)
                )
                break
            ...
            if type(column[3]) == list:
                if column[2][1][1] == "@.@":
                    fk = check_relation(
                        table_json["foreign_keys"],
                        table_json["primary_keys"],
                        table_json["table_column_names_original"],
                        None,
                        column[3],
                        table_list,
                    )
                ...
                (table_left, column_left) = (
                    c.lower() for c in column[2][1][1].split(".")
                )
                (table_right, column_right) = (
                    c.lower() for c in column[3][1].split(".")
                )
                if (break_idx != -1 or next_type) and column[1] == 15:
                    continue
                if column[1] == 15 and table_right.lower() not in table_list:  # jion:
                    table_list.append(table_right.lower())
                elif column[1] == 15 and table_right.lower() in table_list:  # jion:
                    continue
                elif (
                    (not args.in_sub_sql and column[1] == 8)
                    or (not args.eq_sub_sql and column[1] == 2 and table_left != "@")
                ) and fk:
                    if break_idx != -1 or next_type:
                        continue
                    if (
                        table_left.lower() not in table_list and table_left != "@"
                    ):  # V1.2
                    ...
                    and column[1] <= 7
                    and not fk
                    and (not (column[2][0] or column[2][1][0] or column[3][0]))
                ):
                    # different column in the same table
                    if break_idx != -1 or next_type or see_sub_sql:
                        continue
                    sql_str += condition_str(
                        column
                    )  # for columnA != columnB in the same table
                    ...
                    "*" in column_right and column[1] == 2 and column[3][0] == 0
                ):  # V1.2
                    # ? = * to join on
                    if break_idx != -1 or next_type or see_sub_sql:
                        continue
                    ...
                    if not order_by_:
                        break_idx, next_type = (
                            (idx, SQL_SUB)
                            if (idx > start_index and not next_type)
                            or (sql_type == SQL_TOP and not next_type)
                            else (break_idx, next_type)
                        )
                        if last_column_type and sql_type > last_column_type:
                            # (and + list) sql_type = 1 = new sub sql, it end 2 and
                            # 3(SQL_SUB, SQL_SUBSUB); sub + list = 2 = new subsub
                            # sql, it end 3(SQL_SUBSUB)
                            break
                        if (
                            last_column_type and sql_type != last_column_type
                        ):  # it will true, when last is sub, and sql_type is
                            # Filter by last if: 1.SQL_SUB meet 'and list sub sql
                            # model' 2. SQL_SUBSUB meet 'and list sub sql model'
                            # 3. SQL_SUBSUB meet 'sub list subsub sql model'
                            # It should True here when: 1. SQL_TOP meet 'sub list
                            # subsub sql model'
                            continue
                        # It should run here when: 1. SQL_TOP meet 'and list sub
                        # sql model'; 2. SQL_SUB meet 'sub list sub sql model'
                        see_sub_sql = True
                        if column[2][1][1] == "@.@":  # V1.2
                            ...
                            column[3][1] = (
                                table_json["table_column_names_original"][fk[1]][1]
                                if column[2][1][1].lower()
                                == table_json["table_column_names_original"][fk[0]][
                                    1
                                ].lower()
                                else table_json["table_column_names_original"][fk[0]][1]
                            )
                        if column[2][0] or column[2][1][0]:
                            having += (
                                val_unit_back(column[2]).lower()
                                + " "
                                + WHERE_OPS[column[1]]
                                + " (@@@) "
                            )
                            ...
                            if (
                                sql_str.strip()
                                and not sql_str.strip().endswith("and")
                                and not sql_str.strip().endswith("or")
                            ):
                                sql_str += " and "
                            sql_str += (
                                column[2][1][1].lower()
                ...
                if column[2][0] or column[2][1][0]:  # having
                    and_or = (
                        sql_dict["where"][idx - 1] + " "
                        if idx > 0 and type(sql_dict["where"][idx - 1]) == str
                        else None
                    )
...
def get_table_network(
    ...
    def create_foreign_key(table_json, table_idx_list, restrict=True):
        ...
        def restrict_check(table_json, pkey):
            if (
                table_json["column_names"][pkey[0][0]][1] == "id"
                or table_json["column_names"][pkey[1][0]][1] == "id"
            ):
                return None
            if (
                table_json["column_types"][pkey[0][0]]
                == table_json["column_types"][pkey[1][0]]
            ):
                            ...
                            if other_fk in fk2:  # jump here:
                                ...
                                for k, fk3 in enumerate(table_json["foreign_keys"]):
                                    ...
                                    if pk_r in fk3:
                                        ...
                                        if (
                                            table_json["column_names"][jump_fk2][0]
                                            == jump_tble
                                        ):
                                            return [
                                                [[pk_l, jump_fk], fk3],
                                                [table_l, jump_tble, table_r],
                                            ]
            return None
        ...
        for pk in pkey:  # same name with primary key to become JOIN ON
            ...
            for i, _o_col, col in zip(
                ...
                if col[0] == other_table and (
                    table_json["link_back"][pk[0]][1]
                    in table_json["same_col_idxs"][table_json["link_back"][i][1]]
                    or table_json["link_back"][i][1]
                    in table_json["same_col_idxs"][table_json["link_back"][pk[0]][1]]
                ):
        ...
        for i, c_n, c_on in zip(
            range(len(table_json["column_names"])),
            table_json["column_names"],
            table_json["column_names_original"],
        ):
            if c_n[0] == table_idx_list[0]:
                for j, c_n2, c_on2 in zip(
                    range(len(table_json["column_names"])),
                    table_json["column_names"],
                    table_json["column_names_original"],
                ):
        ...
        for k in table_json["foreign_keys"]:
            if (
                table_json["column_names"][k[0]][0] in table_idx_list
                and table_json["column_names"][k[0]][0] not in tables
            ):
                pkey.append([k[0], table_json["column_names"][k[0]][0]])
                tables.append(table_json["column_names"][k[0]][0])
            if (
                table_json["column_names"][k[1]][0] in table_idx_list
                and table_json["column_names"][k[1]][0] not in tables
            ):
    ...
    def get_fk_network(table_json, table_list, return_num=1):
        ...
        for t in table_list:
            if isinstance(t, table_With_FK):
                ...
                table_index_list.append(
                    [n.lower() for n in table_json["table_names_original"]].index(
                        t.lower()
                    )
        ...
        for _idx, network in enumerate(table_json["network"]):
            if len(network[1]) == len(table_index_list) or (
                ...
                if (
                    success
                    and network[1][0] in table_index_list
                    and network[1][-1] in table_index_list
                ):
        ...
        if from_table_net and table_fk_list:
            for net in from_table_net:
                for tf in table_fk_list:
                    if (
                        table_json["column_names"][tf.keys[0]][0] in net[1]
                        and table_json["column_names"][tf.keys[1]][0] not in net[1]
                    ) or (
                        table_json["column_names"][tf.keys[1]][0] in net[1]
                        and table_json["column_names"][tf.keys[0]][0] not in net[1]
                    ):
                        net[0].append(tf.keys)
                        net[1].append(
                            [
                                n.lower() for n in table_json["table_names_original"]
                            ].index(tf.table.lower())
                        )
    ...
    def analyze_table_net_with_sq(
        sq, from_table_net, table_index_list, table_json, sql_dict
    ):
        def table_not_in_db_match(sq, t_id, sub_idx, table_json):
            for dbms in sq.db_match[sub_idx]:
                for dbm in dbms:
                    ...
                    if (
                        table_json["column_names"][col_id][0] == t_id
                        and "name" in table_json["column_names"][col_id][1]
                    ):
                        return False
                    if (
                        table_json["table_names"][t_id].split(" | ")[0]
                        in table_json["column_names"][col_id][1]
                    ):
                        return False
            return True
        ...
        def there_is_not_close_column(sq, t_id, sub_idx, sub_idx2, table_json):
            if (
                sub_idx2 + 1 < len(sq.col_match[sub_idx])
                and sq.col_match[sub_idx][sub_idx2 + 1]
            ):
                return False
            if (
                sub_idx2 - 1 > 0
                and sq.col_match[sub_idx][sub_idx2 - 2]
                and sq.sub_sequence_toks[sub_idx][sub_idx2 - 1] == "of"
            ):
            ...
            for t in t_matchs:
                if with_weight:
                    if t != except_t and t in net_tables:
                        return False
                elif t[0] != except_t and t[0] in net_tables:
                    return False
            return True
        ...
        def the_word_is_not_a_column(sq, t_id, sub_idx, sub_idx2, table_json):
            if sq.col_match[sub_idx][sub_idx2]:
                if hasattr(sq, "table_match_weight"):
                    for t, tw in zip(
                        sq.table_match[sub_idx][sub_idx2],
                        sq.table_match_weight[sub_idx][sub_idx2],
                    ):
                        if t == t_id and tw != 1:
                            return False
                else:
        ...
        def use_the_net_col_appear_in_question(table_net, idx1, idx2, sq, table_json):
            ...
            cols1_str = (
                " ".join([table_json["column_names"][col][1] for col in cols1])
            ).split(" ")
            cols2_str = (
                " ".join([table_json["column_names"][col][1] for col in cols2])
            ).split(" ")
        ...
        if sq and len(from_table_net) > 1:
            ...
            for i in range(len(from_table_net) - 1, 0, -1):
                if len(from_table_net[i][1]) > min_table + 1:
                    ...
                    if sort_from_table_net[i][1] == sort_from_table_net[0][1]:
                        del_idxs.add(
                            use_the_net_col_appear_in_question(
                                sort_from_table_net, i, 0, sq, table_json
                            )
                        )
                    ...
                    for i in range(len(from_table_net) - 1, 0, -1):
                        if i != j:
                            ...
                            if sort_from_table_net[i][1] == sort_from_table_net[j][1]:
                                del_idxs.add(
                                    use_the_net_col_appear_in_question(
                                        sort_from_table_net, i, j, sq, table_json
                                    )
                                )
            ...
            for diff, net in zip(differen_list, from_table_net):
                if diff:
                    ...
                    for d in diff:
                        ...
                        if hasattr(sq, "table_match_weight"):
                            for (i, tm), tws in zip(
                                enumerate(sq.table_match), sq.table_match_weight
                            ):
                                for (j, ts), twl in zip(enumerate(tm), tws):
                                    for t, tw in zip(ts, twl):
                                        if (
                                            t == d
                                            and tw > max_d
                                            and the_word_is_not_a_column(
                                                sq, d, i, j, table_json
                                            )
                                            and table_in_original(net[1], ts, d, True)
                                            and table_not_in_db_match(
                                                sq, d, i, table_json
                                            )
                                            and there_is_not_close_column(
                                                sq, d, i, j, table_json
                                            )
                                            and "NOT" not in sq.pattern_tok[i]
                                            and the_table_is_not_a_column_in_sql_dict(
                                                sql_dict, table_json, d
                                            )
                                        ):
                                            max_d = tw
                        else:
                                ...
                                for j, ts in enumerate(tm):
                                    for t in ts:
                                        if (
                                            ...
                                            and table_in_original(net[1], ts, d)
                                            and table_not_in_db_match(
                                                sq, d, i, table_json
                                            )
                                            and there_is_not_close_column(
            ...
            max_index, max_number = max(
                enumerate(differen_weight), key=operator.itemgetter(1)
            )
    ...
    from_table_net, table_fk_list, table_index_list = get_fk_network(
        table_json, table_list, 0
    )

    from_table_net = analyze_table_net_with_sq(
        sq, from_table_net, table_index_list, table_json, sql_dict
    )
    ...
    if not from_table_net and len(table_index_list) == 2:
        if re_single:
            return create_foreign_key(
                table_json, table_index_list, False
            ), table_fk_list
        return [create_foreign_key(table_json, table_index_list, False)], table_fk_list
    if not from_table_net and len(table_index_list) == 3:
        from_table_net1, table_fk_list1, table_index_list1 = get_fk_network(
            table_json, [table_list[0], table_list[1]]
        )
        from_table_net2, table_fk_list2, table_index_list2 = get_fk_network(
            table_json, [table_list[0], table_list[2]]
        )
        from_table_net3, table_fk_list3, table_index_list3 = get_fk_network(
            table_json, [table_list[1], table_list[2]]
        )
        net1 = None
        net2 = None
        net3 = None
        if not from_table_net1 and not from_table_net2 and from_table_net3:
            net1 = create_foreign_key(
                table_json, [table_index_list[0], table_index_list[1]]
            )
            net2 = create_foreign_key(
                table_json, [table_index_list[0], table_index_list[2]]
            )
        ...
        if net1 and not net2:
            ...
            if (
                net1[0][0][0] in table_json["primary_keys"]
                or net1[0][0][1] in table_json["primary_keys"]
            ):
                net2 = net1
            net3[0].append(net2[0][0])
            net3[1].append(net2[1][0] if net2[1][0] not in net3[1] else net2[1][1])
    ...
    if from_table_net and group_list:
        table_list = [
            table_json["column_names"][table_json["tc_fast"].index(col)][0]
        ...
        if len(table_list) >= 1:
            for i, net in enumerate(from_table_net):
                join_cols = [
                    ...
                    for cols in net[0]
                    for c in cols
                    if c in table_json["primary_keys"]
                    and table_json["column_names"][c][0] in table_list
                ]
                if join_cols:
                    idx = i
                    break
...
def maybe_order_by(
    ...
    if (
        (sql_dict["where"][idx][3][0] == 1 or sql_dict["where"][idx][3][0] == 2)
        and sql_dict["where"][idx][2][1][1] == sql_dict["where"][idx][3][1]
        and sql_dict["where"][idx][1] == 2
    ):  # sql_dict['where'][idx][1] == 2 means '='
        break_idx, next_type = (
            (idx, SQL_SUB)
            if (idx > start_index and not next_sql_type)
            or (sql_type_now == SQL_TOP and not next_sql_type)
            else (idx, next_sql_type)
        )
        (
            break_idx,
            table_list,
        ...
        if (not next_sql and len(sql_dict["where"]) == idx + 1) or (
            next_sql and break_idx <= idx + 2
        ):  # the end of the sql or sub sql.
...
def is_orderby_for_subquery(sql_dict):
    ...
    if not sql_dict["limit"] or not sql_dict["orderBy"]:
        return False
    return bool(is_there_subquery(sql_dict["where"]))
...
def orderby_to_subquery(sql_dict, tb_list):
    ...
    if is_orderby_for_subquery(sql_dict):
        orderby_sql, table_list, agg_in_order = create_order_by(
            sql_dict["orderBy"], sql_dict["limit"]
        )
        for t in table_list:
            if t not in tb_list:
                tb_list.append(t)
...
def primary_keys(table_json, table_id, pk_only=False):
    ...
    for key, _col in enumerate(table_json["column_names"]):
        if table_json["column_names"][key][0] == table_id:
            ...
            col_n = col_n.replace(
                table_json["table_names"][table_id].lower(), ""
            ).strip()
            col_n = col_n.replace(
                table_json["table_names_original"][table_id].lower(), ""
            ).strip()
            if col_n in ["id", "ids"]:
                return key
    return -1
...
def infer_at_col(column, table_list, table_json):
    ...
    def find_fk_cols(l_table, r_table, table_json, r_col=None):
        if r_col:
            for fk in table_json["foreign_keys"]:
                ...
                if r_col == fk[1] and table_json["column_names"][fk[0]][0] == l_table:
                    return True, fk[0], r_col
        else:
            for fk in table_json["foreign_keys"]:
                if (
                    table_json["column_names"][fk[0]][0] == r_table
                    and table_json["column_names"][fk[1]][0] == l_table
                ):
                    return True, fk[1], fk[0]
                if (
                    table_json["column_names"][fk[1]][0] == r_table
                    and table_json["column_names"][fk[0]][0] == l_table
                ):
                    return True, fk[0], fk[1]
            for net in table_json["network"]:
                if (
                    len(net[1]) == 3
                    and r_table in net[1]
                    and l_table in net[1]
                    and r_table not in table_json["bridge_table"]
                    and l_table not in table_json["bridge_table"]
                    and contain_bridge_table(net[1], table_json["bridge_table"])
                ):
                    for fk in net[0]:
                        if (
                            table_json["column_names"][fk[0]][0] == l_table
                            and table_json["column_names"][fk[1]][0]
                            in table_json["bridge_table"]
                        ):
                            return True, fk[0], fk[1]
                        if (
                            table_json["column_names"][fk[1]][0] == l_table
                            and table_json["column_names"][fk[0]][0]
                            in table_json["bridge_table"]
                        ):
                            return True, fk[1], fk[0]
        return False, 0, 0
    ...
    def find_col(l_tables, r_table, r_col, table_json):
            ...
            for lt in l_tables[1]:
                for i, col_o, col in zip(
                    ...
                    if col_o[0] == lt and (
                        col_o[1].lower() == r_col
                        or (
                            col[1] == table_json["column_names"][r_col_idx][1]
                            and col[1] not in ["name", "names", "id", "ids"]
                        )
                        or col[1].lower()
                        == (
                            table_json["table_names"][
                                table_json["column_names"][r_col_idx][0]
                            ]
                            + " "
                            + table_json["column_names"][r_col_idx][1]
                        ).lower()
                        or (table_json["table_names"][col[0]] + " " + col[1]).lower()
                        == table_json["column_names"][r_col_idx][1].lower()
                    ):
                        return i, r_col_idx
        ...
        for j, rcol_o, rcol in zip(
            ...
            for lt in l_tables[1]:
                for i, col_o, col in zip(
                    ...
                    if col_o[0] == lt and (
                        (
                            col_o[1].lower() == rcol_o[1].lower()
                            and col_o[1].lower() not in ["name", "names", "id", "ids"]
                        )
                        or (
                            col[1].lower() == rcol[1].lower()
                            and col[1].lower() not in ["name", "names", "id", "ids"]
                        )
                        or (
                            (table_json["table_names"][col[0]] + " " + col[1]).lower()
                            == rcol[1].lower()
                            or (
                                table_json["table_names"][rcol[0]] + " " + rcol[1]
                            ).lower()
                            == col[1].lower()
                        )
                    ):
        ...
        if result:
            for r in result:
                # if table_json['column_names_original'][r[0]][1].lower()
                # not in ["name","id"] and
                # table_json['column_names'][r[0]][1].lower()
                # not in ["name","id"]:
                if (
                    r[0] in table_json["primary_keys"]
                    or r[1] in table_json["primary_keys"]
                ):
                    return r[0], r[1]
            for r in result:
                if (
                    table_json["column_names_original"][r[0]][1].lower()
                    not in ["name", "id"]
                    and table_json["column_names_original"][r[0]][1]
                    == table_json["column_names_original"][r[1]][1]
                ):
                    return r[0], r[1]
    ...
    table_right_idx = [n.lower() for n in table_json["table_names_original"]].index(
        table_right
    )
    if table_right in table_list:  # It will be the same column for both side
        ...
        if from_table_net and table_right_idx in from_table_net[1]:
            ...
            if col_right == "*":
                primarykey = primary_keys(table_json, table_right_idx)
                if primarykey >= 0:
                    column[3][1] = table_json["table_column_names_original"][
                        primarykey
                    ][1]
            column[2][1][1] = column[3][1]
        elif from_table_net:
            # There isn't same tables, so look for the foreign key relations.
            col_left_idx, col_right_idx = find_col(
                from_table_net, table_right_idx, col_right, table_json
            )
        ...
        # There isn't same tables, so look for the foreign key relations.
        table_left_idx = [n.lower() for n in table_json["table_names_original"]].index(
            table_list[0].lower()
        )
        col_left_idx, col_right_idx = find_col(
            [[], [table_left_idx]], table_right_idx, col_right, table_json
        )
        column[3][1] = table_json["table_column_names_original"][col_right_idx][1]
        column[2][1][1] = table_json["table_column_names_original"][col_left_idx][1]
    return column
...
def intersect_where_order(wheres, top_select_table_list):
    ...
    if (
        ...
        if (
            left[0] not in top_select_table_list
            and right[0] in top_select_table_list
            and left[1] == right[1]
        ):
            (wheres[0], wheres[2]) = (wheres[2], wheres[0])
    return wheres
...
def intersect_inference(wheres, sq):
    def replace_str_for_inference(wheres, where_idx_1, where_idx_2, sq, middle=0):
        if (
            sq
            and wheres[where_idx_1][1] in [4, 6, 3, 5]
            and wheres[where_idx_2][1] in [4, 6, 3, 5]
            and isinstance(wheres[where_idx_1][3], str)
            and isinstance(wheres[where_idx_2][3], str)
        ):
            num_count = 0
            ...
            if num_count == 2:
                for toks, sub_q in zip(sq.pattern_tok, sq.sub_sequence_toks):
                    for tok, q_tok in zip(toks, sub_q):
                        if tok in ["NUM", "YEAR"]:
                            wheres[where_idx_1 if num_count == 2 else where_idx_2][
                                3
                            ] = str2num(q_tok)
                            num_count -= 1
        middle = int((where_idx_1 + where_idx_2) / 2) if not middle else middle
        if (
            wheres[where_idx_1][1] == 2
            and wheres[where_idx_2][1] == 2
            and not (
                wheres[where_idx_2][2][1][0] == 3 and wheres[where_idx_1][2][1][0] == 0
            )
            or ".*" in wheres[where_idx_1][2][1][1]
            and ".*" in wheres[where_idx_2][2][1][1]
            or (
                wheres[where_idx_2][2][1][0] in [1, 2, 4, 5, 6]
                and wheres[where_idx_1][1] not in [8, 12, 15]
            )
            or (wheres[where_idx_2][2][1][0] and wheres[where_idx_1][2][1][0])
            or wheres[where_idx_1][1] == 4
            and wheres[where_idx_2][1] == 3
            and isinstance(wheres[where_idx_1][3], str)
            and isinstance(wheres[where_idx_2][3], str)
            and wheres[where_idx_1][2][1] == wheres[where_idx_2][2][1]
            or wheres[where_idx_1][1] == 6
            and wheres[where_idx_2][1] == 5
            and isinstance(wheres[where_idx_1][3], str)
            and isinstance(wheres[where_idx_2][3], str)
            and wheres[where_idx_1][2][1] == wheres[where_idx_2][2][1]
            or wheres[where_idx_1][1] == 6
            ...
            and isinstance(wheres[where_idx_2][3], str)
            and wheres[where_idx_1][2][1] == wheres[where_idx_2][2][1]
            or wheres[where_idx_1][1] == 3
            and wheres[where_idx_2][1] == 4
            and isinstance(wheres[where_idx_1][3], list)
            and isinstance(wheres[where_idx_2][3], list)
            and wheres[where_idx_1][3][1].split(".")[1]
            == wheres[where_idx_2][3][1].split(".")[1]
            or wheres[where_idx_1][1] == 4
            and wheres[where_idx_2][1] == 3
            and isinstance(wheres[where_idx_1][3], list)
            and isinstance(wheres[where_idx_2][3], list)
            and wheres[where_idx_1][3][1].split(".")[1]
            == wheres[where_idx_2][3][1].split(".")[1]
            or wheres[where_idx_1][1] not in [8, 12, 15]
            and wheres[where_idx_2][1] == 15
            and wheres[0][1] not in [8, 12, 15]
            or wheres[where_idx_1][1] == 3
            and wheres[where_idx_2][1] == 4
            and (wheres[where_idx_1][3]) >= (wheres[where_idx_2][3])
            and wheres[where_idx_1][2][1][1] == wheres[where_idx_2][2][1][1]
            or wheres[where_idx_1][1] == 4
            and wheres[where_idx_2][1] == 3
            and (wheres[where_idx_1][3]) <= (wheres[where_idx_2][3])
            and wheres[where_idx_1][2][1][1] == wheres[where_idx_2][2][1][1]
            or wheres[where_idx_1][1] == 5
            and wheres[where_idx_2][1] == 6
            and (wheres[where_idx_1][3]) > (wheres[where_idx_2][3])
            and wheres[where_idx_1][2][1][1] == wheres[where_idx_2][2][1][1]
            or wheres[where_idx_1][1] == 6
            and wheres[where_idx_2][1] == 5
    ...
    try:
        if (
            (len(wheres) == 3 and wheres[1] == "and")
            or (len(wheres) == 5 and wheres[1] == "and" and wheres[3] == "and")
        ) and (
            wheres[0][2][1][1] == wheres[2][2][1][1]
            or (".*" in wheres[0][2][1][1] and ".*" in wheres[2][2][1][1])
            or wheres[2][2][1][0] != 0
        ):
        ...
        if (len(wheres) == 5 and wheres[1] == "and" and wheres[3] == "and") and (
            ...
            or (".*" in wheres[4][2][1][1] and ".*" in wheres[2][2][1][1])
            or wheres[4][2][1][0] != 0
        ):
            if (
                wheres[4][2] == wheres[2][2]
                and wheres[4][1] == wheres[2][1]
                and wheres[0][1] == 2
            ):
            ...
            and not (
                wheres[0][1] == wheres[4][1]
                and wheres[0][1] in [8, 12]
                and wheres[0][3][1] != wheres[4][3][1]
            )
            and (
                (
                    wheres[0][2][1][1] == wheres[4][2][1][1]
                    and wheres[2][2][1][1] == wheres[6][2][1][1]
                )
                or (
                    wheres[2][2][1][1] == wheres[4][2][1][1]
                    and wheres[0][2][1][1] == wheres[6][2][1][1]
                )
            )
        ):
...
def union_inference(wheres, sq, table_json, top_select_table_list):
    try:
        if len(wheres) == 5 and wheres[1] == "or" and wheres[3] == "and":
            ...
            and (
                (
                    wheres[0][2][1] == wheres[4][2][1]
                    and wheres[2][2][1] == wheres[6][2][1]
                )
                or (
                    wheres[0][2][1] == wheres[6][2][1]
                    and wheres[2][2][1] == wheres[4][2][1]
                )
            )
            or len(wheres) == 5
            and wheres[1] == "and"
            ...
            and not (
                wheres[0][1] == 15
                and wheres[2][2][1][1].split(".")[0] == wheres[4][2][1][1].split(".")[0]
            )
        ):
            wheres[3] = "union_"
            ...
            and (
                (
                    (wheres[0][2][1][0] != 0 or wheres[2][2][1][0] != 0)
                    and (
                        wheres[0][2][1][1] != wheres[2][2][1][1]
                        or ".*" in wheres[0][2][1][1]
                        or ".*" in wheres[2][2][1][1]
                    )
                )
                or wheres[2][1] == 15
            )
            ...
            and not (
                wheres[2][2][1][1] == "@.@"
                and wheres[0][1] == wheres[2][1]
                and wheres[0][3][1].split(".")[0] == wheres[2][3][1].split(".")[0]
            )
        ):
            wheres[1] = "union_"
        elif (
            ...
            if right[0] not in top_select_table_list and left[0] != right[0]:
                ...
                from_table_net, table_fk_list = get_table_network(
                    table_json, top_select_table_list, None
                )
                top_select_table_list = []
                for tb in from_table_net:
                    if tb:
                        top_select_table_list.append(
                            table_json["table_names_original"][tb[0]]
                        )
...
def except_inference(wheres, sq):
    try:
        ...
        if (len(wheres) == 5 and wheres[1] == "and" and wheres[3] == "and") and wheres[
            ...
            if (
                (wheres[0][1] in [8, 12] and wheres[2][1] == 2)
                or (wheres[0][1] == 2 and wheres[2][1] == 2)
                or wheres[2][2][1][0]
            ):
                wheres[3] = "except_"
                wheres[4][1] = 2
        elif (
            (len(wheres) == 5 and wheres[1] == "and" and wheres[3] == "and")
            and wheres[0][1] == 2
            and wheres[2][1] == 7
        ):
            wheres[1] = "except_"
            ...
            (len(wheres) == 5 and wheres[1] == "and" and wheres[3] == "or")
            and wheres[0][1] not in [8, 12, 15]
            and (wheres[2][1] == 7 or wheres[4][1] == 7)
            and (
                wheres[0][2][1][1].split(".")[0] != wheres[2][2][1][1].split(".")[0]
                or wheres[0][2][1][1].split(".")[0] != wheres[4][2][1][1].split(".")[0]
            )
        ):
            wheres[1] = "except_"
            wheres[2][1] = 2
...
def infer_group_for_exact_match(
    group_list,
    table_json,
    table_list,
    num_select,
    ...
    if len(group_list) > 1:
        ...
        if len(select_no_agg_tables) == 1:
            ...
            for col in group_list:
                if (
                    "name" in col
                    and "firstname" not in col
                    and "first_name" not in col
                    and "lastname" not in col
                    and "last_name" not in col
                    and "forename" not in col
                    and "surname" not in col
                ):
                    return [col]
        elif len(select_no_agg_tables) >= 2 and len(table_list) >= 2:
            from_table_net, table_fk_list = get_table_network(
                table_json, table_list, None, group_list=group_list
            )
            if from_table_net:
                return filter_PK(
                    from_table_net, agg_tables, table_json, select_no_agg_tables
                )
    elif (
        sq
        and num_select == 1
        and len(table_list) >= 2
        and group_list
        and "lname" not in group_list[0]
        and "fname" not in group_list[0]
        and "lastname" not in group_list[0]
        and "firstname" not in group_list[0]
        and "l_name" not in group_list[0]
        and "f_name" not in group_list[0]
        and "last_name" not in group_list[0]
        and "first_name" not in group_list[0]
    ):
        if (
            sq.sub_sequence_type.count(1) == 1
            and 0 not in sq.sub_sequence_type
            and sq.sub_sequence_type[0] == 1
            and sq.pattern_tok[0][-1] in ["SC", "STC", "COL", "BCOL", "TABLE-COL"]
            or len(sq.sub_sequence_type) == 1
            and "TABLE" not in sq.pattern_tok[0]
            and "ST" not in sq.pattern_tok[0]
        ):
            pass
        else:
            from_table_net, table_fk_list = get_table_network(
...
def filter_PK(from_table_net, agg_tables, table_json, select_no_agg_tables):
    ...
    if agg_tables:
        for fk in from_table_net[0]:
            if (
                fk[0] in table_json["primary_keys"]
                and table_json["column_names"][fk[0]][0] in agg_tables[1]
            ) or (
                fk[1] in table_json["primary_keys"]
                and table_json["column_names"][fk[1]][0] in agg_tables[1]
            ):
                continue
            if (
                not re_col
                and table_json["column_names"][fk[0]][0] in select_no_agg_tables
            ):
                re_col = fk[0]
            elif (
                not re_col
                and table_json["column_names"][fk[1]][0] in select_no_agg_tables
            ):
                re_col = fk[1]
            if (
                table_json["column_names"][fk[0]][0] in select_no_agg_tables
                or table_json["column_names"][fk[1]][0] in select_no_agg_tables
            ):
                ...
                if (
                    ...
                    re_col = fk[1]
    return (
        [table_json["tc_fast"][re_col]]
        if re_col
        else [table_json["tc_fast"][from_table_net[0][0][0]]]
    )
...
def get_agg_tables(sql_dict, table_json):
    ...
    for sel in sql_dict["select"][1]:
        if sel[1][1][0] or sel[0]:
            ...
            agg_tables[1].append(
                table_json["column_names"][
                    table_json["tc_fast"].index(sel[1][1][1].lower())
                ][0]
            )
    sub_query = False
    for w in sql_dict["where"]:
        if type(w) == list and w[2][1][0]:
            ...
            agg_tables[1].append(
                table_json["column_names"][
                    table_json["tc_fast"].index(w[2][1][1].lower())
                ][0]
            )
    ...
    if not sub_query and sql_dict["orderBy"] and sql_dict["orderBy"][1][0][1][0]:
        ...
        agg_tables[1].append(
            table_json["column_names"][
                table_json["tc_fast"].index(sql_dict["orderBy"][1][0][1][1].lower())
            ][0]
        )
    return agg_tables


def group_back_because_eva_bug_in_spider(
    groupby_list, table_json, agg_tables, or_groupby_list, from_table_net
):
    if len(groupby_list) == 1:
        ...
        for t_name, t_idx in zip(agg_tables[0], agg_tables[1]):
            if g_table == t_name:
                return groupby_list
            for fk in table_json["foreign_keys"]:
                if g_col_idx in fk and (
                    (
                        fk[0] == g_col_idx
                        and table_json["column_names"][fk[1]][0] == t_idx
                    )
                    or (
                        fk[1] == g_col_idx
                        and table_json["column_names"][fk[0]][0] == t_idx
                    )
                ):
                    if (
                        or_groupby_list
                        and len(or_groupby_list) == 1
                        and (
                            (fk[0] == g_col_idx and fk[1] in table_json["primary_keys"])
                            or (
                                fk[0] != g_col_idx
                                and fk[0] in table_json["primary_keys"]
                            )
                        )
                    ):
                        return or_groupby_list
                    return (
                        [table_json["tc_fast"][fk[1]]]
                        if fk[0] == g_col_idx
                        else [table_json["tc_fast"][fk[0]]]
                    )
            if (
                g_col not in ["id", "ids", "name", "names"]
                and g_table != t_name
                and t_name + "." + g_col in table_json["tc_fast"]
                and table_json["tc_fast"].index(t_name + "." + g_col) in net_fks
            ):
                return [t_name + "." + g_col]
    return groupby_list


def infer_IUE_select_col(
    sql_dict, table_json, break_idx, top_sql_list, top_select_table_list
):
    ...
    if top_select_t == start_table:
        return " " + sql_dict["where"][break_idx][:-1] + " " + top_sql_list[
            0
        ], top_select_table_list
    ...
    if len(sql_dict["select"][1]) > 1:
        if (
            table_json["table_names_original"][new_t_idx].lower()
            not in top_select_table_list
        ):
            top_select_table_list += [
                table_json["table_names_original"][new_t_idx].lower()
            ]
    ...
    if other_fk > 0:
        ...
        ).lower(), [table_json["table_names_original"][new_t_idx].lower()]

    # find same name column
    final_choice = None
    for o_col, col in zip(
        table_json["column_names_original"], table_json["column_names"]
    ):
        if o_col[0] == new_t_idx and (
            o_col[1] == table_json["column_names_original"][old_c_idx][1]
            or col[1] == table_json["column_names"][old_c_idx][1]
            or col[1]
            == table_json["table_names"][table_json["column_names"][old_c_idx][0]]
            + " "
            + table_json["column_names"][old_c_idx][1]
        ):
            if o_col[1] == "*":
                return " " + sql_dict["where"][break_idx][:-1] + " " + top_sql_list[
                    0
            ...
            final_choice = (
                (
                    sub_sql
                    + table_json["table_names_original"][new_t_idx]
                    + "."
                    + o_col[1]
                ).lower(),
                [table_json["table_names_original"][new_t_idx].lower()],
...
def inference_sql(
    sql_dict, table_json, args, join_on_label=None, sq=None, return_with_join_on=False
):
    re_generate, re_sql, _, __ = search_all_join_on(
        sql_dict, table_json, args, join_on_label, sq, True
    )
    if re_generate:
        if re_generate == 1:
            ...
            args_ = args
        re_generate, re_sql, _, __ = search_all_join_on(
            re_sql, table_json, args_, join_on_label, sq, False
        )
    ...
    for fts in from_table_netss:
        ...
        for ft in fts:
            str_list.append(
                create_from_table(
                    ft,
                    table_json["table_names_original"],
                    table_json["table_column_names_original"],
                    [],
                )
            )
...
def fk_replace_IUE(
    ...
    if len(sql_dict["select"][1]) == 1 and (
        (break_idx == 0 and len(sql_dict["where"]) > 3)
        or (break_idx > 0 and len(sql_dict["where"]) > break_idx + 1)
    ):
        if break_idx == 0:
            next_con = 3
        else:
        ...
        for i, col in enumerate(table_json["table_column_names_original"]):
            if col[1].lower() == col_name:
                for fk in table_json["foreign_keys"]:
                    if (
                        i == fk[0]
                        and table_json["column_names_original"][fk[1]][0]
                        == table_json["column_names_original"][col_idx2][0]
                    ) or (
                        i == fk[1]
                        and table_json["column_names_original"][fk[0]][0]
                        == table_json["column_names_original"][col_idx2][0]
                    ):
                        col_idx = i
                        break
                if (
                    col_idx2
                    and not col_idx
                    and table_json["column_names"][i][1].lower()
                    not in ["name", "names", "id", "ids"]
                    and break_idx > 0
                    and break_idx + 1 < len(sql_dict["where"])
                    and isinstance(sql_dict["where"][break_idx - 1], list)
                    and isinstance(sql_dict["where"][break_idx + 1], list)
                    and sql_dict["where"][break_idx - 1][2][1][1].split(".")[0]
                    != sql_dict["where"][break_idx + 1][2][1][1].split(".")[0]
                ):
                    fk_or_pk = False
                    ...
                    if not fk_or_pk:
                        ...
                        for j, col in enumerate(table_json["column_names_original"]):
                            if (
                                col[0]
                                == table_json["column_names_original"][col_idx2][0]
                                and col[1].lower() == pure_col_name
                                and table_json["column_names"][i][1].lower()
                                == table_json["column_names"][j][1].lower()
                            ):
                                col_idx = i
                                break
                elif table_json["column_names"][i][1].lower() in [
                    "name",
                    "names",
                    "id",
                    "ids",
                    ...
                    if not fk_or_pk:
                        pure_col_name = (
                            table_json["table_names"][table_json["column_names"][i][0]]
                            + " "
                        ...
                        for _j, col in enumerate(table_json["column_names"]):
                            if col[0] == table_json["column_names"][col_idx2][0] and (
                                ...
                                == (
                                    table_json["table_names"][col[0]] + " " + col[1]
                                ).lower()
                            ):
                                col_idx = i
                                break
...
def search_bcol(sq, table, where, sql_dict, where_i):
                    ...
                    for col in sq.col_match[i_t][j]:
                        ...
                        if col_id == where_col_idx:
                            if (
                                ...
                                or (
                                    where_i > 1
                                    and type(sql_dict["where"][where_i - 2]) == list
                                    and sql_dict["where"][where_i - 2][1] == 12
                                )
                                or (
                                    where_i > 3
                                    and type(sql_dict["where"][where_i - 4]) == list
                                    and sql_dict["where"][where_i - 4][1] == 12
                                )
                            ):
    ...
    if where[3] == '"terminal"' and table["data_samples"][where_col_idx]:
        where[3] = (
            ...
            if type(table["data_samples"][where_col_idx][0]) == str
            else table["data_samples"][where_col_idx][0]
        )
...
def search_db(sq, table, where, used_value, sq_sub_idx=-1):
    ...
    for v in table["data_samples"][where_col_idx]:
        if (
            where[3] == '"terminal"'
            and " " + str(v).lower() + " " in " " + sq.question_or.lower() + " "
        ):
            vl = str(v).lower()
                ...
                for i, sts in enumerate(sq.sub_sequence_toks):
                    ...
                    if where[3] == '"terminal"':
                        for j, st in enumerate(sts):
                            if len(st) >= 3 and (
                                (
                                    [i, j] not in used_value
                                    and " " not in v
                                    and (
                                        st.lower().startswith(vl)
                                        or vl.startswith(st.lower())
                                    )
                                )
                                or (
                                    " " in v
                                    and [i, j] not in used_value
                                    and vl.startswith(st.lower() + " ")
                                    and "DB" in sq.pattern_tok[i][j]
                                )
                            ):
                                where[3] = "'" + v + "'"
                                used_value.append([i, j])
                            ...
                            if (
                                ...
                                and j + 1 < len(sts)
                                and [i, j] not in used_value
                                and [i, j + 1] not in used_value
                                and len(sts[j + 1]) > 2
                                and (
                                    vl.startswith(st.lower() + " " + sts[j + 1] + " ")
                                    or vl.startswith(
                                        st.lower() + " " + sts[j + 1][:-1] + " "
                                    )
                                    or vl == st.lower() + " " + sts[j + 1]
                                    or vl == st.lower() + " " + sts[j + 1][:-1]
                                )
                            ):
                                where[3] = "'" + v + "'"
                                used_value.append([i, j])
...
def no_more_num_cond(wheres, i, table_json):
    for w_i, where in enumerate(wheres):
        if (
            ...
            and (
                (
                    where[2][1][1] in table_json["tc_fast"]
                    and table_json["column_types"][
                        table_json["tc_fast"].index(where[2][1][1])
                    ]
                    in ["number", "year", "time"]
                )
                or (
                    type(where[2][1][1]) == int
                    and table_json["column_types"][where[2][1][1]]
                    in ["number", "year", "time"]
                )
            )
        ):
...
def generate_where_values(
    i, pts, sq, used_value, patterns, where, table_json, sql_dict, where_idx
):
    def add_one_value(pt, pt_target, pts, sq, used_value, i, j):
        ...
        if (
            pt == pt_target
            and j + 1 < len(pts)
            and pts[j + 1] == pt_target
            and (pt_target != "DB" or sq.db_match[i][j] == sq.db_match[i][j + 1])
        ):
            re_num += " " + sq.sub_sequence_toks[i][j + 1]
            used_value.append([i, j + 1])
            if (
                j + 2 < len(pts)
                and pts[j + 2] == pt_target
                and (
                    pt_target != "DB" or sq.db_match[i][j + 2] == sq.db_match[i][j + 1]
                )
            ):
                ...
                if (
                    j + 3 < len(pts)
                    and pts[j + 3] == pt_target
                    and (
                        pt_target != "DB"
                        or sq.db_match[i][j + 3] == sq.db_match[i][j + 2]
                    )
                ):
                    ...
                    if (
                        j + 4 < len(pts)
                        and pts[j + 4] == pt_target
                        and (
                            pt_target != "DB"
                            or sq.db_match[i][j + 4] == sq.db_match[i][j + 3]
                        )
                    ):
                        ...
                        if (
                            j + 5 < len(pts)
                            and pts[j + 5] == pt_target
                            and (
                                pt_target != "DB"
                                or sq.db_match[i][j + 5] == sq.db_match[i][j + 4]
                            )
                        ):
    ...
    def search_like(sq, used_value, i, j, sql_dict, where_idx, recurrent=False):
        ...
        for (w_j, w), pt in zip(enumerate(sq.sub_sequence_toks[i]), sq.pattern_tok[i]):
            if re_w:
                if [i, w_j] not in used_value and (
                    pt in ["PDB", "UDB", "SDB", "DB"]
                    or (w.isalpha() and not w.islower())
                ):
                    re_w += " " + w
                    used_value.append([i, w_j])
                else:
                    break
            elif [i, w_j] not in used_value and (
                pt in ["PDB", "UDB", "SDB", "DB"]
                or (w.isalpha() and not w.islower() and w_j != 0)
            ):
                re_w = w
                used_value.append([i, w_j])
        if recurrent:
        ...
        if (
            not re_w
            and len(sql_dict["where"]) == 3
            and where_idx == 2
            and i + 1 < len(sq.sub_sequence_toks)
        ):
            re_w = search_like(
                sq, used_value, i + 1, 0, sql_dict, where_idx, recurrent=True
            )

        if not re_w and not recurrent:
            for (w_j, w), pt in zip(
                enumerate(sq.sub_sequence_toks[i]), sq.pattern_tok[i]
            ):
                if w_j <= j:
                    continue
                if [i, w_j] in used_value or pt in [
                    "ST",
                    "STC",
                    "SC",
                    "COL",
                ...
                if w not in [
                    ...
                    "letter",
                    "character",
                ] and (w.isupper() or pt not in ["IN", "#"]):
                    re_w = w
                    used_value.append([i, w_j])
                    break
                if (
                    w_j > 0
                    and sq.sub_sequence_toks[i][w_j - 1]
                    in ["word", "substring", "letter", "character"]
                    and w.isalpha()
                ):
                    if (
                        w == "the"
                        and w_j + 1 < len(sq.sub_sequence_toks[i])
                        and sq.sub_sequence_toks[i][w_j + 1].isalpha()
                    ):
        ...
        if re_w:
            if (
                " start with " in sq.question_or
                or " starting with " in sq.question_or
                or " started with " in sq.question_or
                or " starts with " in sq.question_or
                or " begin with " in sq.question_or
                or " beginning with " in sq.question_or
                or " began with " in sq.question_or
                or " begins with " in sq.question_or
                or " start from " in sq.question_or
                or " starts from " in sq.question_or
                or " starting from " in sq.question_or
                or " started from " in sq.question_or
                or " begin from " in sq.question_or
                or " begins from " in sq.question_or
                or " began from " in sq.question_or
                or " beginning from " in sq.question_or
            ):
                re_w = re_w + "%"
            elif (
                " end with " in sq.question_or
                or " ending with " in sq.question_or
                or " ended with " in sq.question_or
                or " ends with " in sq.question_or
                or " end by " in sq.question_or
                or " ending by " in sq.question_or
                or " ended by " in sq.question_or
                or " ends by " in sq.question_or
            ):
                re_w = "%" + re_w
            re_w = "'" + re_w + "'"
    ...
    for j, pt in enumerate(pts):
        if (
            patterns == ["NUM", "DATE", "YEAR"]
            and pt in ["NUM", "DATE", "YEAR"]
            and [i, j] not in used_value
        ):
            if pt == "DATE":
                re_num, used_value = add_one_value(
                    pt, "DATE", pts, sq, used_value, i, j
                )
            else:
                re_num = sq.sub_sequence_toks[i][j]
                ...
                used_value.append([i, j])
                if (
                    sql_dict["where"][where_idx][1] != 1
                    and re_num == 1
                    and "NUM" in pts[j + 1 :]
                    and [i, pts.index("NUM", j + 1)] not in used_value
                    and no_more_num_cond(sql_dict["where"], where_idx, table_json)
                    or sql_dict["where"][where_idx][1] != 1
                    and re_num == 1
                    and "YEAR" in pts[j + 1 :]
                    and [i, pts.index("YEAR", j + 1)] not in used_value
                    and no_more_num_cond(sql_dict["where"], where_idx, table_json)
                ):
                    continue
                if (
                    ...
                    and re_num == 1
                    and "DB" in pts
                    and [i, pts.index("DB")] not in used_value
                    and where_idx + 1 >= len(sql_dict["where"])
                    and sq.sub_sequence_toks[i][pts.index("DB")].isdigit()
                    or sql_dict["where"][where_idx][1] != 1
                    and re_num == 1
                    and i + 1 < len(sq.pattern_tok)
                    and "NUM" in sq.pattern_tok[i + 1]
                    and [i + 1, sq.pattern_tok[i + 1].index("NUM")] not in used_value
                    and where_idx + 1 >= len(sql_dict["where"])
                    or sql_dict["where"][where_idx][1] != 1
                    and re_num == 1
                    and i + 1 < len(sq.pattern_tok)
                    and "YEAR" in sq.pattern_tok[i + 1]
                    and [i + 1, sq.pattern_tok[i + 1].index("YEAR")] not in used_value
                    and where_idx + 1 >= len(sql_dict["where"])
                    or sql_dict["where"][where_idx][1] != 1
                    and re_num == 1
                    and i + 1 < len(sq.pattern_tok)
                    and "DB" in sq.pattern_tok[i + 1]
                    and [i + 1, sq.pattern_tok[i + 1].index("DB")] not in used_value
                    and where_idx + 1 >= len(sql_dict["where"])
                    and sq.sub_sequence_toks[i + 1][
                        sq.pattern_tok[i + 1].index("DB")
                    ].isdigit()
                ):
        ...
        if (
            ...
            and pt == "DB"
            and [i, j] not in used_value
            and str_is_num(sq.sub_sequence_toks[i][j])
        ):
        ...
        if (
            ...
            and pt == "DB"
            and [i, j] not in used_value
            and str_is_date(sq.sub_sequence_toks[i][j])
        ):
        ...
        if (
            ...
            and pt == "SDB"
            and [i, j] not in used_value
            and sq.sub_sequence_toks[i][j].lower()
            in ["none", "nil", "null", "inf", "-inf", "+inf"]
        ):
            re_num = "'" + sq.sub_sequence_toks[i][j] + "'"
            used_value.append([i, j])
            return True, re_num, used_value
        if patterns == ["NOT"] and pt == "NOT" and [i, j] not in used_value:
            re_num = "'null'"
            used_value.append([i, j])
            return True, re_num, used_value
        if patterns == ["TEXT_DB", "DB"] and pt == "DB" and [i, j] not in used_value:
            if (
                where[2][1][1] in table_json["tc_fast"]
                and natsql_idx_to_table2_idx(
                    table_json["tc_fast"].index(where[2][1][1]), table_json
                )
                in sq.full_db_match[i][j]
            ) or (
                type(where[2][1][1]) == int
                and natsql_idx_to_table2_idx(where[2][1][1], table_json)
                in sq.full_db_match[i][j]
            ):
                re_num, used_value = add_one_value(pt, "DB", pts, sq, used_value, i, j)
                return True, re_num, used_value
        elif (
            (patterns == ["DB"] and pt == "DB")
            or (patterns == ["SDB"] and pt == "SDB")
            or (patterns == ["PDB"] and pt == "PDB")
            or (patterns == ["UDB"] and pt == "UDB")
            or (patterns == ["NN"] and pt == "NN")
        ) and [i, j] not in used_value:
            re_num, used_value = add_one_value(pt, pt, pts, sq, used_value, i, j)
            return True, re_num, used_value
            ...
            patterns == ["BOOL_NUM", "NUM"]
            and pt in ["NUM"]
            and [i, j] not in used_value
        ):
            re_num = sq.sub_sequence_toks[i][j]
            if (re_num.isdigit() and re_num in ["1", "0"]) or re_num in [
                "'1'",
                "'0'",
                '"1"',
                '"0"',
...
def analyse_num(
    sq, used_value, sql_dict, w_i, where, available_pattern, table_json, sq_sub_idx=-1
):
    ...
    for (i_t, _), pts in zip(enumerate(sq.sub_sequence_type), sq.pattern_tok):
        ...
        if there_is_available_pattern:
            success, num, used_value = generate_where_values(
                i_t,
                pts,
                sq,
                used_value,
...
def fill_values(sql_dict, sq, table_json):
    ...
    if sql_dict["limit"]:
        ...
        if sql_dict["limit"] == 1:
            ...
            if there_is_agg and num_in_select:
                for (i, type_), pts in zip(
                    enumerate(sq.sub_sequence_type), sq.pattern_tok
                ):
                    if type_ <= 1 and "NUM" in pts:
                        num_idx, num = get_num_for_limit(
                            pts, sq, sql_dict, i, table_json
                        )
                        if num and type(num) == int:
                            sql_dict["limit"] = num
                            used_value.append([i, num_idx])
    ...
    for i, where in enumerate(sql_dict["where"]):
        if type(where) != list or where[2][1][1] not in table_json["tc_fast"]:
            continue
        if (
            where[1] in [1, 3, 4, 5, 6, 2, 7, 14]
            and (
                where[1] in [1, 3, 4, 5, 6]
                or where[2][1][0]
                or table_json["column_types"][
                    table_json["tc_fast"].index(where[2][1][1])
                ]
                in ["number", "year", "time"]
            )
            and where[3] == '"terminal"'
        ):
            success, used_value, sql_dict = analyse_num(
                sq, used_value, sql_dict, i, where, ["NUM", "DATE", "YEAR"], table_json
            )
            if not success:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["DB", "SDB"], table_json
                )
            if not success and len(sql_dict["where"]) == 1 and where[1] == 2:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["NOT"], table_json
                )
            if (
                not success
                and i - 2 >= 0
                and type(sql_dict["where"][i - 2]) == list
                and (
                    sql_dict["where"][i - 2][1] in [1, 3, 4, 5, 6]
                    or sql_dict["where"][i - 2][2][1][0]
                    or table_json["column_types"][
                        table_json["tc_fast"].index(sql_dict["where"][i - 2][2][1][1])
                    ]
                    in ["number", "year", "time"]
                )
                and type(sql_dict["where"][i - 2][3]) != list
                and sql_dict["where"][i - 2][3] != '"terminal"'
            ):
            ...
            if (
                not success
                and i - 4 >= 0
                and type(sql_dict["where"][i - 4]) == list
                and sql_dict["where"][i - 4][1] == sql_dict["where"][i][1]
                and sql_dict["where"][i][2][1][1] == sql_dict["where"][i - 4][2][1][1]
                and type(sql_dict["where"][i - 4][3]) != list
                and sql_dict["where"][i - 4][3] != '"terminal"'
            ):
                sql_dict["where"][i][3] = sql_dict["where"][i - 4][3]
                success = True
            if not success:
                if (
                    table_json["data_samples"][
                        table_json["tc_fast"].index(where[2][1][1])
                    ]
                    and type(
                        table_json["data_samples"][
                    ...
                    sql_dict["where"][i][3] = table_json["data_samples"][
                        table_json["tc_fast"].index(where[2][1][1])
                    ][0]
                else:
            ...
            where[1] in [2, 7]
            and table_json["column_types"][table_json["tc_fast"].index(where[2][1][1])]
            in ["text"]
            and where[3] == '"terminal"'
        ):
            success, used_value, sql_dict = analyse_num(
                sq, used_value, sql_dict, i, where, ["TEXT_DB", "DB"], table_json
            )
            if not success and no_more_num_cond(sql_dict["where"], i, table_json):
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["DB_NUM", "NUM"], table_json
                )
            if not success:
    ...
    for i, where in enumerate(sql_dict["where"]):
        ...
        if (
            ...
            success, used_value, sql_dict = analyse_num(
                sq, used_value, sql_dict, i, where, ["DB"], table_json
            )
            if not success:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["PDB"], table_json
                )
            if not success:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["UDB"], table_json
                )
            if not success:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["SDB"], table_json
                )
            if not success:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["BOOL_NUM", "NUM"], table_json
                )
            if not success:
            ...
            if (
                type(where[3]) == str
                and where[3][1:-1]
                not in table_json["data_samples"][
                    table_json["tc_fast"].index(where[2][1][1])
                ]
            ):
                for v in table_json["data_samples"][
                    table_json["tc_fast"].index(where[2][1][1])
                ]:
                    if type(v) == str and (
                        where[3][1:].lower().startswith(v.lower())
                        or v.lower().startswith(where[3][1:].lower())
                    ):
                        where[3] = "'" + v + "'"
            if (
                i == 2
                and len(sql_dict["where"]) == 3
                and type(sql_dict["where"][0]) == list
                and sql_dict["where"][0][2][1][1] == where[2][1][1]
                and where[3] == sql_dict["where"][0][3]
            ):
                for v in table_json["data_samples"][
                    table_json["tc_fast"].index(where[2][1][1])
                ]:
    ...
    for i, where in enumerate(sql_dict["where"]):
        if type(where) != list or where[2][1][1] not in table_json["tc_fast"]:
            continue
        if where[1] in [9, 13] and where[3] == '"terminal"':
            success, used_value, sql_dict = analyse_num(
                sq, used_value, sql_dict, i, where, ["PDB"], table_json
            )
            if not success:
            ...
            if not success:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["tilt"], table_json
                )
            if not success:
                success, used_value, sql_dict = analyse_num(
                    sq, used_value, sql_dict, i, where, ["SDB"], table_json
            ...
            if not success:
                where[3] = "'%'"
            elif table_json["column_types_checked"][
                table_json["tc_fast"].index(where[2][1][1])
            ] in ["date", "year", "time", "number"]:
                if type(where[3]) == str and where[3].startswith("'"):
                    ...
                    if value.isdigit():
                        ...
                        where[3] = (
                            where[3][0] + "%" + where[3][1:-1] + "%" + where[3][-1]
                        )
                else:
                    where[3] = "'" + str(where[3]) + "%'"
            ...
            if "%" not in where[3] and len(where[3]) > 2:
                if (
                    where[3][1:-1].isdigit()
                    or " start with " in sq.question_or
                    or " starting with " in sq.question_or
                    or " started with " in sq.question_or
                    or " starts with " in sq.question_or
                    or " begin with " in sq.question_or
...
def IUE2Where(sql_dict, table_json):
                ...
                if not tmp_take_tbl[i]:
                    ...
                    for col in table_json["column_names"]:
                        if (
                            col[0] == t_idx
                            and col[1].lower()
                            == table_json["column_names"][where_col_idx][1].lower()
                        ):
                            tmp_take_tbl[i] = True
                            new_where_cond[i] = copy.deepcopy(sql_dict["where"][w_idx])
                            new_where_cond[i][2][1][1] = table_json[
    ...
    for w, where in enumerate(sql_dict["where"]):
        if type(where) == list:
            ...
            if where_col != "@.@":
                take_tbl, new_where_cond = search_where_col(
                    tbl, take_tbl, where_col, table_json, w, sql_dict
                )
            elif type(where[3]) == list and where[3][1] == "@.@":
                take_tbl, new_where_cond = search_where_col(
                    tbl, take_tbl, where[3][1], table_json, w, sql_dict
                )
            if take_tbl[0]:
    ...
    if (not sql_dict["where"]) or (
        len(sql_dict["where"]) == 1 and sql_dict["where"][0][1] == 15
    ):
        tbl = ["", ""]
        take_tbl = [False, False]
        if sql_dict["union"]:
    ...
    if take_tbl[0]:
        iue_where[0] = [
            ...
            [
                ...
                10,
                [0, [0, "@.@", False], None],
                [0, sql_dict["union"]["select"][1][0][1][1][1], False],
                None,
            ],
        ]
    ...
    if take_tbl[1]:
        iue_where[1] = [
            ...
            [
                ...
                10,
                [0, [0, "@.@", False], None],
                [0, sql_dict["intersect"]["select"][1][0][1][1][1], False],
                None,
            ],
        ]
        if len(sql_dict["where"]) == 1 and sql_dict["where"][0][1] == 15:
...
def iue2subquery(
    ...
    if sql_dict["where"][1] in ["except_", "intersect_", "union_"]:
        ...
        len(sql_dict["where"]) == 5
        and sql_dict["where"][3] in ["except_", "intersect_", "union_"]
        and type(sql_dict["where"][0]) == list
        and sql_dict["where"][0][2][1][1].lower() in table_json["tc_fast"]
    ):
        t_id_list.append(
            table_json["column_names"][
                table_json["tc_fast"].index(sql_dict["where"][0][2][1][1].lower())
            ][0]
        )
    if (
        len(sql_dict["where"]) == 5
        and len(set(t_id_list)) == 1
        and sql_dict["where"][3] in ["except_", "intersect_", "union_"]
    ):
        insert_idx = 2
    else:
    ...
    for fks in from_table_net[0]:
        if (
            table_json["column_names"][fks[0]][0] in t_id_list
            and table_json["column_names"][fks[1]][0] not in t_id_list
        ):
            ...
            sql_dict["where"].insert(
                ...
                [
                    ...
                    8,
                    [0, [0, "@.@", False], None],
                    [0, table_json["tc_fast"][fks[1]], None],
                    None,
                ],
            )
            return True, sql_dict
        if (
            table_json["column_names"][fks[1]][0] in t_id_list
            and table_json["column_names"][fks[0]][0] not in t_id_list
        ):
            sql_dict["where"].insert(insert_idx, "and")
            sql_dict["where"].insert(
                insert_idx,
                [
                    ...
                    8,
                    [0, [0, "@.@", False], None],
                    [0, table_json["tc_fast"][fks[0]], None],
                    None,
                ],
            )
            return True, sql_dict
    if (
        len(sql_dict["select"][1]) == 1
        and (sql_dict["select"][1][0][0] or sql_dict["select"][1][0][1][1][0])
        and len(sql_dict["where"]) in [3, 5]
        and (
            sql_dict["where"][1] in ["except_", "intersect_", "union_"]
            or (
                len(sql_dict["where"]) == 5
                and sql_dict["where"][3] in ["except_", "intersect_", "union_"]
            )
        ...
        sql_dict["where"].insert(
            ...
            [
                ...
                [
                    0,
                    sql_dict["where"][insert_idx + 1][2][1][1].split(".")[0] + ".*",
                    None,
                ],
                None,
            ],
...
def join2subquery(
    from_table_net, select_table_list, table_json, groupby_list, sql_dict
):
    t_id_list = []
    for t in select_table_list:
        t_id_list.append(table_json["table_orig_low"].index(t))
    ...
    for fks in from_table_net[0]:
        if (
            fks[0] in table_json["primary_keys"]
            and fks[1] not in table_json["primary_keys"]
            and table_json["column_names"][fks[0]][0] in t_id_list
            and fks[1] not in table_json["unique_fk"]
            and table_json["column_names"][fks[1]][0] not in t_id_list
            ...
            return True, sql_dict
        if (
            fks[1] in table_json["primary_keys"]
            and fks[0] not in table_json["primary_keys"]
            and table_json["column_names"][fks[1]][0] in t_id_list
            and fks[0] not in table_json["unique_fk"]
            and table_json["column_names"][fks[0]][0] not in t_id_list
...
def groupby2subquery(
    ...
    if table_json["column_names"][group_col_idx][0] not in t_id_list:
        pass
    elif group_col_idx in table_json["primary_keys"] or (
        len(sql_dict["select"][1]) == 1
        and sql_dict["select"][1][0][1][1][1].lower() == groupby_list[0].lower()
    ):
        return False, sql_dict
    else:
        pass
    ...
    sql_dict["where"].insert(
        ...
        [
            ...
            8,
            [0, [0, "@.@", False], None],
            [0, table_json["tc_fast"][group_col_idx], None],
            None,
        ],
    )
    sql_dict["groupBy"] = []
...
def simplify_join(
    ...
    if (
        sql_dict["where"]
        and type(sql_dict["where"][0]) == list
        and sql_dict["where"][0][1] == 12
        and sql_dict["where"][0][2][1][1] == sql_dict["where"][0][3][1]
    ):
        where_col_idx = table_json["tc_fast"].index(
            sql_dict["where"][0][2][1][1].lower()
        )
        need_simplify = False
        other_col = 0
        for fks in from_table_net[0]:
...
def search_all_join_on(
    sql_dict, table_json, args, join_on_label=None, sq=None, first_round=True
):
    """
    ...
    for column in sql_dict["select"][1]:
        ...
        if not (column[0] or column[1][1][0]):
            if select_unit != "*":
                top_groupby_list.append(
                    column[1][1][1].lower() if column[1][0] else select_unit
                )
        else:
            agg_in_select = True
        re_sql += select_unit + " , "
    ...
    if first_round:
        ...
        sql_dict["where"] = union_inference(
            sql_dict["where"], sq, table_json, top_select_table_list
        )
    ...
    (
        ...
        next_table_list,
        top_left_col_list,
    ) = get_where_column(sql_dict, table_list, 0, SQL_TOP, table_json, args)
    if (
        (" * not in " in sql_where or " * in " in sql_where)
        and len(sql_dict["where"]) - break_idx >= 3
        and sql_dict["where"][break_idx][1] in [12, 8]
        and sql_dict["where"][break_idx + 2][2][1][1].split(".")[0]
        != sql_dict["where"][break_idx][3][1].split(".")[0]
    ):
        ...
        if sql_dict["where"][break_idx + 2][2][1][1].split(".")[0] != "@":
            sql_dict["where"][break_idx][3][1] = (
                sql_dict["where"][break_idx + 2][2][1][1].split(".")[0] + ".*"
            )
        else:
            sql_dict["where"][break_idx][3][1] = (
                sql_dict["where"][break_idx + 2][3][1].split(".")[0] + ".*"
            )
        (
            break_idx,
            _,
    ...
    if (
        break_idx < 0
        or next_sql == SQL_TOP
        or (sql_dict["orderBy"] and not sql_dict["limit"])
    ):
        orderby_sql, table_list_order, agg_in_order = create_order_by(
            sql_dict["orderBy"], sql_dict["limit"]
        )
    ...
    from_table_net, table_fk_list = get_table_network(
        ...
        sq=sq,
        sql_dict=sql_dict,
        group_list=top_left_col_list + top_groupby_list if top_groupby_list else [],
    )
    from_table_netss, _ = get_table_network(
        table_json,
        table_list,
        join_on_label,
        False,
    ...
    if sql_dict["groupBy"]:  # V1.1:
        ...
        if break_idx == SQL_TOP:
            add_top_group = False
        if (
            (len(top_groupby_list) != len(sql_dict["select"][1]) and top_groupby_list)
            or sql_having.strip() != ""
            or (agg_in_order and top_groupby_list)
            or orderby_sql_.strip()
        ):
            add_top_group = True
    elif (
        not args.not_infer_group
        and (len(top_groupby_list) != len(sql_dict["select"][1]) and top_groupby_list)
        or sql_having.strip() != ""
        or (agg_in_order and top_groupby_list)
        or orderby_sql_.strip()
    ):
        table_list = list(set(table_list))
        if args.group_for_exact_match and (
            len(top_groupby_list) > 1
            or len(table_list) > 1
            or len(top_groupby_list) == 0
        ):
            agg_tables = get_agg_tables(sql_dict, table_json)
            groupby_list = infer_group_for_exact_match(
                top_groupby_list,
                table_json,
                table_list,
                len(sql_dict["select"][1]),
                sq,
                agg_tables,
                from_table_net,
            )
            groupby_list = group_back_because_eva_bug_in_spider(
                groupby_list, table_json, agg_tables, top_groupby_list, from_table_net
            )
            groupby_top = " group by " + ",".join(groupby_list)
        else:
    ...
    if (
        ...
        and (
            (
                len(sql_dict["where"]) in [3, 5]
                and sql_dict["where"][1] in ["except_", "intersect_", "union_"]
            )
            or (
                len(sql_dict["where"]) == 5
                and sql_dict["where"][3] in ["except_", "intersect_", "union_"]
    ...
    if (
        args.join2subquery
        and agg_in_select
        and not sql_dict["select"][0]
        and not sql_dict["limit"]
        and not (
            sql_dict["where"]
            and type(sql_dict["where"][0]) == list
            and sql_dict["where"][0][1] in [12, 8]
        )
    ):
        success, sql_dict_orig = join2subquery(
            from_table_net,
            top_select_table_list,
            table_json,
            groupby_list,
    ...
    if (
        args.groupby2subquery
        and groupby_top
        and len(groupby_list) == 1
        and not agg_in_select
    ):
        success, sql_dict_orig = groupby2subquery(
            from_table_net, top_select_table_list, table_json, groupby_list, sql_dict
        )
        if success:
            return 3, sql_dict_orig, None, None
    success, sql_dict = simplify_join(
        from_table_net, top_select_table_list, table_json, groupby_list, sql_dict
    )
    if success:
    ...
    if sql_dict["where"]:
        ...
        while next_sql:
            ...
            if next_sql == SQL_TOP:
                if (
                    type(sql_dict["where"][0]) == str
                    and sql_dict["where"][1][2][1][1] == "@.@"
                    and sql_dict["where"][1][1] == 10
                ):  # and  ".*" in sql_dict['where'][1][3][1]:
                    # V1.4 NatSQL extension: where IUE @.@ is table.* :
                    if not (
                        len(sql_dict["where"]) > 3 and sql_dict["where"][2] == "and"
                    ):
                        sub_sql, table_list = infer_IUE_select_col(
                            sql_dict,
                            table_json,
                            break_idx,
                            top_sql_list,
                        ...
                        sub_sql, table_list = fk_replace_IUE(
                            sql_dict,
                            table_json,
                            break_idx,
                            top_sql_list,
                    ...
                    len(top_sql_list) == 2
                    and sql_dict["where"][break_idx]
                    in ["intersect_", "union_", "except_"]
                    and break_idx > 1
                    and sub_sql_list
                    and sub_sql_select_table_list
                    and sql_dict["where"][break_idx - 1][2][1]
                    == sql_dict["where"][break_idx + 1][2][1]
                    and top_sql_list[1].strip().endswith(")")
                ):
                    # IUE in sub query
                    sub_sql, table_list = fk_replace_IUE(
                        sql_dict,
                    ...
                    len(top_sql_list) == 2
                    and sql_dict["where"][break_idx] in ["intersect_"]
                    and break_idx > 1
                    and sql_dict["where"][break_idx - 1][2]
                    == sql_dict["where"][break_idx + 1][2]
                    and (
                        break_idx == 1
                        or (
                            break_idx == 3
                            and type(sql_dict["where"][0]) == list
                            and sql_dict["where"][0][1] == 15
                        )
                    )
                ):
                    sub_sql, table_list = fk_replace_IUE(
                ...
                if "." not in select_column:
                    sub_sql_select_table_list = [
                        sql_dict["where"][break_idx][3][1].split(".")[0].lower()
                    ]
                else:
                    sub_sql_select_table_list = [
                        sql_dict["where"][break_idx][3][1].split(".")[0].lower()
                    ]  # [select_column.split(".")[0].lower()]
                if (
                    sql_dict["where"][break_idx][3][1].split(".")[0].lower()
                    not in table_list
                ):
            ...
            if (
                args.orderby_to_subquery
                and not orderby_sql_
                and not (
                    previous_break_idx > 1
                    and sql_dict["where"][previous_break_idx - 1] == "sub"
                )
            ):
                orderby_sql_, table_list = orderby_to_subquery(
                    sql_dict, table_list
                )  # v1.1
            ...
            if sub_sql.startswith(" except "):
                from_table_net, table_fk_list = get_table_network(
                    table_json, table_list, join_on_label
                )
            else:
                from_table_net, table_fk_list = get_table_network(
                    table_json, table_list, join_on_label, sq=sq, sql_dict=sql_dict
                )
            from_table_netss, _ = get_table_network(
                table_json, table_list, join_on_label, False
            )
            all_from.append(from_table_netss)

            if (
                sub_sql.startswith("select ") and select_column.count(".") == 1
            ):  # sub query
                sub_sql_table = select_column.split(".")[0]
                if "(" in sub_sql_table:
                    sub_sql_table = sub_sql_table.split("(")[1]
                sub_sql += create_from_table(
                    from_table_net,
                    table_json["table_names_original"],
                    table_json["table_column_names_original"],
                    table_fk_list,
            ...
            if not start_new_top_sql:
                if (sql_having.strip() and select_column) or (
                    (
                        "max(" in orderby_sql_
                        or "min(" in orderby_sql_
                        or "count(" in orderby_sql_
                        or "sum(" in orderby_sql_
                        or "avg(" in orderby_sql_
                    )
                    and select_column
                ):  # v1.0
                    sub_sql += " group by " + select_column
            elif (sql_having.strip() != "" and groupby_list) or (
                orderby_sql_.strip() and groupby_list
            ):
                if groupby_list and groupby_list[0].split(".")[0] not in table_list:
                    ...
                    groupby_list = infer_group_for_exact_match(
                        groupby_list, table_json, table_list, len(sql_dict["select"][1])
                    )
            ...
            if (
                ...
                top_sql_list[len(top_sql_list) - 1] = top_sql_list[
                    len(top_sql_list) - 1
                ].replace("@@@", sub_sql, 1)
