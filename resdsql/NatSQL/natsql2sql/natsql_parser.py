...
from .natsql2sql import Args, inference_sql
from .natsql2sql import natsql_version as n_version
from .process_sql import Schema, get_schema, get_tables_with_alias
...
def tokenize(string):
    ...
    eq_idxs = [
        ...
        if tok == "=" or tok == "in" or tok == "like"
    ]  # make 'not in' and 'not like' together
    eq_idxs.reverse()
    prefix = ("!", ">", "<")
    for eq_idx in eq_idxs:
        ...
        if pre_tok in prefix:
            ...
            toks = (
                toks[: eq_idx - 1] + [pre_tok + " " + toks[eq_idx]] + toks[eq_idx + 1 :]
            )  # make 'not in' and 'not like' together
    return toks
...
def parse_value(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    if toks[idx] == "select":
        ...
        val = toks[idx]
        idx += 1
    elif toks[idx] == "value":  # token is a string value
        val = 1
        idx += 1
    else:
        try:
            ...
            while (
                ...
                and toks[end_idx] not in CLAUSE_KEYWORDS2
                and toks[end_idx] not in COND_OPS
            ):  # toks[end_idx] != 'sub' and toks[end_idx] != 'or':
                end_idx += 1

            if (
                toks[start_idx] in AGG_OPS and toks[end_idx] == ")"
            ):  # Add for where column = agg(column)
                end_idx += 1
