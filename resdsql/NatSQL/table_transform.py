...
def construct_hyper_param():
    ...
    parser.add_argument(
        "--star_type", default="others", type=str, help="what type of * in column type?"
    )
    ...
    parser.add_argument(
        "--use_table_name_for_star_in_col_name", action="store_true", default=False
    )

    parser.add_argument(
        "--force_modify_star_column_names", action="store_true", default=False
    )

    parser.add_argument(
        "--remove_star_from_col", action="store_true", default=False
    )  # Only for training. If True, put star to table name.

    parser.add_argument(
        "--add_alpha_to_table", action="store_true", default=False
    )  # For syntaxSQL. Here is True, modify_column_names is True
    ...
    parser.add_argument(
        "--recover_previous_column_content", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "--keepOriginal", action="store_true", default=False
    )  # keep the original data as same as tables.json
...
def table_transform(table, args, schema):
    ...
    for item in table["column_names"]:
        ...
        if last_table_index != item[0] and not args.remove_star_from_col:
            if args.modify_column_names or args.force_modify_star_column_names:
                new_table["column_names"].insert(
                    insert_index,
                    [
                        item[0],
            ...
            elif args.use_table_name_for_star_in_col_name:
                new_table["column_names"].insert(
                    insert_index, [item[0], table["table_names"][item[0]]]
                )
            else:
            ...
            new_table["table_column_names_original"].append(
                [item[0], table["table_names_original"][item[0]] + ".*"]
            )
            insert_index += 1
        ...
        if (
            args.modify_column_names
            and schema.column_tokens_lemma_str.count(
                schema.column_tokens_lemma_str[insert_index_schema]
            )
            > 1
        ):
            for (tb_idx, last_table_string), last_table_lemma in zip(
                enumerate(table["table_names"]), schema.table_tokens_lemma_str
            ):
                last_table_string = last_table_string.split(" | ")
                last_table_lemma = last_table_lemma.split(" | ")
                if (
                    not new_table["column_names"][insert_index][1].startswith(
                        last_table_string[0] + " "
                    )
                    and new_table["column_names"][insert_index][1]
                    != last_table_string[0]
                    and not schema.column_tokens_lemma_str[
                        insert_index_schema
                    ].startswith(last_table_lemma[0] + " ")
                    and schema.column_tokens_lemma_str[insert_index_schema]
                    != last_table_lemma[0]
                    and (
                        len(last_table_string) == 1
                        or (
                            not schema.column_tokens_lemma_str[
                                insert_index_schema
                            ].startswith(last_table_lemma[-1] + " ")
                            and schema.column_tokens_lemma_str[insert_index_schema]
                            != last_table_lemma[-1]
                        )
                    )
                    and " " + schema.column_tokens_lemma_str[insert_index_schema]
                    not in last_table_lemma[0]
                    and schema.column_tokens_lemma_str[insert_index_schema] + " "
                    not in last_table_lemma[0]
                ):
                    pass
                    ...
                    for i2, col2 in enumerate(new_table["column_names"]):
                        if (
                            ...
                            == new_table["column_names"][insert_index][1]
                            and new_table["column_names"][i2][0] == tb_idx
                        ):
            ...
            if last_table_string:
                ...
                if (
                    " | " not in new_table["column_names"][insert_index][1]
                    and len(last_table_string) > 1
                ):
                    new_table["column_names"][insert_index][1] = " | ".join(
                        [
                            lts + " " + new_table["column_names"][insert_index][1]
                            for lts in last_table_string
                        ]
                    )
                else:
        ...
        new_table["table_column_names_original"].append(
            [
                item[0],
                table["table_names_original"][item[0]]
                + "."
                + table["column_names_original"][item_index][1],
            ]
        )

    if not args.remove_star_from_col:
        add = 0 if args.add_alpha_to_table else 1
        assert len(new_table["column_names"]) + add - table_num == len(
            table["column_names"]
        )
...
def get_next_table(table_idx, table_range, table_fk, table_col):
    ...
    for fk in table_fk:
        if (
            fk[0] >= col_range[0] and fk[0] <= col_range[1]
        ):  # Check the left fk belong to the table of table_idx
    ...
    while True:
        ...
        for jj in range(2):
            if index_table_in_table_list >= 0:
                ...
                next_tables, fk_next_tables = get_next_table(
                    table_fk_left,
                    table["index_range"],
                    table["foreign_keys"],
                    ...
                    for nt, fk in zip(next_tables, fk_next_tables):
                        if nt not in table["network"][table_net_idx][1]:
                            if jj == 1 and fk_of_one_net_table_list:
                                new = copy.deepcopy(
                                    [fk_of_one_net_table_list, one_net_table_list]
                                )
                            else:
                            ...
                            if to_all_table not in table_all:
                                ...
                                max_table_net_len = max(max_table_net_len, len(new[1]))

            if (
                len(one_net_table_list) >= 2
                and index_table_in_table_list == len(one_net_table_list) - 1
            ):
                one_net_table_list = [r for r in reversed(one_net_table_list)]
                fk_of_one_net_table_list = [
                    r for r in reversed(table["network"][table_net_idx][0])
                ]
            else:
                break
...
def enlarge_network(net_work, table):
    def in_side_enlarge(t1t2_fk, table_1, table_2, table):
        ...
        for fk in table["foreign_keys"]:
            ...
            if fk != t1t2_fk and (
                (
                    table["column_names"][fk[0]][0] == table_1
                    and table["column_names"][fk[1]][0] == table_2
                )
                or (
                    table["column_names"][fk[0]][0] == table_2
                    and table["column_names"][fk[1]][0] == table_1
                )
            ):
                new_fk.append(fk)
        return new_fk
        ...
        for i, t in enumerate(table["table_names_original"]):
            if i not in net[1]:
                for fk in table["foreign_keys"]:
                    if (
                        table["column_names"][fk[0]][0] == table_1
                        and table["column_names"][fk[1]][0] == i
                    ) or (
                        table["column_names"][fk[0]][0] == i
                        and table["column_names"][fk[1]][0] == table_1
                    ):
                        for fk2 in table["foreign_keys"]:
                            if (
                                table["column_names"][fk2[0]][0] == table_2
                                and table["column_names"][fk2[1]][0] == i
                            ) or (
                                table["column_names"][fk2[0]][0] == i
                                and table["column_names"][fk2[1]][0] == table_2
                            ):
    ...
    for net in net_work:
        ...
        for i in range(1, len(net[1]), 1):
            new_net_work = add_table_to_enlarge(
                net, i, net[1][i - 1], net[1][i], table, new_net_work
            )
    ...
    for i, net in enumerate(new_net):  # 3 tables network
        for j, net2 in enumerate(new_net):
            if i < j and (
                (net[1][1] in net2[1] and net[1][0] not in net2[1])
                or (net[1][0] in net2[1] and net[1][1] not in net2[1])
            ):
                tmp_n3 = [
                    [net[0][0], net2[0][0]],
                    [
                        net[1][0],
                        net[1][1],
                        net2[1][0] if net2[1][1] in net[1] else net2[1][1],
                    ],
                ]
    ...
    for i, net in enumerate(new_net):  # 4 tables network
        for j, net2 in enumerate(new_net_3):
            if (net2[1][-1] == net[1][0] and net[1][1] not in net2[1]) or (
                ...
                tmp_n4 = [
                    [net2[0][0], net2[0][1], net[0][0]],
                    [
                        net2[1][0],
                        net2[1][1],
                        net2[1][2],
                        net[1][1] if net2[1][-1] == net[1][0] else net[1][0],
                    ],
                ]
    ...
    for net in net_work:
        ...
        for i, t in enumerate(table["table_names_original"]):
            if i not in net[1]:
                for fk in table["foreign_keys"]:
                    if (
                        table["column_names"][fk[0]][0] == table_idx
                        and table["column_names"][fk[1]][0] not in net[1]
                    ):
                        new_tb_idx = table["column_names"][fk[1]][0]
                    elif (
                        table["column_names"][fk[0]][0] not in net[1]
                        and table["column_names"][fk[1]][0] == table_idx
                    ):
...
def build_super_column_name(table):
    ...
    table["super_table_column_names_original"] = copy.deepcopy(
    ...
    for idx in range(len(table["column_names"])):
        ...
        if t_col[1].endswith("*"):
            ...
            table["super_table_column_names_original"].append(col_ar)
        elif table["new_column_types"][idx] == "number":
...
def check_col_table_similarity(_tokenizer, table_names, col_token):
    ...
    if (
        col_lemma == table_names
        or " ".join([tok.text for tok in col_token]) == table_names
    ):
        return True
    table_token = _tokenizer.tokenize(table_names)
    if (
        table_token[-1].lemma_ == col_token[0].lemma_
        or col_lemma == " ".join([tok.lemma_ for tok in table_token])
        or table_token[-1].lemma_ == col_token[0].text
    ):
        return True
    return False
...
def re_identify_boolean_type(tables, use_extra_col_types, database_path):
    for db_i, table in enumerate(tables):
        ...
        for t in range(len(table["table_names"])):
            ...
            table["data_samples"].extend(data_samples)

        all_cols = []
        for t in range(len(table["table_names"])):
            cols, all_col = db_.db_col_type_check(t)
            all_cols.extend(all_col)
        assert len(all_cols) == len(table["column_types"]) - 1
        tables[db_i]["column_types_checked"] = ["None"] * len(table["column_types"])

        for i, col in enumerate(all_cols):
            if col[1] == 0:  # boolean
                if table["column_types"][i + 1] != "boolean":
                    print(
                        table["column_names"][i + 1][1]
                        + " "
                        + table["column_types"][i + 1]
                        + " --> boolean"
                    )
                tables[db_i]["column_types"][i + 1] = "boolean"
                tables[db_i]["column_types_checked"][i + 1] = "boolean"
            elif col[1] == 1:  # text
                if table["column_types"][i + 1] != "text":
                    print(
                        table["column_names"][i + 1][1]
                        + " "
                        + table["column_types"][i + 1]
                ...
                if " or " in table["column_names"][i + 1][1]:
                    tables[db_i]["column_types"][i + 1] = "boolean"
                    tables[db_i]["column_types_checked"][i + 1] = "boolean"
                else:
                ...
                if table["column_types"][i + 1] != "number":
                    print(
                        table["column_names"][i + 1][1]
                        + " "
                        + table["column_types"][i + 1]
                ...
                if table["column_types"][i + 1] != "time":
                    print(
                        table["column_names"][i + 1][1]
                        + " "
                        + table["column_types"][i + 1]
                ...
                if table["column_types"][i + 1] != "year":
                    print(
                        table["column_names"][i + 1][1]
                        + " "
                        + table["column_types"][i + 1]
...
def remove_start_table(tables, schemas):
    for i, table, schema in zip(range(len(tables)), tables, schemas):
        ...
        for j, col in enumerate(table["column_names"]):
            if col[0] >= 0 and (
                col[1].startswith(schema.table_tokens_lemma_str[col[0]] + " ")
                or col[1].startswith(schema.table_tokens_text_str[col[0]] + " ")
            ):
                tables[i]["column_names"][j][1] = " ".join(
                    tables[i]["column_names"][j][1].split(" ")[
                        schema.table_tokens_lemma_str[col[0]].count(" ") + 1 :
                    ]
                )
            elif (
                j in table["primary_keys"]
                and tables[i]["column_names"][j][1].endswith(" id")
                and schema.table_tokens_lemma_str[col[0]].startswith(
                    tables[i]["column_names"][j][1][0:-3]
                )
            ):
                tables[i]["column_names"][j][1] = tables[i]["column_names"][j][1][-2:]
            elif (
                (
                    tables[i]["column_names"][j][1]
                    == schema.table_tokens_lemma_str[col[0]]
                    or tables[i]["column_names"][j][1]
                    == schema.table_tokens_text_str[col[0]]
                )
                and tables[i]["column_types"][j] == "text"
                and "name" not in schema.table_col_lemma[col[0]]
            ):
                tables[i]["column_names"][j][1] = (
                    tables[i]["column_names"][j][1] + " | name"
                )
    return tables
    ...
    for i, table in enumerate(tables):
        ...
        for j, col, ocol in zip(
            ...
            if (
                ...
                and " | " not in col[1]
                and ocol[1].count(" ") > col[1].count(" ")
                and try_to_add.count(col[0]) == 1
            ):
                print(table["column_names"][j][1])
    ...
    def they_are_same(col_1_idx, col_2_idx, table, schema, all_pair, db_):
        def contain_table_name(col_1_idx, col_2_idx, table, schema):
            for ttls in schema.table_tokens_lemma_str[
                schema.column_names_original[col_1_idx][0]
            ].split(" "):
                for ttls2 in schema.table_tokens_lemma_str[
                    schema.column_names_original[col_2_idx][0]
                ].split(" "):
                    if ttls == ttls2:
                        return True
            ...
            for ttls in schema.table_tokens_lemma_str:
                for t in ttls.split(" "):
                    if (
                        t in schema.column_tokens_lemma_str[col_1_idx]
                        or t in schema.column_tokens_lemma_str[col_2_idx]
                    ):
                        return True
            return False
        ...
        if [col_1_idx, col_2_idx] in all_pair or [col_2_idx, col_1_idx] in all_pair:
            return True
        if (
            " " not in schema.column_tokens_lemma_str[col_1_idx]
            and " " not in schema.column_tokens_lemma_str[col_2_idx]
        ):
            return False
        if col_1_idx != col_2_idx:
            if (
                schema.column_tokens_lemma_str[col_1_idx]
                == schema.column_tokens_lemma_str[col_2_idx]
                or schema.table_tokens_lemma_str[table["column_names"][col_1_idx][0]]
                + schema.column_tokens_lemma_str[col_1_idx]
                == schema.column_tokens_lemma_str[col_2_idx]
                or schema.column_tokens_lemma_str[col_1_idx]
                == schema.table_tokens_lemma_str[table["column_names"][col_2_idx][0]]
                + schema.column_tokens_lemma_str[col_2_idx]
                or schema.table_tokens_lemma_str[table["column_names"][col_1_idx][0]]
                + schema.column_tokens_text_str[col_1_idx]
                == schema.column_tokens_text_str[col_2_idx]
                or schema.column_tokens_text_str[col_1_idx]
                == schema.table_tokens_text_str[table["column_names"][col_2_idx][0]]
                + schema.column_tokens_lemma_str[col_2_idx]
                or schema.table_tokens_text_str[table["column_names"][col_1_idx][0]]
                + schema.column_tokens_lemma_str[col_1_idx]
                == schema.column_tokens_lemma_str[col_2_idx]
                or schema.column_tokens_lemma_str[col_1_idx]
                == schema.table_tokens_text_str[table["column_names"][col_2_idx][0]]
                + schema.column_tokens_lemma_str[col_2_idx]
                or schema.column_tokens_text_str[col_1_idx]
                == schema.column_tokens_text_str[table["column_names"][col_2_idx][0]]
                + schema.column_tokens_text_str[col_2_idx]
                or schema.column_tokens_text_str[table["column_names"][col_1_idx][0]]
                + schema.column_tokens_text_str[col_1_idx]
                == schema.column_tokens_text_str[col_2_idx]
            ):
                pass
                ...
                return False
            if (
                schema.column_types[col_1_idx] == schema.column_types[col_2_idx]
                and contain_table_name(col_1_idx, col_2_idx, table, schema)
                and (
                    (db_ and db_.db_content_are_same(col_1_idx, col_2_idx))
                    or db_ is None
                )
            ):
                print(
            ...
            if col[0] >= 0 and " " not in col[1] and len(col[1]) > 8:
                if col[1] not in all_words:
                    for x in range(4, len(col[1]) - 3):
                        if (
                            col[1][0:x] in all_words
                            and col[1][x:] in all_words
                            and col[1][0:x] not in STOP_WORDS
                            and col[1][x:] not in STOP_WORDS
                            and col[1][x:] not in suffix
                        ):
                            print(
                                tables[i]["column_names"][j][1]
                                + " ==================> "
                ...
                if (
                    not bk_bool
                    and schema.column_tokens_lemma_str[j] != col[1]
                    and len(schema.column_tokens_lemma_str[j]) > 8
                    and schema.column_tokens_lemma_str[j] not in all_words
                ):
                    col_lemma = schema.column_tokens_lemma_str[j]
                    for x in range(4, len(schema.column_tokens_lemma_str[j]) - 3):
                        if (
                            col_lemma[0:x] in all_words
                            and col_lemma[x:] in all_words
                            and col_lemma[0:x] not in STOP_WORDS
                            and col_lemma[x:] not in STOP_WORDS
                            and col_lemma[x:] not in suffix
                        ):
                            print(
                                tables[i]["column_names"][j][1]
                                + " ==================> "
            ...
            for db_info in db_infos:
                ...
                for line in lines:
                    ...
                    if line.strip().upper().endswith(" NOT NULL"):
                        line = line[:-8]
                    if (
                        line.strip().upper().endswith(" PRIMARY KEY,")
                        or line.strip().upper().endswith(" PRIMARY KEY ,")
                        or line.strip().upper().endswith(" PRIMARY KEY")
                    ):
                        while True:
                            if line[0].isalpha():
                        ...
                        col = (
                            ...
                            + "."
                            + "".join(
                                [t if t.isalpha() or t == "_" else "" for t in ls[0]]
                            )
                        ).lower()
                            ...
                            if len(rp_keys) > 1:
                                for col in rp_keys:
                                    if col in schema.original_table["tc_fast"]:
                                        p_id = schema.original_table["tc_fast"].index(
                                            col
                                        )
    ...
    for it, table in enumerate(tables):
        ...
        for net in table["network"]:
            ...
            if (
                len(net[1]) == 3
                and len(net[0]) == 2
                and net[0][0][0] not in net[0][1]
                and net[0][0][1] not in net[0][1]
            ):
                for fks in net[0]:
...
if __name__ == "__main__":
    ...
    if args.correct_col_type:
        tables = re_identify_boolean_type(
            tables, args.use_extra_col_types, database_path
        )
                ...
                for it, item in enumerate(table["column_names_original"]):
                    ...
                    if item[0] >= 0:
                        table["table_column_names_original"].append(
                            [
                                item[0],
                                table["table_names_original"][item[0]] + "." + item[1],
                            ]
                        )
                    else:
