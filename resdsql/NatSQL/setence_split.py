...
from natsql2sql.preprocess.sentence_analyse import (
    anaylse_punctuate,
    combine_none_subquestion,
    correct_121_pattern,
    correct_special_pattern,
    db_continue_correction,
    db_correction,
    easy_cut,
    final_correct_special_pattern,
    pattern_generate_col,
    pattern_sentence_analyse,
    re_analyse_sentence,
    reset_uncontinue_type,
    reshap_token,
    select_split,
    sentence_cut,
    sentence_dump,
...
def special_replace(str_):
    ...
    str_ = str_.replace(
        "Return in alphabetical order ", "In alphabetical order , show "
    )
...
def question_repair_select_part(sql, token_list, schema):
    def select_word_repair(
        sentences, table_matchs, list_idxs, schema: Schema_Token, sub_q, q_sql
    ):
        replace_old = []
        replace_new = []
        ...
        for sentence, table_match, list_idx in zip(sentences, table_matchs, list_idxs):
            (
                ...
                original_table,
                original_idx,
            ) = construct_select_data(
                sentence, table_match, sub_q.original_idx[list_idx], schema
            )
            for s, t, i in zip(availble_sent, availble_table, availble_idx):
                if (
                    len(s) > 1 and len(s[-1]) == 1 and not t[-1][0]
                ):  # and not datebase_match_tables(schema,q_sql.question_tokens[i[-1][0]],i[-1][0],q_sql.question_tokens,[i for i in range(len(schema.table_tokens))],return_all_match = True):
    ...
    sq = SubQuestion(
        copy.deepcopy(sql["question"]),
        copy.deepcopy(sql["question_type"]),
        copy.deepcopy(sql["table_match"]),
        copy.deepcopy(sql["question_tag"]),
        copy.deepcopy(sql["question_dep"]),
        copy.deepcopy(sql["question_entt"]),
        dict(),
    )
    ...
    sq.tokenize(qsql)
    replace_old, replace_new = select_word_repair(
        select_list, select_table, select_list_idx, schema, sq, qsql
    )
    return replace_old, replace_new


def final_cut(
    token_list, db_match, table_match, col_match, question, keep_original_question
):
        ...
        for tl, db, i in zip(
            ...
            if tl[0].text == "and":
                if (
                    ...
                    or i + 2 < len(db_match)
                    and (
                        (db_match[i + 1] and not token_list[i + 1][0].text.islower())
                        or (db_match[i + 2] and not token_list[i + 2][0].text.islower())
                    )
                    and i > 2
                    and (
                        (db_match[i - 1] and not token_list[i - 1][0].text.islower())
                        or (db_match[i - 2] and not token_list[i - 2][0].text.islower())
                        or (db_match[i - 3] and not token_list[i - 3][0].text.islower())
                    )
                    or i + 3 < len(db_match)
                    and (db_match[i + 3] and not token_list[i + 3][0].text.islower())
                    and i > 2
                    and (
                        (db_match[i - 1] and not token_list[i - 1][0].text.islower())
                        or (db_match[i - 2] and not token_list[i - 2][0].text.islower())
    ...
    if len(token_list) == len(db_match):
        for (i, tl), dbm in zip(enumerate(token_list), db_match):
            if (
                dbm
                and i > 0
                and token_list[i - 1][1] != tl[1]
                and db_match[i - 1] == dbm
            ):
                start_type = tl[1]
...
def generate_full_db_match(token_list, col_match, sql, schema):
    ...
    for start_idx, tok in enumerate(token_list):
        if (
            ...
            db_str = get_database_col(
                schema,
                sql["db_match"][start_idx],
                question_toks[
                    start_idx : start_idx
...
def preprocess_sql(
    ...
    other_words=[],
    all_word=[],
    keep_original_question=True,
    use_pattern_generate_col=False,
):
    """
    1. repair question
    2. calc table match
        ...
        for i, sql in enumerate(sqls):
            ...
            if ONE_ID:
                all_schema[sql["db_id"]] = Schema_Token(
                    _tokenizer, None, table_dict[sql["db_id"]], None
                )
            ...
            qq = question_repair(
                ...
                sql["question"],
                all_word,
                full_word[sql["db_id"]][0],
                full_word[sql["db_id"]][1],
                full_word[sql["db_id"]][2],
            )
            sql["question"] = " ".join([tok.text for tok in qq])
            sql["question"] = sql["question"].replace("'s '", "' s '")
            sql["question"] = special_replace(sql["question"])
            sql["question"] = special_word_modify(
                sql["question"], all_schema[sql["db_id"]]
            )
    ...
    for i, sql in enumerate(sqls):
        ...
        _, table_match = return_table_name(
            TokenString(None, list(sentences)), all_schema[sql["db_id"]]
        )
        offset = 0
        offset_list = []
        ...
        for si, sent in enumerate(sentences.sents):
            token_list += sentence_cut(
                sent, table_match, offset, si, len(list(sentences.sents))
            )
        ...
        token_list = re_analyse_sentence(token_list)
        token_list = anaylse_punctuate(
            token_list,
            sql["question"],
            all_schema[sql["db_id"]],
            all_word,
            keep_original_question,
        )
        ...
        sql["db_match"] = None
        token_list, previous_col_match, previous_db_match, patterns = (
            pattern_sentence_analyse(
                token_list, sql, all_schema[sql["db_id"]], sentence_num, 1
            )
        )
        token_list, previous_col_match, previous_db_match, patterns = (
            pattern_sentence_analyse(
                token_list, sql, all_schema[sql["db_id"]], sentence_num, 2, patterns
            )
        )
        ...
        token_list, offset, new_insert_idxs = pattern_generate_col(
            token_list,
            sql,
            all_schema[sql["db_id"]],
            previous_col_match,
        ...
        if new_insert_idxs:
            ...
            for ti, tok in enumerate(token_list):
                if (
                    tok[0].text[0] == "*"
                    and tok[0].text[-1] == "*"
                    and len(tok[0].text) >= 3
                ):
                    if (
                        sql["table_match"][ti]
                        or sql["db_match"][ti]
                        or sql["question_entt"][ti]
                    ):
                        for idx in new_insert_idxs:
        ...
        token_list = combine_none_subquestion(
            token_list, sql, all_schema[sql["db_id"]], sentence_num, col_match
        )
        token_list = combine_none_subquestion(
            token_list, sql, all_schema[sql["db_id"]], sentence_num, col_match
        )
        ...
        ########################################
        replace_old, replace_new = question_repair_select_part(
            sql, token_list, all_schema[sql["db_id"]]
        )
        if replace_old:
        ...
        if token_list:
            ...
            print(
                [
                    str(rl) + " " + tok
                    for rl, tok in zip(sql["question_type"], sql["question"].split(" "))
                ]
            )
    return sqls
...
if __name__ == "__main__":
    ...
    sqls = preprocess_sql(
        ...
        all_word=all_word,
        keep_original_question=args.keep_or,
