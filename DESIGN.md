...
For advanced users who need to extend the pipeline with custom steps:
...
# Define a custom post-processing step
class CustomPostprocessor(JsonListTransformer):
    """Custom step to add domain-specific post-processing."""
    ...
    async def process_item(self, item: dict) -> dict:
        ...
        # Example: Add query hints or optimization directives
        if "SELECT" in sql.upper():
            sql = sql + " -- Generated by MaskSQL"
...
print(result.postprocessed)  # True
```
...
class TextToSQLModelConfig:
    ...
    model_type: str = "openai"  # "openai", "anthropic", "custom", "local"
    ...
    # Custom model configuration (for local or custom endpoints)
...
class MaskSQL:
    ...
    def __init__(
        ...
        steps : Optional[List[JsonListTransformer]]
            Custom pipeline steps to add. These steps will be integrated into
            the pipeline at appropriate points (e.g., preprocessing, post-processing).
        ...
        # With custom steps
        >>> custom_steps = [CustomPreprocessor({}), CustomPostprocessor({})]
        >>> masksql = MaskSQL(config_file="config.yaml", steps=custom_steps)

        # With config object
        >>> config = MaskSQLConfig(database_path="./databases")
...
models:
  # Custom text-to-SQL model
  text_to_sql:
    model_type: "custom"
    model_name: "healthspark-sql-v1"
    temperature: 0.0
    max_tokens: 4096
    timeout: 120.0
    custom_endpoint: "https://api.healthspark.ai/v1/text-to-sql"
    custom_headers:
      Authorization: "Bearer ${HEALTHSPARK_API_KEY}"
      X-Model-Version: "v1"
...
### Example 5: Custom Pipeline Steps (HealthSpark Integration)
...
class HealthSparkPreprocessor(JsonListTransformer):
    """Custom preprocessor for HealthSpark medical terminology."""
    ...
    async def process_item(self, item: dict) -> dict:
        """Normalize medical terminology in questions."""
        question = item["question"]

        # Expand medical acronyms for better understanding
        for acronym, full_term in self.medical_terms.items():
            if acronym in question:
                question = question.replace(
                    acronym, f"{acronym} ({full_term})"
                )

        item["question"] = question
        item["healthspark_preprocessed"] = True
        return item


class HealthSparkValidator(JsonListTransformer):
    """Custom validator to check SQL against HealthSpark policies."""

    def __init__(self, config: dict):
        super().__init__(config)
        self.api_endpoint = config.get("validation_endpoint")

    async def process_item(self, item: dict) -> dict:
        """Validate generated SQL against HealthSpark compliance rules."""
        sql = item.get("sql", "")

        # Call HealthSpark validation API
        response = requests.post(
            self.api_endpoint,
            json={"sql": sql, "db_id": item["db_id"]}
        )

        validation_result = response.json()
        item["healthspark_validated"] = validation_result["is_valid"]
        item["compliance_score"] = validation_result["compliance_score"]
...
async def main():
    # Define custom steps
    custom_steps = [
        HealthSparkPreprocessor({
            "medical_terms": {"HIV": "Human Immunodeficiency Virus"}
        }),
        HealthSparkValidator({
            "validation_endpoint": "https://api.healthspark.ai/validate"
        })
    ]

    # Initialize MaskSQL with custom steps
    masksql = MaskSQL(
        config_file="healthspark_config.yaml",
        steps=custom_steps
    )

    # Execute query
    result = await masksql.query(
        question="How many patients with HIV were admitted?",
        db_id="hospital_db"
    )

    print(f"SQL: {result.sql}")
    print(f"Validated: {result.healthspark_validated}")
